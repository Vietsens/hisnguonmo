            ...
            try
            {
                ...
                if (dllFiles != null && dllFiles.Length > 0)
                {
                    ...
                    foreach (string s in dllFiles)
                    {
                        ...
                        var asm = Assembly.LoadFrom(s);

                        var TargetFramework = (TargetFrameworkAttribute)asm.GetCustomAttributes(typeof(TargetFrameworkAttribute)).FirstOrDefault();
                        if (TargetFramework != null)
                        {
                            if (!TargetFramework.FrameworkName.Contains("4.5") && !TargetFramework.FrameworkName.Contains("4.0"))
                            {
                                Inventec.Common.Logging.LogSystem.Debug("ERROR Framework: " + System.IO.Path.GetFileName(s) + "_______" + TargetFramework.FrameworkName);
                            }
