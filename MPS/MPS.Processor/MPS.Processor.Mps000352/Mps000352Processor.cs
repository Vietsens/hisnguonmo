...
using MPS.Processor.Mps000352.PDO;
...
namespace MPS.Processor.Mps000352
{
    class Mps000352Processor : AbstractProcessor
    {
        Mps000352PDO rdo;
        public Mps000352Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000352PDO)rdoBase;
        }
        ...
        List<string> GetListStringApprovalLogFromExpMestMedicineMaterial(List<V_HIS_EXP_MEST_MEDICINE> expMestMedicineList)
        {
            ...
            try
            {
                ...
                {
                    result = expMestMedicineList.Where(p => !string.IsNullOrEmpty(p.APPROVAL_LOGINNAME))
        ...
        List<string> GetListStringExpLogFromExpMestMedicineMaterial(List<V_HIS_EXP_MEST_MEDICINE> expMestMedicineList)
        {
            ...
            try
            {
                if (expMestMedicineList != null && expMestMedicineList.Count > 0)
                {
                    result = expMestMedicineList.Where(p => !string.IsNullOrEmpty(p.EXP_LOGINNAME))
        ...
        List<string> GetListStringExpTimeLogFromExpMestMedicineMaterial(List<V_HIS_EXP_MEST_MEDICINE> expMestMedicineList)
        {
            ...
            try
            {
                if (expMestMedicineList != null && expMestMedicineList.Count > 0)
                {
                    result = expMestMedicineList.Where(p => p.EXP_TIME != null)
        ...
        private void SetSingleKey()
        {
            try
            {
                ...
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.NUM_ORDER, rdo.Transaction.NUM_ORDER));

                if (this.rdo._SaleExpMests != null && this.rdo._SaleExpMests.Count > 0)
                {
                    SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    ...
                    SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.IS_NOT_IN_WORKING_TIME, this.rdo._SaleExpMests.FirstOrDefault().IS_NOT_IN_WORKING_TIME));
                    if (this.rdo._SaleExpMests.FirstOrDefault().CASHIER_ROOM_ID > 0)
                    {
                        var cashierRoom = BackendDataWorker.Get<HIS_CASHIER_ROOM>().Where(ca => ca.ID == this.rdo._SaleExpMests.FirstOrDefault().CASHIER_ROOM_ID).FirstOrDefault(); ;
                        if(cashierRoom!=null)
                            SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.CASHIER_ROOM_NAME, cashierRoom.CASHIER_ROOM_NAME));
                    }
                }
                ...
                if (this.rdo._Medicines != null && this.rdo._Medicines.Count > 0)
                {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            Mps000352ADO ado = new Mps000352ADO(dic.Value);
                ...
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.EXP_TIME, expTime));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.APPROVAL_LOGINNAME, approvalLoginname));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.EXP_LOGINNAME, expLoginName));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_NOT_ROUND, totalPriceNotRound));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.DISCOUNT, discount));
                string sumText = String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT, Inventec.Common.Number.Convert.NumberToString(totalPrice - discount)));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice - discount)))));

                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_NO_TH, totalPriceNoTh));

                string sumNotRoundText = String.Format("{0:0}", totalPriceNotRound);
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_NOT_ROUND_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumNotRoundText)));
                decimal sumAfterDiscountNotRound = Math.Round((totalPriceNotRound - discount), 0, MidpointRounding.AwayFromZero);
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_NOT_ROUND, sumAfterDiscountNotRound));
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_NOT_ROUND_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(String.Format("{0:0}", sumAfterDiscountNotRound))));
                ...
                SetSingleKey(new KeyValue(Mps000352ExtendSingleKey.MOBA_IMP_MEST_COUNT, mobaImpMestCount));
                rdo.listAdo = rdo.listAdo.OrderBy(o => o.NUM_ORDER).ThenByDescending(t => t.TYPE_ID).ToList();
        ...
        private void SetBarcodeKey()
        {
            try
            {
                if (rdo.Transaction != null && !String.IsNullOrEmpty(rdo.Transaction.TRANSACTION_CODE))
                {
                    ...
                    dicImage.Add(Mps000352ExtendSingleKey.TRANSACTION_CODE_BAR, barcodeTransactionCode);
                }
                ...
                if (rdo._SaleExpMests != null && rdo._SaleExpMests.Count > 0)
                {
                    ...
                    dicImage.Add(Mps000352ExtendSingleKey.EXP_MEST_CODE_BAR, barcodeExpMestCode);
                }
            }
            catch (Exception ex)
            {
