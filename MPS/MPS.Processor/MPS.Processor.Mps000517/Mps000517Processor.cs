...
{
    public class Mps000517Processor : AbstractProcessor
    {
        Mps000517PDO rdo;
        ...
        // Mps000517: tầng gom nhóm "Loại XN" (Huyết học / Sinh hóa / Miễn dịch / Vi sinh...)
        List<TestTypeADO> ListTestType = new List<TestTypeADO>();

        // Đa mẫu: lấy mẫu / y lệnh ĐẦU TIÊN làm đại diện cho các key header & barcode dùng 1 giá trị.
        private V_LIS_SAMPLE currentSample;
        private HIS_SERVICE_REQ currentServiceReq;

        public Mps000517Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000517PDO)rdoBase;
            this.currentSample = (rdo != null && rdo.currentSamples != null) ? rdo.currentSamples.FirstOrDefault() : null;
            this.currentServiceReq = (rdo != null && rdo.currentServiceReqs != null) ? rdo.currentServiceReqs.FirstOrDefault() : null;
        }
        ...
        public void SetBarcodeKey()
        {
            try
            {
                if (rdo.currentTreatment != null)
                {
                    ...
                    dicImage.Add(Mps000517ExtendSingleKey.BARCODE_PATIENT_CODE_STR, barcodePatientCode);
                    ...
                    dicImage.Add(Mps000517ExtendSingleKey.BARCODE_TREATMENT_CODE_STR, barcodeTreatment);
                }

                if (currentServiceReq != null)
                {
                    Inventec.Common.BarcodeLib.Barcode barcodeServiceReqCode = new Inventec.Common.BarcodeLib.Barcode(currentServiceReq.SERVICE_REQ_CODE);
                    ...
                    dicImage.Add(Mps000517ExtendSingleKey.BARCODE_SERVICE_REQ_CODE_STR, barcodeServiceReqCode);
                }

                if (currentSample != null && !String.IsNullOrWhiteSpace(currentSample.BARCODE))
                {
                    Inventec.Common.BarcodeLib.Barcode barBarcode = new Inventec.Common.BarcodeLib.Barcode(currentSample.BARCODE);
                    ...
                    dicImage.Add(Mps000517ExtendSingleKey.BAR_BARCODE_STR, barBarcode);
                }
                    ...
                    foreach (string url in logos)
                    {
                        try
                        {
                            ...
                            if (imgLogo != null && imgLogo.Length > 0)
                            {
                                string key = "";
                                if (index == 0)
                                    key = Mps000517ExtendSingleKey.IMAGE_ISO_LOGO_STR;
                                else
                                {
                                    key = String.Format("{0}_{1}", Mps000517ExtendSingleKey.IMAGE_ISO_LOGO_STR, index);
                                    index++;
                                }
                                imgLogo.Position = 0;
        ...
        public void SetSingleKey()
        {
            try
            {
                ...
                // Mps000517: gộp toàn bộ mã barcode + mã y lệnh của các mẫu đã chọn để in lên header.
                if (rdo.currentSamples != null && rdo.currentSamples.Count > 0)
                {
                    var barcodes = rdo.currentSamples.Where(o => o != null && !String.IsNullOrWhiteSpace(o.BARCODE)).Select(o => o.BARCODE).Distinct().ToList();
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.SAMPLE_BARCODEs, String.Join(", ", barcodes)));
                }
                if (rdo.currentServiceReqs != null && rdo.currentServiceReqs.Count > 0)
                {
                    var reqCodes = rdo.currentServiceReqs.Where(o => o != null && !String.IsNullOrWhiteSpace(o.SERVICE_REQ_CODE)).Select(o => o.SERVICE_REQ_CODE).Distinct().ToList();
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.SERVICE_REQ_CODEs, String.Join(", ", reqCodes)));
                }

                if (currentSample != null && String.IsNullOrWhiteSpace(currentSample.SERVICE_REQ_CODE))
                {
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.TDL_PATIENT_NAME, currentSample.LAST_NAME + " " + currentSample.FIRST_NAME));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.PATIENT_CONDITION_CODE, currentSample.PATIENT_CONDITION_CODE));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.PATIENT_CONDITION_NAME, currentSample.PATIENT_CONDITION_NAME));
                    if (currentSample.GENDER_CODE == "01")
                    {
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.GENDER_NAME, "Nữ"));
                    }
                    else if (currentSample.GENDER_CODE == "02")
                    {
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.GENDER_NAME, "Nam"));
                    }
                    else if (currentSample.GENDER_CODE == "03")
                    {
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.GENDER_NAME, "Không xác định"));
                    }
                    else
                    {
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.GENDER_NAME, ""));
                    }

                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.TDL_PATIENT_DOB, currentSample.DOB));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.VIR_ADDRESS, currentSample.ADDRESS));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.SAMPLE_NAME, currentSample.SAMPLE_NAME));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.SAMPLE_STATE, currentSample.SAMPLE_STATE));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.SAMPLE_POSITION, currentSample.SAMPLE_POSITION));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.CARTRIDGE_LOT, currentSample.CARTRIDGE_LOT));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.CARTRIDGE_CAL, currentSample.CARTRIDGE_CAL));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.RANGE_TIMES, currentSample.RANGE_TIMES));

                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_HEIN, ""));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_NOT_HEIN, ""));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_RIGHT_ROUTE, ""));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_NOT_RIGHT_ROUTE, ""));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.HEIN_CARD_NUMBER, ""));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.ICD_MAIN_TEXT, ""));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.ICD_NAME, ""));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, ""));
                }
                if (rdo.PatientTypeAlter != null && !String.IsNullOrWhiteSpace(rdo.PatientTypeAlter.HEIN_CARD_NUMBER))
                {
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.HEIN_CARD_NUMBER, rdo.PatientTypeAlter.HEIN_CARD_NUMBER));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.HEIN_CARD_FROM_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.PatientTypeAlter.HEIN_CARD_FROM_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.HEIN_CARD_TO_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.PatientTypeAlter.HEIN_CARD_TO_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.HEIN_MEDI_ORG_CODE, rdo.PatientTypeAlter.HEIN_MEDI_ORG_CODE));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.HEIN_ADDRESS, rdo.PatientTypeAlter.ADDRESS));
                    if (rdo.PatientTypeAlter.RIGHT_ROUTE_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinRightRouteType.HeinRightRouteTypeCode.EMERGENCY)
                    {
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, "X"));
                    }
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_HEIN, "X"));
                    if (rdo.PatientTypeAlter.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.TRUE)
                    {
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_RIGHT_ROUTE, "X"));
                    }
                    else
                    {
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_NOT_RIGHT_ROUTE, "X"));
                    }
                }
                else
                {
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.IS_NOT_HEIN, "X"));
                }

                if (rdo.currentTreatment != null)
                {
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.OPEN_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.currentTreatment.IN_TIME)));
                    if (rdo.currentTreatment.OUT_TIME.HasValue)
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.CLOSE_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.currentTreatment.OUT_TIME.Value)));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.AGE, AgeUtil.CalculateFullAge(rdo.currentTreatment.TDL_PATIENT_DOB)));

                    HIS_MEDICINE_TYPE vaccine = GetVaccineFromVaccineId(rdo.currentTreatment.VACCINE_ID);
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.VACCINE_CODE, vaccine != null ? vaccine.MEDICINE_TYPE_CODE : null));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.VACCINE_NAME, vaccine != null ? vaccine.MEDICINE_TYPE_NAME : null));
                }

                if (rdo.ServiceParent != null)
                {
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.SERVICE_NAME_PARENT, rdo.ServiceParent.SERVICE_NAME));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.PARENT_CODE, rdo.ServiceParent.SERVICE_CODE));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.PROCESS_CODE_PARENT, rdo.ServiceParent.PROCESS_CODE));
                }

                SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.LOGIN_USER_NAME, Inventec.UC.Login.Base.ClientTokenManagerStore.ClientTokenManager.GetUserName()));
                if (rdo.currentTreatBedRoom != null)
                {
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.BED_CODE, rdo.currentTreatBedRoom.BED_CODE));
                    SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.BED_NAME, rdo.currentTreatBedRoom.BED_NAME));
                }
                if (currentSample != null)
                    AddObjectKeyIntoListkey<V_LIS_SAMPLE>(currentSample, false);
                if (currentServiceReq != null)
                    AddObjectKeyIntoListkey<HIS_SERVICE_REQ>(currentServiceReq, false);
                if (rdo.currentTreatment != null)
                    AddObjectKeyIntoListkey<HIS_TREATMENT>(rdo.currentTreatment, false);
                if (currentSample != null)
                    Inventec.Common.Logging.LogSystem.Debug("currentSample.SERVICE_REQ_CODE: " + currentSample.SERVICE_REQ_CODE);
                if (ListTestParent != null && ListTestParent.Count > 0)
                {
                    ...
                    if (sampleTypeName != null && sampleTypeName.Count > 0)
                        SetSingleKey(new KeyValue(Mps000517ExtendSingleKey.SAMPLE_TYPE_NAMEs, String.Join(", ", sampleTypeName)));
                }
            ...
            try
            {
                ...
                if (testIndexs != null && testIndexs.Count > 0)
                {
                    ...
                    foreach (var group in groupListResult)
                    {
                        ...
                        // Mps000517: gán loại XN (test type) lấy thẳng từ V_HIS_SERVICE — chiều gom nhóm thứ 2.
                        hisSereServTeinSDO.TEST_TYPE_ID = service != null ? service.TEST_TYPE_ID : null;
                        hisSereServTeinSDO.TEST_TYPE_CODE = service != null ? service.TEST_TYPE_CODE : null;
                        hisSereServTeinSDO.TEST_TYPE_NAME = service != null ? service.TEST_TYPE_NAME : null;
                        ...
                        // Mps000517: khóa gom nhóm tầng giữa = nhóm DV cha + loại XN.
                        hisSereServTeinSDO.GROUP_ID = hisSereServTeinSDO.SERVICE_PARENT_ID + "_" + (hisSereServTeinSDO.TEST_TYPE_ID.HasValue ? hisSereServTeinSDO.TEST_TYPE_ID.Value.ToString() : "0");
                        ...
                        if (lstResultItem != null && (lstResultItem.Count > 1 || (lstResultItem.Count == 1 && testIndFist != null && testIndFist.IS_NOT_SHOW_SERVICE != 1)))
                        {
                            foreach (var ssTein in lstResultItem)
                            {
                                ...
                                // Mps000517: copy loại XN xuống dòng chỉ số (để có thể lọc/hiển thị nếu cần).
                                hisSereServTein.TEST_TYPE_ID = hisSereServTeinSDO.TEST_TYPE_ID;
                                hisSereServTein.TEST_TYPE_CODE = hisSereServTeinSDO.TEST_TYPE_CODE;
                                hisSereServTein.TEST_TYPE_NAME = hisSereServTeinSDO.TEST_TYPE_NAME;
                                hisSereServTein.GROUP_ID = hisSereServTeinSDO.GROUP_ID;
                ...
                foreach (var hisSereServTeinSDO in this.ListTestParent)
                {
                    ...
                    if (rdo.currentTreatment != null)
                        ...
                        testIndexRange = GetTestIndexRange(currentSample != null ? (currentSample.DOB ?? 0) : 0, rdo.genderId, hisSereServTeinSDO.TEST_INDEX_CODE, ref testIndexRangeAll);
                    if (testIndexRange != null)
                    {
                        ProcessMaxMixValue(hisSereServTeinSDO, testIndexRange);
                    }
                ...
                // Mps000517: sắp xếp ưu tiên Nhóm DV cha → Loại XN → Dịch vụ → Tên.
                this.ListTestParent = this.ListTestParent != null && this.ListTestParent.Count > 0
                    ? this.ListTestParent.OrderByDescending(o => o.SERVICE_PARENT_ORDER).ThenBy(o => o.TEST_TYPE_ID).ThenBy(o => o.SERVICE_NUM_ORDER).ThenBy(p => p.SERVICE_NAME).ToList()
                    : this.ListTestParent;
                Inventec.Common.Logging.LogSystem.Debug("ListTestParent count: " + ListTestParent.Count);

                // Mps000517: dựng tầng "Loại XN" — gom ListTestParent theo GROUP_ID (nhóm cha + loại XN).
                this.ListTestType = (this.ListTestParent != null && this.ListTestParent.Count > 0)
                    ? this.ListTestParent
                        .GroupBy(o => o.GROUP_ID)
                        .Select(g =>
                        {
                            var f = g.First();
                            return new TestTypeADO
                            {
                                GROUP_ID = f.GROUP_ID,
                                SERVICE_PARENT_ID = f.SERVICE_PARENT_ID,
                                TEST_TYPE_ID = f.TEST_TYPE_ID,
                                TEST_TYPE_CODE = f.TEST_TYPE_CODE,
                                TEST_TYPE_NAME = String.IsNullOrWhiteSpace(f.TEST_TYPE_NAME) ? "Khác" : f.TEST_TYPE_NAME,
                                NUM_ORDER = f.TEST_TYPE_ID
                            };
                        })
                        .OrderBy(o => o.SERVICE_PARENT_ID).ThenBy(o => o.TEST_TYPE_ID)
                        .ToList()
                    : new List<TestTypeADO>();
                Inventec.Common.Logging.LogSystem.Debug("ListTestType count: " + ListTestType.Count);
                ...
                this.ListTestChild = this.ListTestChild != null && this.ListTestChild.Count > 0
                    ...
                    : this.ListTestChild;
                Inventec.Common.Logging.LogSystem.Debug("ListTestChild count: " + ListTestChild.Count);
                ...
                Inventec.Common.Logging.LogSystem.Debug("ListServiceParent count: " + ListServiceParent.Count);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        /// Tao ra cac doi tuong du lieu su dung trong thu vien xu ly file excel
        /// </summary>
        public override bool ProcessData()
        {
            ...
            try
            {
                ...
                objectTag.AddObjectData(store, "ListTestType", this.ListTestType);
                ...
                // Lồng nhóm 3 tầng: Nhóm DV cha ▷ Loại XN ▷ Dịch vụ ▷ Chỉ số.
                objectTag.AddRelationship(store, "ServiceParent", "ListTestType", "ID", "SERVICE_PARENT_ID");
                objectTag.AddRelationship(store, "ListTestType", "ListTestParent", "GROUP_ID", "GROUP_ID");
        ...
        public override string ProcessPrintLogData()
        {
            ...
            try
            {
                string reqCodes = rdo.currentServiceReqs != null
                    ? String.Join(",", rdo.currentServiceReqs.Where(o => o != null).Select(o => o.SERVICE_REQ_CODE).Distinct())
                    : "";
                log = LogDataServiceReq(rdo.currentTreatment.TREATMENT_CODE, reqCodes, "In phiếu kết quả xét nghiệm tổng hợp đa mẫu");
            }
            catch (Exception ex)
            {
                log = "";
        ...
        public override string ProcessUniqueCodeData()
        {
            ...
            try
            {
                if (rdo != null && rdo.currentTreatment != null && rdo.currentServiceReqs != null && rdo.currentServiceReqs.Count > 0)
                {
                    string reqCodes = String.Join(",", rdo.currentServiceReqs.Where(o => o != null && !String.IsNullOrWhiteSpace(o.SERVICE_REQ_CODE)).Select(o => o.SERVICE_REQ_CODE).Distinct());
                    result = String.Format("{0} TREATMENT_CODE:{1} SERVICE_REQ_CODE:{2}",
                        this.printTypeCode,
                        rdo.currentTreatment.TREATMENT_CODE,
                        reqCodes);
                }
            }
            catch (Exception ex)
