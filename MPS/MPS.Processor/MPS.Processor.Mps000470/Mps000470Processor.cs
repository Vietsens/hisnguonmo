...
using MOS.Filter;
using MPS.Processor.Mps000470.PDO;
...
namespace MPS.Processor.Mps000470
{
    public class Mps000470Processor: AbstractProcessor
    {
        Mps000470PDO rdo;
        public Mps000470Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000470PDO)rdoBase;
        }
        ...
        public void SetBarcodeKey()
        {
            try
            {
                if (rdo.currentTreatment != null)
                {
                    ...
                    dicImage.Add(Mps000470ExtendSingleKey.BARCODE_PATIENT_CODE_STR, barcodePatientCode);
                    ...
                    dicImage.Add(Mps000470ExtendSingleKey.BARCODE_TREATMENT_CODE_STR, barcodeTreatment);
                }
                ...
                if (rdo.currentServiceReq != null)
                {
                    ...
                    dicImage.Add(Mps000470ExtendSingleKey.BARCODE_SERVICE_REQ_CODE_STR, barcodeServiceReqCode);
                }
                ...
                if (rdo.currentSample != null && !String.IsNullOrWhiteSpace(this.rdo.currentSample.BARCODE))
                {
                    ...
                    dicImage.Add(Mps000470ExtendSingleKey.BAR_BARCODE_STR, barBarcode);
                }
                    ...
                    foreach (string url in logos)
                    {
                        try
                        {
                            ...
                            if (imgLogo != null && imgLogo.Length > 0)
                            {
                                string key = "";
                                if (index == 0)
                                    key = Mps000470ExtendSingleKey.IMAGE_ISO_LOGO_STR;
                                else
                                {
                                    key = String.Format("{0}_{1}", Mps000470ExtendSingleKey.IMAGE_ISO_LOGO_STR, index);
                                    index++;
                                }
                                imgLogo.Position = 0;
        ...
        public void SetSingleKey()
        {
            try
            {
                ...
                {
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.TDL_PATIENT_NAME, rdo.currentSample.LAST_NAME + " " + rdo.currentSample.FIRST_NAME));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.PATIENT_CONDITION_CODE, rdo.currentSample.PATIENT_CONDITION_CODE));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.PATIENT_CONDITION_NAME, rdo.currentSample.PATIENT_CONDITION_NAME));
                    if (rdo.currentSample.GENDER_CODE == "01")
                    {
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.GENDER_NAME, "Nữ"));
                    }
                    else if (rdo.currentSample.GENDER_CODE == "02")
                    {
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.GENDER_NAME, "Nam"));
                    }
                    else if (rdo.currentSample.GENDER_CODE == "03")
                    {
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.GENDER_NAME, "Không xác định"));
                    }
                    else
                    {
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.GENDER_NAME, ""));
                    }

                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.TDL_PATIENT_DOB, rdo.currentSample.DOB));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.VIR_ADDRESS, rdo.currentSample.ADDRESS));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_HEIN, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_NOT_HEIN, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_RIGHT_ROUTE, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_NOT_RIGHT_ROUTE, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.HEIN_CARD_NUMBER, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.ICD_MAIN_TEXT, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.ICD_NAME, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.BED_NAME, ""));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, ""));
                }
                if (rdo.PatientTypeAlter != null && !String.IsNullOrWhiteSpace(rdo.PatientTypeAlter.HEIN_CARD_NUMBER))
                {
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.HEIN_CARD_NUMBER, rdo.PatientTypeAlter.HEIN_CARD_NUMBER));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.HEIN_CARD_FROM_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.PatientTypeAlter.HEIN_CARD_FROM_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.HEIN_CARD_TO_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.PatientTypeAlter.HEIN_CARD_TO_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.HEIN_MEDI_ORG_CODE, rdo.PatientTypeAlter.HEIN_MEDI_ORG_CODE));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.HEIN_ADDRESS, rdo.PatientTypeAlter.ADDRESS));
                    if (rdo.PatientTypeAlter.RIGHT_ROUTE_TYPE_CODE == MOS.LibraryHein.Bhyt.HeinRightRouteType.HeinRightRouteTypeCode.EMERGENCY)
                    {
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, "X"));
                    }
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_HEIN, "X"));
                    if (rdo.PatientTypeAlter.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.TRUE)
                    {
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_RIGHT_ROUTE, "X"));
                    }
                    else
                    {
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_NOT_RIGHT_ROUTE, "X"));
                    }
                }
                else
                {
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.IS_NOT_HEIN, "X"));
                }

                if (rdo.currentTreatment != null)
                {
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.OPEN_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.currentTreatment.IN_TIME)));
                    if (rdo.currentTreatment.OUT_TIME.HasValue)
                        SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.CLOSE_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.currentTreatment.OUT_TIME.Value)));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.AGE, AgeUtil.CalculateFullAge(rdo.currentTreatment.TDL_PATIENT_DOB)));

                    HIS_MEDICINE_TYPE vaccine = GetVaccineFromVaccineId(rdo.currentTreatment.VACCINE_ID);
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.VACCINE_CODE, vaccine != null ? vaccine.MEDICINE_TYPE_CODE : null));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.VACCINE_NAME, vaccine != null ? vaccine.MEDICINE_TYPE_NAME : null));
                }

                if (rdo.ServiceParent != null)
                {
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.SERVICE_NAME_PARENT, rdo.ServiceParent.SERVICE_NAME));
                    SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.SERVICE_CODE_PARENT, rdo.ServiceParent.SERVICE_CODE));
                }

                SetSingleKey(new KeyValue(Mps000470ExtendSingleKey.LOGIN_USER_NAME, Inventec.UC.Login.Base.ClientTokenManagerStore.ClientTokenManager.GetUserName()));
