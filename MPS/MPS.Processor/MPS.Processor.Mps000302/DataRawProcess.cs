    ...
    public class DataRawProcess
    {
        ...
        public static PatyAlterBhytADO PatyAlterBHYTRawToADO(HIS_PATIENT_TYPE_ALTER patyAlter, List<HIS_PATIENT_TYPE_ALTER> patyAlterAlls, V_HIS_TREATMENT treatment, HIS_BRANCH branch, List<HIS_TREATMENT_TYPE> treatmentTypes, V_HIS_PATIENT_TYPE_ALTER currentPatyAlter, List<SereServADO> listSereServ)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(patyAlter.HEIN_CARD_NUMBER))
                {
                    ...
                    patyAlterBhytADO.HEIN_CARD_NUMBER_6 = patyAlter.HEIN_CARD_NUMBER.Substring(10);
                }
                ...
                patyAlterBhytADO.RATIO_STR = GetDefaultHeinRatioForView(patyAlterBhytADO.HEIN_CARD_NUMBER, treatmentType.HEIN_TREATMENT_TYPE_CODE, branch.HEIN_LEVEL_CODE, patyAlterBhytADO.RIGHT_ROUTE_CODE, currentPatyAlter.FACILITY_CLASS, currentPatyAlter.FORMER_LEVEL_CODE, currentPatyAlter.CLASSIFY_POINT ?? 0, treatment != null ? treatment.CLINICAL_IN_TIME ?? 0 : 0);
                ...
                if (patyAlterAlls != null && treatment != null)
                {
                    ...
                    if (patyAlterBhytADO.LOG_TIME < 20210101000000 && !listSereServ.Exists(o => o.TDL_INTRUCTION_DATE < 20210101000000)
                        && patyAlterBhytADO.LEVEL_CODE == MOS.LibraryHein.Bhyt.HeinLevel.HeinLevelCode.PROVINCE
                        && patyAlterBhytADO.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.FALSE
                        && treatment.TDL_TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNOITRU)
                    {
                        patyAlterBhytADO.KBCB_TIME_FROM_STR = Inventec.Common.DateTime.Convert.TimeNumberToDateString(listSereServ.Min(o => o.TDL_INTRUCTION_TIME));
                    }
                    else if (patyAlterBhytADO.LOG_TIME < (patyAlterBhytADO.HEIN_CARD_FROM_TIME ?? 0))
                    {
                        patyAlterBhytADO.KBCB_TIME_FROM_STR = Inventec.Common.DateTime.Convert.TimeNumberToTimeString(patyAlterBhytADO.HEIN_CARD_FROM_TIME ?? 0);
                    }

                    HIS_PATIENT_TYPE_ALTER patientTypeAlterNext = patyAlterAlls.FirstOrDefault(o =>
                        o.LOG_TIME > patyAlterBhytADO.LOG_TIME
                        &&
                        (PatientTypeAlterProcessor.ToString(o) != PatientTypeAlterProcessor.ToString(patyAlterBhytADO)));

                    if (patientTypeAlterNext != null)
                    {
                        if ((patyAlterBhytADO.HEIN_CARD_TO_TIME ?? 0) > patientTypeAlterNext.LOG_TIME)
                        {
                            patyAlterBhytADO.KBCB_TIME_TO_STR = Inventec.Common.DateTime.Convert.TimeNumberToTimeString(patyAlterBhytADO.HEIN_CARD_TO_TIME ?? 0);
                        }
                        else
                        {
                    ...
                    else
                    {
                        if (patyAlterBhytADO.LOG_TIME < 20210101000000 && !listSereServ.Exists(o => o.TDL_INTRUCTION_DATE > 20210101000000)
                            ...
                            && treatment.TDL_TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNOITRU)
                        {
                            patyAlterBhytADO.KBCB_TIME_TO_STR = Inventec.Common.DateTime.Convert.TimeNumberToDateString(listSereServ.Max(o => o.TDL_INTRUCTION_TIME));
                        }
        ...
        public static PatyAlterBhytADO PatyAlterBHYTRawToADO(V_HIS_PATIENT_TYPE_ALTER patyAlter, HIS_BRANCH branch, List<HIS_TREATMENT_TYPE> treatmentTypes, V_HIS_TREATMENT treatment)
        {
            ...
            try
            {
                ...
                patyAlterBhytADO.RATIO_STR = GetDefaultHeinRatioForView(patyAlterBhytADO.HEIN_CARD_NUMBER, patyAlter.HEIN_TREATMENT_TYPE_CODE, branch.HEIN_LEVEL_CODE, patyAlterBhytADO.RIGHT_ROUTE_CODE, patyAlter.FACILITY_CLASS, patyAlter.FORMER_LEVEL_CODE, patyAlter.CLASSIFY_POINT ?? 0, treatment != null ? treatment.CLINICAL_IN_TIME ?? 0 : 0);
