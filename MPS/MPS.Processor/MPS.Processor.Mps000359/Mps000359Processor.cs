...
namespace MPS.Processor.Mps000359
{
    public partial class Mps000359Processor : AbstractProcessor
    {
        ...
        private List<SereServADO> sereServNoPriceADOs { get; set; }

        Mps000359PDO rdo;
        public Mps000359Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000359PDO)rdoBase;
        }
        ...
        internal void SetBarcodeKey()
        {
            try
            {
                ...
                dicImage.Add(Mps000359ExtendSingleKey.TREATMENT_CODE_BAR, barcodeTreatment);
            }
            catch (Exception ex)
            {
        ...
        public override bool ProcessData()
        {
            ...
            try
            {
                ...
                SetBarcodeKey();
                //SetImageKey();
                if ((sereServADOs == null || sereServADOs.Count == 0) && (sereServNoPriceADOs == null || sereServNoPriceADOs.Count <= 0))
                    return false;
                ...
                objectTag.AddObjectData(store, "GroupPatientType", GroupPatientType);

                objectTag.AddRelationship(store, "GroupPatientType", "PatyAlterBHYT", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientType", "Service", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientType", "HeinServiceType", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientType", "HeinServiceTypeBed", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientType", "ReqExeDepartment", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                ...
                ProcessNoPrice(objectTag, store);
                result = true;
            }
            catch (Exception ex)
            {
        ...
        private void ProcessNoPrice(Inventec.Common.FlexCellExport.ProcessObjectTag objectTag, Inventec.Common.FlexCellExport.Store store)
        {
            try
            {
                ...
                List<GroupPatientTypeADO> GroupPatientType = null;

                GroupDisplayProcess(this.sereServNoPriceADOs, ref patyAlterBHYTADOs, ref heinServiceTypeADOs, ref HeinServiceTypeBeds, ref GroupDepartments, ref GroupPatientType);

                objectTag.AddObjectData(store, "HeinServiceTypeNoPrice", heinServiceTypeADOs);
                objectTag.AddObjectData(store, "ServiceNoPrice", this.sereServNoPriceADOs);
                objectTag.AddObjectData(store, "PatyAlterBHYTNoPrice", patyAlterBHYTADOs);
                objectTag.AddObjectData(store, "PatyAlterBHYTAllNoPrice", patyAlterBHYTADOs);
                objectTag.AddObjectData(store, "HeinServiceTypeBedNoPrice", HeinServiceTypeBeds);
                objectTag.AddObjectData(store, "ReqExeDepartmentNoPrice", GroupDepartments);
                objectTag.AddObjectData(store, "GroupPatientTypeNoPrice", GroupPatientType);

                objectTag.AddRelationship(store, "GroupPatientTypeNoPrice", "PatyAlterBHYTNoPrice", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientTypeNoPrice", "ServiceNoPrice", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientTypeNoPrice", "HeinServiceTypeNoPrice", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientTypeNoPrice", "HeinServiceTypeBedNoPrice", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");
                objectTag.AddRelationship(store, "GroupPatientTypeNoPrice", "ReqExeDepartmentNoPrice", "PATIENT_TYPE_ID", "PATIENT_TYPE_ID");

                objectTag.AddRelationship(store, "PatyAlterBHYTNoPrice", "ServiceNoPrice", "KEY", "KEY_PATY_ALTER");
                objectTag.AddRelationship(store, "PatyAlterBHYTNoPrice", "HeinServiceTypeNoPrice", "KEY", "KEY_PATY_ALTER");
                objectTag.AddRelationship(store, "PatyAlterBHYTNoPrice", "HeinServiceTypeBedNoPrice", "KEY", "KEY_PATY_ALTER");
                objectTag.AddRelationship(store, "PatyAlterBHYTNoPrice", "ReqExeDepartmentNoPrice", "KEY", "KEY_PATY_ALTER");

                objectTag.AddRelationship(store, "ReqExeDepartmentNoPrice", "ServiceNoPrice", "GROUP_DEPARTMENT_ID", "GROUP_DEPARTMENT_ID");
                objectTag.AddRelationship(store, "ReqExeDepartmentNoPrice", "HeinServiceTypeNoPrice", "GROUP_DEPARTMENT_ID", "GROUP_DEPARTMENT_ID");
                objectTag.AddRelationship(store, "ReqExeDepartmentNoPrice", "HeinServiceTypeBedNoPrice", "GROUP_DEPARTMENT_ID", "GROUP_DEPARTMENT_ID");

                objectTag.AddRelationship(store, "HeinServiceTypeNoPrice", "ServiceNoPrice", "ID", "HEIN_SERVICE_TYPE_ID");
                objectTag.AddRelationship(store, "HeinServiceTypeNoPrice", "HeinServiceTypeBedNoPrice", "ID", "PARENT_ID");

                objectTag.AddRelationship(store, "HeinServiceTypeBedNoPrice", "ServiceNoPrice", "ID", "HEIN_SERVICE_TYPE_PARENT_1_ID");
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        void ProcessSingleKey()
        {
            try
            {
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.RATIO_STR, rdo.SingleKeyValue.ratio));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_CODE, rdo.Treatment.TREATMENT_CODE));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.USERNAME_RETURN_RESULT, rdo.SingleKeyValue.userNameReturnResult));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.STATUS_TREATMENT_OUT, rdo.SingleKeyValue.statusTreatmentOut));
                if (rdo.transReq != null)
                {
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.PAYMENT_AMOUNT, rdo.transReq.AMOUNT));
                }
                if (rdo.CurrentPatyAlter != null)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TYPE_INDEX, typeIndex));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TYPE_NAME, typeName));

                    if (rdo.CurrentPatyAlter.FREE_CO_PAID_TIME.HasValue)
                    {
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.FREE_CO_PAID_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.CurrentPatyAlter.FREE_CO_PAID_TIME.Value)));
                    }
                }
                ...
                if (rdo.PatientTypeAlterAlls != null && rdo.PatientTypeAlterAlls.Count > 0)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.HEIN_MEDI_ORG_CODE, heinMediOrgCode));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.HEIN_MEDI_ORG_NAME, heinMediOrgName));
                    ...
                    if (patyAlterBhytADO != null)
                    {
                        ...
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.IS_HEIN, "X"));
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.HEIN_CARD_ADDRESS, patyAlterBhytADO.ADDRESS));
                        ...
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.IS_NOT_HEIN, "X"));
                }
                else
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.IS_NOT_HEIN, "X"));
                ...
                if (rdo.DepartmentTrans != null && rdo.DepartmentTrans.Count > 0)
                {
                    ...
                    {
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.OPEN_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.DepartmentTrans[0].DEPARTMENT_IN_TIME ?? 0)));
                    }

                    if (rdo.DepartmentTrans[rdo.DepartmentTrans.Count - 1] != null && rdo.DepartmentTrans.Count > 1)
                    {
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.DEPARTMENT_NAME_CLOSE_TREATMENT, rdo.DepartmentTrans[rdo.DepartmentTrans.Count - 1].DEPARTMENT_NAME));
                    }
                }

                if (!String.IsNullOrEmpty(rdo.SingleKeyValue.roomName))
                {
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.ROOM_NAME, rdo.SingleKeyValue.roomName));
                }
                ...
                if (rdo.Treatment != null)
                {
                    ...
                    {
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.CLINICAL_IN_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.Treatment.CLINICAL_IN_TIME.Value)));
                    }

                    if (rdo.Treatment.OUT_TIME.HasValue)
                    {
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.CLOSE_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.Treatment.OUT_TIME.Value)));
                    }
                    ...
                    if (rdo.Treatment.END_DEPARTMENT_ID.HasValue)
                    {
                        ...
                        if (department != null)
                        {
                            SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.DEPARTMENT_BHYT_CODE, department.BHYT_CODE));
                            SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.END_DEPARTMENT_CODE, department.DEPARTMENT_CODE));
                            SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.END_DEPARTMENT_NAME, department.DEPARTMENT_NAME));
                        }
                    }
                    ...
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.GENDER_INDEX, genderIndex));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.GENDER_NAME, genderName));

                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TRAN_PATI_MEDI_ORG_CODE, rdo.Treatment.TRANSFER_IN_MEDI_ORG_CODE));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TRAN_PATI_MEDI_ORG_NAME, rdo.Treatment.TRANSFER_IN_MEDI_ORG_NAME));
                    ...
                    if (rdo.Treatment.TREATMENT_RESULT_ID.HasValue || rdo.Treatment.TREATMENT_RESULT_ID.HasValue)
                    {
                        ...
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_RESULT_INDEX, treatmentResultIndex));
                    }
                        ...
                        if (dateBefore != null && dateAfter != null)
                        {
                            ...
                            if (difference.Days > 1 || (difference.Days == 1 && (difference.Hours >= 1 || difference.Minutes >= 1 || difference.Seconds >= 1)))
                            {
                                if (rdo.Treatment.TDL_TREATMENT_TYPE_ID != IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__KHAM)
                                {
                                    ...
                                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_DAY_COUNT_6556, diffDay));
                                }
                            }
                            else if (rdo.Treatment.TDL_TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNOITRU)
                            {
                                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_DAY_COUNT_6556, 1));
                            }
                        }
                    }
                }

                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_DAY, rdo.SingleKeyValue.today));

                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.CURRENT_DATE_SEPARATE_FULL_STR, rdo.SingleKeyValue.currentDateSeparateFullTime));
                ...
                if (sereServADOs != null && sereServADOs.Count > 0)
                {
                    ...
                    {
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_NUMBER_FILM, totalNumberFilm));
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_NUMBER_FILM_STR, String.Format("Bệnh nhân đã nhận đủ số phim . Số phim {0}", totalNumberFilm)));
                    }
                ...
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.EXECUTE_ROOM_EXAM, executeRoomExam));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.FIRST_EXAM_ROOM_NAME, executeRoomExamFirst));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.LAST_EXAM_ROOM_NAME, executeRoomExamLast));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.OTHER_PAY_SOURCE_NAME, otherPaySource));

                if (!String.IsNullOrEmpty(rdo.SingleKeyValue.departmentName))
                {
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.DEPARTMENT_NAME, rdo.SingleKeyValue.departmentName));
                }
                ...
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_SURCHARGE, Inventec.Common.Number.Convert.NumberToStringRoundAuto(totalSurcharge, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_SURCHARGE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(totalSurcharge).ToString())));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.SURCHARGE_COUNT, surchargeCount));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.SURCHARGE_SECTION_NO, surchargeSectionNo));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.SURCHARGE_SECTION_LABEL, surchargeCount > 0 ? (surchargeSectionNo + ". Phu phi") : ""));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE, Inventec.Common.Number.Convert.NumberToStringRoundAuto(thanhtien_tong, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_BHYT, Inventec.Common.Number.Convert.NumberToStringRoundAuto(thanhtienBH_tong, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_HEIN, Inventec.Common.Number.Convert.NumberToStringRoundAuto(bhytthanhtoan_tong, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_PATIENT, Inventec.Common.Number.Convert.NumberToStringRoundAuto(bnthanhtoan_tong, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_PATIENT_SELF, Inventec.Common.Number.Convert.NumberToStringRoundAuto(tongtienbenhnhantutra, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_OTHER, Inventec.Common.Number.Convert.NumberToStringRoundAuto(nguonkhac_tong, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(thanhtien_tong).ToString())));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_HEIN_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(bhytthanhtoan_tong).ToString())));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_PATIENT_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(bnthanhtoan_tong).ToString())));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_OTHER_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(nguonkhac_tong).ToString())));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_PATIENT_NO_PAY_RATE, Inventec.Common.Number.Convert.NumberToStringRoundAuto(tongTienBenhNhan, 0)));
                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_PRICE_PATIENT_NO_PAY_RATE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(tongTienBenhNhan).ToString())));

                if (rdo.TreatmentFees != null)
                {
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_DEPOSIT_AMOUNT, Inventec.Common.Number.Convert.NumberToStringRoundAuto(rdo.TreatmentFees[0].TOTAL_DEPOSIT_AMOUNT ?? 0, 0)));
                    ...
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_FEE_TOTAL_PRICE, Inventec.Common.Number.Convert.NumberToStringRoundAuto(totalPrice, 0)));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_FEE_TOTAL_PATIENT_PRICE, Inventec.Common.Number.Convert.NumberToStringRoundAuto(totalPatientPrice, 0)));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_FEE_TOTAL_OBTAINED_PRICE, Inventec.Common.Number.Convert.NumberToStringRoundAuto(total_obtained_price, 0)));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TREATMENT_FEE_TRANSFER, Inventec.Common.Number.Convert.NumberToStringRoundAuto(transfer, 0)));
                    AddObjectKeyIntoListkey<V_HIS_TREATMENT_FEE>(rdo.TreatmentFees[0], false);
                }
                else
                {
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.TOTAL_DEPOSIT_AMOUNT, "0"));
                }

                SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.CREATOR_USERNAME, Inventec.UC.Login.Base.ClientTokenManagerStore.ClientTokenManager.GetUserName()));
                ...
                if (rdo.CurrentPatyAlter != null)
                {
                    ...
                    if (rdo.CurrentPatyAlter.HEIN_CARD_FROM_TIME.HasValue)
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.STR_HEIN_CARD_FROM_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.CurrentPatyAlter.HEIN_CARD_FROM_TIME ?? 0)));
                    if (rdo.CurrentPatyAlter.HEIN_CARD_TO_TIME.HasValue)
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.STR_HEIN_CARD_TO_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.CurrentPatyAlter.HEIN_CARD_TO_TIME ?? 0)));
                    if (rdo.CurrentPatyAlter.JOIN_5_YEAR_TIME.HasValue)
                        SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.JOIN_5_YEAR_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.CurrentPatyAlter.JOIN_5_YEAR_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.RATIO_STR, DataRawProcess.GetDefaultHeinRatioForView(rdo.CurrentPatyAlter.HEIN_CARD_NUMBER, rdo.CurrentPatyAlter.HEIN_TREATMENT_TYPE_CODE, rdo.Branch.HEIN_LEVEL_CODE, rdo.CurrentPatyAlter.RIGHT_ROUTE_CODE, rdo.CurrentPatyAlter.FACILITY_CLASS, rdo.CurrentPatyAlter.FORMER_LEVEL_CODE, rdo.CurrentPatyAlter.CLASSIFY_POINT ?? 0)));
                    SetSingleKey(new KeyValue(Mps000359ExtendSingleKey.LIVE_AREA_CODE, rdo.CurrentPatyAlter.LIVE_AREA_CODE));
                }
            }
            catch (Exception ex)
            {
