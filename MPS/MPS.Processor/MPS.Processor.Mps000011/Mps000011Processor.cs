...
{
    public class Mps000011Processor : AbstractProcessor
    {
        Mps000011PDO rdo;
        public Mps000011Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000011PDO)rdoBase;
        }
        ...
        private void SetSingleKey()
        {
            try
            {
                ...
                if (rdo.currentTreatment != null)
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(this.rdo.currentTreatment.TRANSFER_IN_MEDI_ORG_CODE))
                    {
                        HIS_MEDI_ORG transIn = rdo._MediOrgs != null ? rdo._MediOrgs.FirstOrDefault(o => o.MEDI_ORG_CODE == this.rdo.currentTreatment.TRANSFER_IN_MEDI_ORG_CODE) : null;
                        if (transIn != null)
                        {
                            SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.TRANSFER_IN_LEVEL_CODE, transIn.LEVEL_CODE));
                        }
                    }
                    if (!String.IsNullOrWhiteSpace(this.rdo.currentTreatment.MEDI_ORG_CODE))
                    {
                        HIS_MEDI_ORG transOut = rdo._MediOrgs != null ? rdo._MediOrgs.FirstOrDefault(o => o.MEDI_ORG_CODE == this.rdo.currentTreatment.MEDI_ORG_CODE) : null;
                        if (transOut != null)
                        {
                    ...
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.DOCTOR_USERNAME, rdo.currentTreatment.DOCTOR_USERNAME));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.TRAN_PATI_HOSPITAL_LOGINNAME, rdo.currentTreatment.TRAN_PATI_HOSPITAL_LOGINNAME));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.TRAN_PATI_HOSPITAL_USERNAME, rdo.currentTreatment.TRAN_PATI_HOSPITAL_USERNAME));
                }
                ...
                if (rdo._ServiceReq != null && rdo._ServiceReq.EXAM_END_TYPE == 3)
                {
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.EXECUTE_USERNAME_STR, rdo._ServiceReq.EXECUTE_USERNAME));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.EXECUTE_LOGINNAME_STR, rdo._ServiceReq.EXECUTE_LOGINNAME));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.ICD_CODE_STR, rdo._ServiceReq.ICD_CODE));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.ICD_NAME_STR, rdo._ServiceReq.ICD_NAME));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.ICD_SUB_CODE_STR, rdo._ServiceReq.ICD_SUB_CODE));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.ICD_TEXT_STR, rdo._ServiceReq.ICD_TEXT));
                }
                else if (rdo.currentTreatment != null)
                {
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.ICD_CODE_STR, rdo.currentTreatment.ICD_CODE));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.ICD_NAME_STR, rdo.currentTreatment.ICD_NAME));
                    SetSingleKey(new KeyValue(Mps000011ExtendSingleKey.ICD_SUB_CODE_STR, rdo.currentTreatment.ICD_SUB_CODE));
