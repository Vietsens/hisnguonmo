...
namespace MPS.Processor.Mps000158
{
    public class DataRawProcess
    {
        ...
        public static List<PatyAlterBhytADO> PatyAlterBHYTRawToADOs(List<HIS_PATIENT_TYPE_ALTER> patyAlters, HIS_BRANCH branch, List<HIS_TREATMENT_TYPE> treatmentTypes, V_HIS_PATIENT_TYPE_ALTER currentPatyAlter, V_HIS_TREATMENT treatment)
        {
                ...
                if (patyAlters != null && patyAlters.Count > 0)
                {
                    foreach (var item in patyAlters)
                    {
                        ...
                        if (!String.IsNullOrEmpty(item.HEIN_CARD_NUMBER))
                        {
                            ...
                            patyAlterBhytADO.HEIN_CARD_NUMBER_6 = item.HEIN_CARD_NUMBER.Substring(10);
                        }
