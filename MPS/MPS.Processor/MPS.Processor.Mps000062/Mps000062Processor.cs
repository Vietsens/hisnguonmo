...
namespace MPS.Processor.Mps000062
{
    public class Mps000062Processor : AbstractProcessor
    {
            ...
            try
            {
                ...
                foreach (var item in mps000062ADOExt.Mps000062ADOs)
                {
                    ...
                    if (listServiceReqMetyADOs != null && listServiceReqMetyADOs.Count > 0)
                    {
                        foreach (var item1 in listServiceReqMetyADOs)
                        {
                            ...
                            medicine_Merges.Insert(medicine_Merges.Count, new ExpMestMetyReqADO()
                            {
                                ...
                                DAY_COUNT = item1.DAY_COUNT,
                                INTRUCTION_TIME_STR = item1.INTRUCTION_TIME_STR,
                                INTRUCTION_DATE = item1.INTRUCTION_DATE,
                                NUMBER_H_N = rdo._WorkPlaceSDO.UsedDayCountingOutStockOption == 1 ? item1.NUMBER_BY_GROUP : null,
                                NUMBER_BY_TYPE = rdo._WorkPlaceSDO.UsedDayCountingOutStockOption == 1 ? item1.NUMBER_BY_TYPE_IN_OUT : null,
                                TDL_MEDICINE_TYPE_ID = item1.MEDICINE_TYPE_ID,
                                HTU_TEXT = item1.HTU_TEXT
                    ...
                    if (medicine_Merges != null && medicine_Merges.Count > 0)
                    {
                        ...
                        List<ExpMestMetyReqADO> MMDataSort = new List<ExpMestMetyReqADO>();
                        foreach (var medi in medicine_Merges)
                        {
                            ...
                            dataRepx += String.Format("<table><tr><td style =\"vertical-align: top\" width=\"650\" text-align=\"left\" align=\"left\">{0} {1}</td></span><td style =\"vertical-align: top\" text-align=\"right\" align=\"right\" width=\"150\">{2}</td></tr></table>", s1, medi.CONCENTRA, s2);
                        ...
                        var MMDataSortDes = new List<ExpMestMetyReqADO>();
                        DataMedicine_Merge(MMDataSort, MMDataSortDes);
                        var groupIntructionDate = MMDataSortDes.GroupBy(o => Inventec.Common.DateTime.Convert.TimeNumberToDateString(o.INTRUCTION_DATE)).ToList();
                        foreach (var gut in groupIntructionDate)
                        {
                            if (gut.Key != null)
                            {
                                item.MEDICINES_MERGE_DATE___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("Ngày sử dụng: " + gut.Key, FontStyle.Bold);
                                item.MEDICINES_MERGE_DATE_HTU___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("Ngày sử dụng: " + gut.Key, FontStyle.Bold);
                            }
                            else
                            {
                                item.MEDICINES_MERGE_DATE___DATA += "";
                            ...
                            item.MEDICINES_MERGE_DATE___DATA += string.Join("", gut.ToList().Select(o => o.DATA_REPX));
                            item.MEDICINES_MERGE_DATE_HTU___DATA += string.Join("", gut.ToList().Select(o => o.DATA_REPX + (string.IsNullOrEmpty(o.HTU_TEXT) ? "" : (o.HTU_TEXT + Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br)))));
                        }
                    }
                    ...
                    if (medicines != null && medicines.Count > 0)
                    {
                        ...
                        foreach (var medi in medicines)
                        {
                            ...
                            string dataHtuFullRepx = dataRepx;
                            if ((medi.REMEDY_COUNT ?? 0) <= 0 && !string.IsNullOrEmpty(medi.HTU_TEXT))
                            {
                                dataHtuFullRepx += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle(" " + medi.HTU_TEXT, FontStyle.Italic);
                                dataHtuFullRepx += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                            }

                            lstExpMestMety.Add(new ExpMestMetyReqADO() { INTRUCTION_DATE = medi.INTRUCTION_DATE, INTRUCTION_TIME = medi.INTRUCTION_TIME, INTRUCTION_TIME_STR = medi.INTRUCTION_TIME_STR, MEDICINE_GROUP_NUM_ORDER = medi.MEDICINE_GROUP_NUM_ORDER, NUM_ORDER_BY_USE_FORM = medi.MEDICINE_GROUP_NUM_ORDER ?? -1, TDL_SERVICE_REQ_ID = medi.TDL_SERVICE_REQ_ID, NUM_ORDER = medi.NUM_ORDER, NUMBER_H_N = medi.NUMBER_H_N, TDL_MEDICINE_TYPE_ID = medi.TDL_MEDICINE_TYPE_ID, REMEDY_COUNT = medi.REMEDY_COUNT, USE_TIME = medi.USE_TIME, USE_TIME_TO = medi.USE_TIME_TO, ADVISE = medi.ADVISE, HTU_TEXT = medi.HTU_TEXT, ASSIGN_TIME_TO = medi.ASSIGN_TIME_TO, DAY_COUNT = medi.DAY_COUNT, DATA_REPX = dataRepx, DATA_DAY_REPX = dataRepx, DATA_DAY_HTU_REPX = dataHtuRepx, DATA_DAY_HTU_FULL_REPX = dataHtuFullRepx, MEDICINE_TYPE_CODE = medi.MEDICINE_TYPE_CODE });
                        }
                    }
                    ...
                    List<ExpMestMetyReqADO> MMDuTruDataSort = new List<ExpMestMetyReqADO>();
                        ...
                        if (ReqDT.TRACKING_ID == item.ID && ReqDT.IS_NOT_SHOW_MATERIAL_TRACKING == 1)
                        {
                            ...
                            if (medicineDuTru_Merges != null && medicineDuTru_Merges.Count > 0)
                            {
                                ...
                                foreach (var medi in medicineDuTru_Merges)
                                {
                                    ...
                                    string dataRepx = String.Format("<table><tr><td style =\"vertical-align: top\" width=\"650\" text-align=\"left\" align=\"left\">{0}</td></span><td style =\"vertical-align: top\" text-align=\"right\" align=\"right\" width=\"150\">{1}</td></tr></table>", s1, s2);
                            ...
                            if (medicineDuTrus != null && medicineDuTrus.Count > 0)
                            {
                                ...
                                foreach (var medi in medicineDuTrus)
                                {
                                    ...
                                    lstExpMestMety.Add(new ExpMestMetyReqADO() { INTRUCTION_DATE = medi.INTRUCTION_DATE, INTRUCTION_TIME = medi.INTRUCTION_TIME, INTRUCTION_TIME_STR = medi.INTRUCTION_TIME_STR, MEDICINE_GROUP_NUM_ORDER = medi.MEDICINE_GROUP_NUM_ORDER, NUM_ORDER_BY_USE_FORM = medi.MEDICINE_GROUP_NUM_ORDER ?? -1, TDL_SERVICE_REQ_ID = medi.TDL_SERVICE_REQ_ID, NUM_ORDER = medi.NUM_ORDER, NUMBER_H_N = medi.NUMBER_H_N, TDL_MEDICINE_TYPE_ID = medi.TDL_MEDICINE_TYPE_ID, REMEDY_COUNT = medi.REMEDY_COUNT, ASSIGN_TIME_TO = medi.ASSIGN_TIME_TO, USE_TIME = ReqDT.USE_TIME, USE_TIME_TO = medi.USE_TIME_TO, ADVISE = medi.ADVISE, HTU_TEXT = medi.HTU_TEXT, DAY_COUNT = medi.DAY_COUNT, DATA_REPX = dataRepx, DATA_DAY_REPX = dataRepx, DATA_DAY_HTU_REPX = dataHtuRepx, DATA_DAY_HTU_FULL_REPX = dataHtuFullRepx, MEDICINE_TYPE_CODE = medi.MEDICINE_TYPE_CODE });
                                }
                            }
                            #endregion
                            ...
                            if (OutStockDuTrus != null && OutStockDuTrus.Count > 0)
                            {
                                ...
                                foreach (var medi in OutStockDuTrus)
                                {
                                    ...
                                    dataRepx += medi.TUTORIAL;
                                    dataRepx += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                                    item.MEDICINES_OutStock_DuTru__DATA += dataRepx;
                    ...
                    #region MEDICINES_MERGE_DATE_DUTRU___DATA 
                    MMDuTruDataSort = DataMedicineGroupByUseTime_Merge(MMDuTruDataSort);
                    var groupUseTime = MMDuTruDataSort.GroupBy(o => Inventec.Common.DateTime.Convert.TimeNumberToDateString(o.USE_TIME ?? 0)).ToList();
                    item.MEDICINES_MERGE_DATE_DUTRU___DATA = "";
                    item.MEDICINES_MERGE_DATE_DUTRU_HTU___DATA = "";
                    foreach (var gut in groupUseTime)
                    {
                        if (gut.Key != null)
                        {
                            item.MEDICINES_MERGE_DATE_DUTRU___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("Đơn thuốc dự trù ngày " + gut.Key, FontStyle.Bold);
                            item.MEDICINES_MERGE_DATE_DUTRU_HTU___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("Đơn thuốc dự trù ngày " + gut.Key, FontStyle.Bold);
                        }
                        else
                        {
                            item.MEDICINES_MERGE_DATE_DUTRU___DATA += "";
                        ...
                        item.MEDICINES_MERGE_DATE_DUTRU___DATA += string.Join("", DataMedicineGroupByUseTime_Merge(gut.ToList()).Select(o => o.DATA_REPX));
                    ...
                    #region Thuốc thực hiện dự trù
                    List<ExpMestMetyReqADO> MMThdt = new List<ExpMestMetyReqADO>();
                        ...
                        if (ReqTHDT.USED_FOR_TRACKING_ID == item.ID && ReqTHDT.IS_NOT_SHOW_MATERIAL_TRACKING == 1)
                        {
                            ...
                            if (medicineTHDTs != null && medicineTHDTs.Count > 0)
                            {
                                ...
                                foreach (var medi in medicineTHDTs)
                                {
                                    ...
                                    lstExpMestMety.Add(new ExpMestMetyReqADO() { INTRUCTION_DATE = medi.INTRUCTION_DATE, INTRUCTION_TIME = medi.INTRUCTION_TIME, INTRUCTION_TIME_STR = medi.INTRUCTION_TIME_STR, MEDICINE_GROUP_NUM_ORDER = medi.MEDICINE_GROUP_NUM_ORDER, NUM_ORDER_BY_USE_FORM = medi.MEDICINE_GROUP_NUM_ORDER ?? -1, TDL_SERVICE_REQ_ID = medi.TDL_SERVICE_REQ_ID, NUM_ORDER = medi.NUM_ORDER, NUMBER_H_N = medi.NUMBER_H_N, TDL_MEDICINE_TYPE_ID = medi.TDL_MEDICINE_TYPE_ID, REMEDY_COUNT = medi.REMEDY_COUNT, ASSIGN_TIME_TO = medi.ASSIGN_TIME_TO, USE_TIME = ReqTHDT.USE_TIME, USE_TIME_TO = medi.USE_TIME_TO, ADVISE = medi.ADVISE, HTU_TEXT = medi.HTU_TEXT, DAY_COUNT = medi.DAY_COUNT, DATA_REPX = dataRepx, DATA_DAY_REPX = dataRepx, DATA_DAY_HTU_REPX = dataHtuRepx, DATA_DAY_HTU_FULL_REPX = dataHtuFullRepx, MEDICINE_TYPE_CODE = medi.MEDICINE_TYPE_CODE });

                                    MMThdt.Add(new ExpMestMetyReqADO() { USE_TIME = ReqTHDT.USE_TIME, MEDICINE_GROUP_NUM_ORDER = medi.MEDICINE_GROUP_NUM_ORDER, NUM_ORDER_BY_USE_FORM = medi.NUM_ORDER_BY_USE_FORM, TDL_SERVICE_REQ_ID = medi.TDL_SERVICE_REQ_ID, NUM_ORDER = medi.NUM_ORDER, NUMBER_H_N = medi.NUMBER_H_N, USING_COUNT_NUMBER = medi.USING_COUNT_NUMBER, TDL_MEDICINE_TYPE_ID = medi.TDL_MEDICINE_TYPE_ID, DATA_REPX = dataRepx, HTU_TEXT = medi.HTU_TEXT });
                                }
                            ...
                            if (medicineInfusion_THDTs != null && medicineInfusion_THDTs.Count > 0)
                            {
                                ...
                                foreach (var InfusionsGroup in medicineInfusion_THDTGroup)
                                {
                                    ...
                                    foreach (var MediIns in InfusionsList)
                                    {
                                        ...
                                        s2 = strAmount + " " + MediIns.SERVICE_UNIT_NAME + "/ngày";
                                        string dataRepx = String.Format("<table><tr><td style =\"vertical-align: top\" width=\"650\" text-align=\"left\" align=\"left\">{0}</td></span><td style =\"vertical-align: top\" text-align=\"right\" align=\"right\" width=\"150\">{1}</td></tr></table>", s1, s2);
                                        dataRepx += MediIns.TUTORIAL;
                                        item.MEDICINES_INFUSION_THDT___DATA += dataRepx;
                                        dataRepx += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                                        ...
                                        item.MEDICINE_TYPE_ID = MediIns.TDL_MEDICINE_TYPE_ID;
                                        MMThdt.Add(new ExpMestMetyReqADO() { USE_TIME = ReqTHDT.USE_TIME, MEDICINE_GROUP_NUM_ORDER = MediIns.MEDICINE_GROUP_NUM_ORDER, NUM_ORDER_BY_USE_FORM = MediIns.NUM_ORDER_BY_USE_FORM, TDL_SERVICE_REQ_ID = MediIns.TDL_SERVICE_REQ_ID, NUM_ORDER = MediIns.NUM_ORDER, NUMBER_H_N = MediIns.NUMBER_H_N, USING_COUNT_NUMBER = MediIns.USING_COUNT_NUMBER, TDL_MEDICINE_TYPE_ID = MediIns.TDL_MEDICINE_TYPE_ID, DATA_REPX = dataRepx, HTU_TEXT = MediIns.HTU_TEXT });
                                        checkdem++;
                                    }
                            ...
                            if (OutStockTHDTs != null && OutStockTHDTs.Count > 0)
                            {
                                ...
                                foreach (var medi in OutStockTHDTs)
                                {
                                    ...
                                    dataRepx += medi.TUTORIAL;
                                    dataRepx += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                                    item.MEDICINES_OutStock_THDT__DATA += dataRepx;
                                    ...
                                    MMThdt.Add(new ExpMestMetyReqADO() { USE_TIME = ReqTHDT.USE_TIME, MEDICINE_GROUP_NUM_ORDER = medi.NUM_ORDER_MEDICINE_GROUP, NUM_ORDER_BY_USE_FORM = medi.NUM_ORDER_MEDICINE_USE_FORM ?? 0, TDL_SERVICE_REQ_ID = medi.SERVICE_REQ_ID, NUM_ORDER = medi.NUM_ORDER, NUMBER_H_N = medi.NUMBER_BY_GROUP, USING_COUNT_NUMBER = null, TDL_MEDICINE_TYPE_ID = medi.MEDICINE_TYPE_ID, DATA_REPX = dataRepx, HTU_TEXT = medi.HTU_TEXT });
                                    dem++;
                                }
                    ...
                    #endregion MEDICINES_MERGE_DATE_THDT___DATA
                    MMThdt = DataMedicineGroupByUseTime_Merge(MMThdt);
                    var groupTHDTUseTime = MMThdt.GroupBy(o => Inventec.Common.DateTime.Convert.TimeNumberToDateString(o.USE_TIME ?? 0)).ToList();
                    item.MEDICINES_MERGE_DATE_THDT___DATA = "";
                    item.MEDICINES_MERGE_DATE_THDT_HTU___DATA = "";
                    foreach (var gut in groupTHDTUseTime)
                    {
                        if (gut.Key != null)
                        {
                            item.MEDICINES_MERGE_DATE_THDT___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("Thực hiện đơn thuốc dự trù ngày " + gut.Key, FontStyle.Bold);
                            item.MEDICINES_MERGE_DATE_THDT_HTU___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("Thực hiện đơn thuốc dự trù ngày " + gut.Key, FontStyle.Bold);
                        }
                        else
                        {
                            item.MEDICINES_MERGE_DATE_THDT___DATA += "";
                        ...
                        item.MEDICINES_MERGE_DATE_THDT___DATA += string.Join("", DataMedicineGroupByUseTime_Merge(gut.ToList()).Select(o => o.DATA_REPX));
                        item.MEDICINES_MERGE_DATE_THDT_HTU___DATA += string.Join("", DataMedicineGroupByUseTime_Merge(gut.ToList()).Select(o => o.DATA_REPX + (string.IsNullOrEmpty(o.HTU_TEXT) ? "" : (o.HTU_TEXT + Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br)))));
                    }
                    #region 
                    ...
                    if (this._ImpMestMedicineADOs != null && this._ImpMestMedicineADOs.Count > 0)
                    {
                        ...
                        foreach (var impMestMedicine in impMestMedicineAPIs_G)
                        {
                            ...
                            {
                                string s1 = "", s2 = "";
                                if (IMedi.USE_TIME_OLD != null)
                                {
                                    if (UseTime == null || UseTime != IMedi.USE_TIME_OLD)
                                    {
                                ...
                                {
                                    if (IntructionTime == null || IntructionTime != IMedi.INTRUCTION_TIME_OLD)
                                    {
                    ...
                    if (this._ImpMestMaterialADOs != null && this._ImpMestMaterialADOs.Count > 0)
                    {
                        ...
                        foreach (var impMestMaterial in impMestMaterialAPIs_G)
                        {
                            ...
                            {
                                string s1 = "", s2 = "";
                                if (IMate.USE_TIME_OLD != null)
                                {
                                    if (UseTime == null || UseTime != IMate.USE_TIME_OLD)
                                    {
                                ...
                                else if (IMate.INTRUCTION_TIME_OLD != null)
                                {
                                    if (IntructionTime == null || IntructionTime != IMate.INTRUCTION_TIME_OLD)
                                    {
                    ...
                    if (this._ImpMestBloodADOs != null && this._ImpMestBloodADOs.Count > 0)
                    {
                        ...
                        foreach (var impMestBlood in ImpMestBloodAPIs_G)
                        {
                            long? IntructionTime = 0;
                            long? UseTime = null;
                            var impMestBlood_S = impMestBlood.OrderBy(p => p.USE_TIME_OLD ?? 99999999999999).ThenBy(o => o.INTRUCTION_TIME_OLD ?? 99999999999999).ToList();
                            foreach (var IBlood in impMestBlood_S)
                            {
                                string s1 = "", s2 = "";
                                if (IBlood.USE_TIME_OLD != null)
                                {
                                    if (UseTime == null || UseTime != IBlood.USE_TIME_OLD)
                                    {
                                        UseTime = IBlood.USE_TIME_OLD;
                                        item.MOBA_IMP_MEST_BLOOD__DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("Hoàn trả máu dự trù ngày " + Inventec.Common.DateTime.Convert.TimeNumberToDateString(UseTime ?? 0), FontStyle.Bold);
                                    }
                                }
                                else if (IBlood.INTRUCTION_TIME_OLD != null)
                                {
                                    if (IntructionTime == null || IntructionTime != IBlood.INTRUCTION_TIME_OLD)
                                    {
                    ...
                    if (materials != null && materials.Count > 0)
                    {
                        ...
                        {
                            var left = Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle(item1.MATERIAL_TYPE_NAME, FontStyle.Bold);

                            // Số lượng + đơn vị (zero-pad như thuốc)
                            var amt = (item1.AMOUNT >= 1 && item1.AMOUNT < 10)
                                ? "0" + Inventec.Common.Number.Convert.NumberToStringRoundMax4(item1.AMOUNT)
                                : Inventec.Common.Number.Convert.NumberToStringRoundMax4(item1.AMOUNT) + "";
                            var right = amt + " " + item1.SERVICE_UNIT_NAME;

                            // Layout 2 cột như thuốc
                            var row = string.Format(
                                "<table><tr>" +
                                "<td style=\"vertical-align: top\" width=\"650\" text-align=\"left\" align=\"left\">{0}</td>" +
                                "<td style=\"vertical-align: top\" text-align=\"right\" align=\"right\" width=\"150\">{1}</td>" +
                                "</tr></table>", left, right);
                            ...
                            item.MATERIAL_HTU_UNIT___DATA += row;
                        ...
                        if (ReqDuTru.TRACKING_ID == item.ID && ReqDuTru.IS_NOT_SHOW_MEDICINE_TRACKING == 1)
                        {
                            ...
                            if (materialDuTrus != null && materialDuTrus.Count > 0)
                            {
                                ...
                                foreach (var item1 in materialDuTrus)
                                {
                                    ...
                                    item.MATERIAL_DuTru_HTU_UNIT___DATA += row;
                                    if (!string.IsNullOrEmpty(item1.HTU_TEXT))
                                    {
                                        item.MEDICINES_INFUSION_THDT_HTU___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                        ...
                        if (ReqTHDT.USED_FOR_TRACKING_ID == item.ID && ReqTHDT.IS_NOT_SHOW_MEDICINE_TRACKING == 1)
                        {
                            ...
                            if (materialTHDTs != null && materialTHDTs.Count > 0)
                            {
                                ...
                                foreach (var item1 in materialTHDTs)
                                {
                                    ...
                                    item.MATERIAL_THDT_HTU_UNIT___DATA += row;
                                    if (!string.IsNullOrEmpty(item1.HTU_TEXT))
                                    {
                                        item.MATERIAL_THDT_HTU___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                                        item.MATERIAL_THDT_HTU_UNIT___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                    ...
                    else if (rdo._WorkPlaceSDO.IsOrderByType == 4)
                    {
                        ...
                        foreach (var ml in groupByMedicineLine)
                        {
                            ...
                            Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => ml.Key.MEDICINE_LINE_NAME), ml.Key.MEDICINE_LINE_NAME));
                            ...
                            item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("- " + ml.Key.MEDICINE_LINE_NAME, FontStyle.Bold);
                            item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                            ...
                            var SortIntructionTime = ml.ToList()
                                .OrderBy(o => o.INTRUCTION_TIME)
                                .GroupBy(o => new { o.USE_TIME, o.INTRUCTION_TIME, o.MEDICINE_TYPE_CODE, o.TDL_SERVICE_REQ_ID })
                                .Select(g => g.First())
                                .ToList();

                            ProcessAddKeyMergeDay(SortIntructionTime.ToList(), item);
                    ...
                    if (rdo._WorkPlaceSDO.IsOrderByType < 4)
                    {
                        ...
                        item.MEDICINES_MERGE_DAY_HTU_FULL___DATA = string.Join(Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br), lstExpMestMety.Select(o => o.DATA_DAY_HTU_FULL_REPX));
                    }

                    item.SERVICE_MERGE_X01___DATA = "";
                    item.SERVICE_MERGE_X01___DATA1 = "";
                    if (lstServiceCls != null && lstServiceCls.Count > 0)
                    {
                        ...
                        foreach (var st in groupServiceType)
                        {
                            ...
                            item.SERVICE_MERGE_X01___DATA1 += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle("- " + (!string.IsNullOrEmpty(st.ToList()[0].SERVICE_TYPE_NAME) ? st.ToList()[0].SERVICE_TYPE_NAME : rdo._ServiceTypes.FirstOrDefault(o => o.ID == st.Key.TDL_SERVICE_TYPE_ID).SERVICE_TYPE_NAME) + ":", FontStyle.Bold);
                            item.SERVICE_MERGE_X01___DATA1 += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                            ...
                            foreach (var serClsByParent in groupByServiceParent)
                            {
                                ...
                                foreach (var gs in groupService)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(strInstructionNote))
                                    {
                                        ...
                                        item.SERVICE_MERGE_X01___DATA1 += sv.SERVICE_NAME.Replace(":", "").Trim() + ": " + Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle(strInstructionNote, FontStyle.Italic);
                                    }
                                    else
                                    {
                                        item.SERVICE_MERGE_X01___DATA += sv.SERVICE_NAME.Replace(";", "").Trim();
                                        item.SERVICE_MERGE_X01___DATA1 += sv.SERVICE_NAME.Replace(";", "").Trim();
                                    }
                                    ...
                                    var strAmountDT1 = ((amount >= 1 && amount < 10) ? "0" + Inventec.Common.Number.Convert.NumberToStringRoundMax4(amount) : Inventec.Common.Number.Convert.NumberToStringRoundMax4(amount) + "");
                                    item.SERVICE_MERGE_X01___DATA1 += " x" + strAmountDT1;
                                    var serviceUnitDT1 = HIS.Desktop.LocalStorage.BackendData.BackendDataWorker.Get<HIS_SERVICE_UNIT>().FirstOrDefault(o => o.ID == sv.TDL_SERVICE_UNIT_ID);
                                    if (serviceUnitDT1 != null)
                                        item.SERVICE_MERGE_X01___DATA1 += " " + serviceUnitDT1.SERVICE_UNIT_NAME;
                                    item.SERVICE_MERGE_X01___DATA1 += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                                    item.SERVICE_MERGE_X01___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
        ...
        private void ProcessAddKeyMergeDay(List<ExpMestMetyReqADO> SortIntructionTime, Mps000062ExtADO item)
        {
            try
            {
                ...
                foreach (var inStrTime in groupDates)
                {
                    ...
                    item.MEDICINES_MERGE_DAY_HTU___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                    item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += inStrTime.Key.Dates;
                    item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                    ...
                    item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += string.Join("", it.Select(o => o.DATA_DAY_HTU_FULL_REPX).Distinct());
                    ...
                    if (sthang > 0)
                    {
                        ...
                        item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += string.Format("x {0} thang", sthang < 10 ? "0" + sthang : sthang.ToString());
                        item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                    }
                    ...
                    if (rdo._DicServiceReqs != null && rdo._DicServiceReqs.Count > 0 && rdo._DicServiceReqs.ContainsKey(it.ToList().OrderBy(o => o.TDL_SERVICE_REQ_ID ?? 0).FirstOrDefault().TDL_SERVICE_REQ_ID ?? 0) && !string.IsNullOrEmpty(rdo._DicServiceReqs[it.ToList().OrderBy(o => o.TDL_SERVICE_REQ_ID ?? 0).FirstOrDefault().TDL_SERVICE_REQ_ID ?? 0].ADVISE))
                    {
                        ...
                        item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertFontStyle(" " + rdo._DicServiceReqs[it.ToList().OrderBy(o => o.TDL_SERVICE_REQ_ID ?? 0).FirstOrDefault().TDL_SERVICE_REQ_ID ?? 0].ADVISE, FontStyle.Italic);
                        item.MEDICINES_MERGE_DAY_HTU_FULL___DATA += Inventec.Desktop.Common.HtmlString.ProcessorString.InsertSpacialTag("", Inventec.Desktop.Common.HtmlString.SpacialTag.Tag.Br);
                    }
                }
        ...
        public List<ExpMestMetyReqADO> DataMedicineGroupByUseTime_Merge(List<ExpMestMetyReqADO> Data)
        {
            List<ExpMestMetyReqADO> Result = new List<ExpMestMetyReqADO>();
            try
            {
                if (Data != null && Data.Count > 0)
                {
                    var dataGroupsIntruction_Merge = Data.OrderBy(o => o.USE_TIME).GroupBy(o => Inventec.Common.DateTime.Convert.TimeNumberToDateString(o.USE_TIME ?? 0)).Select(o => o.ToList()).ToList();
