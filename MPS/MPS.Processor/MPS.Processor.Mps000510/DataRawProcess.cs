...
namespace MPS.Processor.Mps000510
{
    public class DataRawProcess
    {
        ...
        public static PatyAlterBhytADO PatyAlterBHYTRawToADO(V_HIS_PATIENT_TYPE_ALTER patyAlter, List<HIS_PATIENT_TYPE_ALTER> lstPatientTypeAlter, HIS_BRANCH branch, List<HIS_TREATMENT_TYPE> treatmentTypes, V_HIS_TREATMENT treatment)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(patyAlter.HEIN_CARD_NUMBER) && (patyAlter.HEIN_CARD_NUMBER.Length == 15 || patyAlter.HEIN_CARD_NUMBER.Length == 17))
                {
        ...
        public static string SetHeinCardNumberDisplayByNumber(string heinCardNumber)
        {
            ...
            try
            {
                if (!String.IsNullOrWhiteSpace(heinCardNumber) && (heinCardNumber.Length == 15 || heinCardNumber.Length == 17))
                {
                    ...
                    result = heinCardNumber.Length == 17
                        ? new StringBuilder()
                            ...
                            .Append(heinCardNumber.Substring(5)).ToString()
                        : new StringBuilder()
                            .Append(heinCardNumber.Substring(0, 2)).Append(separateSymbol)
                            .Append(heinCardNumber.Substring(2, 1)).Append(separateSymbol)
                            .Append(heinCardNumber.Substring(3, 2)).Append(separateSymbol)
