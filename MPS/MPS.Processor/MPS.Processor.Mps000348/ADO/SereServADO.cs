...
namespace MPS.Processor.Mps000348.ADO
{
    class SereServADO : HIS_SERE_SERV
    {
        ...
        public SereServADO(HIS_SERE_SERV data, List<HIS_SERE_SERV> SereServs, List<HIS_SERE_SERV_EXT> sereServExts, 
            List<HIS_HEIN_SERVICE_TYPE> heinServiceTypes, List<V_HIS_SERVICE> services, List<V_HIS_ROOM> rooms, 
            List<HIS_MEDICINE_TYPE> medicineTypes, List<HIS_MEDICINE_LINE> medicineLines, List<HIS_MATERIAL_TYPE> materialTypes, 
            PatientTypeCFG patientTypeCFG, HisConfigValue hisConfigValue, List<HIS_SERVICE_UNIT> hisServiceUnit, V_HIS_TREATMENT treatment
    ...
    || service.HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__TH_TL)
                            ...
                            {
                                var listStent = SereServs.Where(o => materialTypes.Where(w => w.IS_STENT == 1).Select(s => s.SERVICE_ID).Contains(o.SERVICE_ID) && o.PARENT_ID == data.PARENT_ID).ToList();
                                bool isStent2 = false;
                                if (listStent != null && listStent.Count > 1)
                                {
                                    listStent = listStent.OrderBy(o => o.STENT_ORDER ?? 9999).ToList();
                                    isStent2 = listStent[1].STENT_ORDER == data.STENT_ORDER;
                                }
                                ...
                                if (this.PARENT_ID.HasValue
                                    ...
                                    || service.HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__VT_TL) && !isStent2)
                                {
                                ...
                                else if (this.PARENT_ID.HasValue && service.HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__VT_TDM)
                                {
                                    ...
                                    else if (((parent.TDL_EXECUTE_ROOM_ID == this.TDL_REQUEST_ROOM_ID) || hisConfigValue.IsNotSameDepartment) && !isStent2)
                                    {
                                        HIS_HEIN_SERVICE_TYPE heinServiceTypeVT = heinServiceTypes.FirstOrDefault(o => o.ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__VT_TT);
                                        this.HEIN_SERVICE_TYPE_ID = this.PARENT_ID;
                                        this.HEIN_SERVICE_TYPE_NUM_ORDER = heinServiceTypeVT.NUM_ORDER;
                ...
                if (hisConfigValue.IsSurgPriceOption && (this.TDL_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__PT || this.TDL_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__TT))
                {
