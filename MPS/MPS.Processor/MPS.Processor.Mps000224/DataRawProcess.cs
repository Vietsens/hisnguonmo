            ...
            try
            {
                if (patyAlters != null && patyAlters.Count > 0)
                {
                    foreach (var item in patyAlters)
                    {
                        ...
                        patyAlterBhytADO.RATIO_STR = GetDefaultHeinRatioForView(patyAlterBhytADO.HEIN_CARD_NUMBER, treatmentType.HEIN_TREATMENT_TYPE_CODE, branch.HEIN_LEVEL_CODE, patyAlterBhytADO.RIGHT_ROUTE_CODE, item.FACILITY_CLASS, item.FORMER_LEVEL_CODE, item.CLASSIFY_POINT ?? 0);
