...
using MOS.SDO;
using MPS.Processor.Mps000318.PDO;
...
namespace MPS.Processor.Mps000318
{
    public class Mps000318Processor : AbstractProcessor
    {
        Mps000318PDO rdo;
        public Mps000318Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000318PDO)rdoBase;
        }
        ...
        void ProcessSingleKey()
        {
            try
            {
                if (rdo._Transaction != null)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000318ExtendSingleKey.AMOUNT_TEXT_UPPER_FIRST, Inventec.Common.String.Convert.CurrencyToVneseString(amountStr)));
                    decimal amountAfterExem = rdo._Transaction.AMOUNT - (rdo._Transaction.EXEMPTION ?? 0);
                    SetSingleKey(new KeyValue(Mps000318ExtendSingleKey.AMOUNT_AFTER_EXEMPTION, Inventec.Common.Number.Convert.NumberToNumberRoundMax4(amountAfterExem)));
                    ...
                    SetSingleKey(new KeyValue(Mps000318ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT, amountAfterExemText));
                    SetSingleKey(new KeyValue(Mps000318ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT_UPPER_FIRST, Inventec.Common.String.Convert.UppercaseFirst(amountAfterExemText)));
                    ...
                    SetSingleKey(new KeyValue(Mps000318ExtendSingleKey.AMOUNT_AWAY_ZERO_TEXT_UPPER_FIRST, Inventec.Common.String.Convert.CurrencyToVneseString(amountAwayZeroStr)));
                    ...
                    SetSingleKey(new KeyValue(Mps000318ExtendSingleKey.CT_AMOUNT, Inventec.Common.Number.Convert.NumberToNumberRoundMax4(canthu)));
                    SetSingleKey(new KeyValue(Mps000318ExtendSingleKey.CT_AMOUNT_TEXT_UPPER_FIRST, Inventec.Common.Number.Convert.NumberToStringRoundAuto(canthu, 4)));
