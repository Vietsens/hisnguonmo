...
namespace MPS.Processor.Mps000512
{
    public class DataRawProcess
    {
        ...
        public static string GetDefaultHeinRatioForView(string heinCardNumber, string treatmentTypeCode, string levelCode, string rightRouteCode, string facilityClassCode, string formerLevelCode = null, long point = 0, long? clinicalInTime = null)
        {
            string result = "";
            try
            {
                result = (((new MOS.LibraryHein.Bhyt.BhytHeinProcessor().GetDefaultHeinRatio(treatmentTypeCode, heinCardNumber, levelCode, rightRouteCode, facilityClassCode, formerLevelCode, point, clinicalInTime ?? 0) ?? 0) * 100)).ToString("0.00") + "%";
