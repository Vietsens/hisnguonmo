    ...
    public partial class Mps000312Processor : AbstractProcessor
    {
        ...
        private void ProcessSingleKey()
        {
            try
            {
                ...
                SetSingleKey(new KeyValue(Mps000312ExtendSingleKey.TOTAL_SURCHARGE, Inventec.Common.Number.Convert.NumberToStringRoundAuto(totalSurcharge, 0)));
                SetSingleKey(new KeyValue(Mps000312ExtendSingleKey.TOTAL_SURCHARGE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(totalSurcharge).ToString())));
                SetSingleKey(new KeyValue(Mps000312ExtendSingleKey.SURCHARGE_COUNT, surchargeCount));
                SetSingleKey(new KeyValue(Mps000312ExtendSingleKey.SURCHARGE_SECTION_NO, surchargeSectionNo));
                SetSingleKey(new KeyValue(Mps000312ExtendSingleKey.SURCHARGE_SECTION_LABEL, surchargeCount > 0 ? (surchargeSectionNo + ". Phu phi") : ""));
