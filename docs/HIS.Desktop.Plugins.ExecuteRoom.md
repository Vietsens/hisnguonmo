...
|------|-----------|-----------------|
| 21/09/2026 | sinhnt | Viec 3352 (PT-56263): cot "Ngay du tru" (`USE_TIME_DISPLAY`) hien them **gio phut** khi y lenh du tru co gio — helper `FormatUseTimeDisplay` theo quy uoc `USE_TIME % 1000000 != 0` -> `TimeNumberToTimeStringWithoutSecond` (dd/MM/yyyy HH:mm), con lai giu `TimeNumberToDateString` (dd/MM/yyyy) nen du lieu cu hien y nhu truoc. Them `gridViewServiceReq_CustomColumnSort` so theo **so** `USE_TIME` va `gridColumn20.SortMode = Custom` — truoc day cot unbound sort theo chuoi "dd/MM/yyyy" nen sai thu tu giua cac thang. |
