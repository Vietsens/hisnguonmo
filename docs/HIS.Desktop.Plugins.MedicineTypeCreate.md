...
|------|-----------|-----------------|
| 2026-05-18 | tuanln | Tách form `frmDepartmentPatientType` (Khoa-ĐTTT) ra plugin chung [[HIS.Desktop.Plugins.HisDepaPatientTypeList]]. `simpleButton24_Click` bây giờ gọi `PluginInstance.GetPluginInstance(moduleData, listArgs)` thay vì `new frmDepartmentPatientType()`. Truyền args: `Module`, `long? serviceId`, `List<HIS_DEPA_PATIENT_TYPE>`, `bool[2] {isCalledApi, isClickPick}`, `DelegateSelectData`. Callback `Frm_OnDepaPatientTypeSaved(object data)` đọc `DepaPatientTypeResultADO` qua reflection để tránh add project ref đến plugin con. File `MedicineTypeCreate/frmDepartmentPatientType.cs` + `.resx` vẫn giữ trong codebase nhưng không còn được caller dùng. |
| 2026-05-06 | dangth2 / Claude | PTTK 42762 (BV HAGL): Fix UX **"luồng cũ sửa liên tục"** sau Sao chép. Trước đây sau khi Sao chép + Save (Create), `ActionType` vẫn là `Add` → user pick thuốc khác qua `cboMedicineType` → load data nhưng Save sẽ **Create duplicate** thay vì Update. Sửa: trong `btnSave_Click` khi `resultData != null`, thêm `this.ActionType = ActionEdit;` sau khi đã set `currentMedicineTypeId = resultData.ID`. Hiệu ứng: sau Save thành công (cả Update lẫn Create), form chuyển về Edit mode focus vào record vừa lưu → user pick thuốc khác qua combo → Edit thuốc đó (Update khi Save). Sao chép tiếp khôi phục Add mode khi user click `btnCopy`. |
| 2026-04-28 | dangth2 / Claude | PTTK 42762 (BV HAGL): Fix bug **Add mode + chọn thuốc mẫu KHÔNG fill data**. `ChangeDataComboMedicineType()` trước đây chỉ load data khi `ActionType == Edit` → trong Add mode chọn template thì rơi vào `SetNullSpinControl` thay vì fill. Sửa: load data khi `currentMedicineTypeId > 0` (bất kể ActionType); chỉ giữ behavior Edit-only (`btnRefresh.Enabled = false`, `rdoUpdateAll/NotFee.ReadOnly = false`) trong nhánh `if (ActionType == Edit)` riêng. |
| 2026-04-28 | dangth2 / Claude | PTTK 42762 (BV HAGL): Enable `cboMedicineType` + `txtMedicineType` ở **CẢ Add lẫn Edit mode** (trước đây chỉ Edit) để user mở thẳng "Tạo mới" cũng chọn được thuốc mẫu. Sửa: (1) `SetDefaultData()` — bỏ if/else, set Enabled=true cho cả 2 mode. (2) `btnRefresh_Click` — set Enabled=true thay vì false. (3) `frmMedicineTypeCreate__Plus__Copy.cs / ResetTemplateSelectorControls()` — set Enabled=true sau Sao chép. |
