...
| 22/07/2026 | huannh | **Fix lọc lưới dưới (BN đang điều trị)**: đổi filter từ POCO tự định nghĩa `TreatmentInfoFilter` (field `DEPARTMENT_ID`, `IS_IN_TREATMENT` — **không tồn tại** trên server filter nên bị BE bỏ qua) sang `MOS.Filter.HisTreatmentView4Filter` **giống chức năng "Chỉ định chạy thận"** (`UCKidneyShift`): set `LAST_DEPARTMENT_ID` (thay `DEPARTMENT_ID`), `IS_PAUSE=false`, `KEY_WORD`, `IN_TIME_FROM/IN_TIME_TO`, `ORDER_FIELD=TDL_PATIENT_FIRST_NAME`/`ORDER_DIRECTION=ASC`. Endpoint không đổi (`HisTreatment/GetView4` qua `GetRO`), vẫn deserialize sang `TreatmentInfoADO` (khớp cột `V_HIS_TREATMENT_4`, giữ `IsSelected`/`DOB_DISPLAY`). `TreatmentInfoFilter.cs` không còn dùng. |
