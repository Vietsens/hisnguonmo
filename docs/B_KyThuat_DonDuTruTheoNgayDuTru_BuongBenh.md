...
**Đánh dấu (QT-08)** — `treeSereServ.NodeCellStyle`, chỉ so sánh `data.IS_ANTICIPATE` rồi set `ForeColor = Color.DarkOrange`. Không LINQ, không gọi cache trong event này (`ui_rules` mục 8). Tooltip lấy từ `ResourceMessage`, truyền 2 tham số ngày.
