...
| 06/09/2026 | khainq | **Lấy HẾT danh mục cổng (phân trang)**: `LayDanhMuc` lặp qua tất cả trang (API mặc định 20/trang, `tongSoTrang`) → lấy đủ 82 bệnh thay vì 20. Xin `kichThuocTrang=500` để giảm số lần gọi. Áp dụng mọi danh mục (bệnh, tỉnh, xã...). |
| 06/09/2026 | khainq | **Ô "ICD (hồ sơ)" lấy ICD được tích bệnh truyền nhiễm**: `GetInfectiousDiagnoses` quét ICD **chính (ICD_CODE) + phụ (ICD_SUB_CODE)** của hồ sơ đối chiếu `HIS_ICD.IS_INFECTIOUS=1` → hiển thị đúng (các) mã truyền nhiễm (kể cả khi nằm ở chẩn đoán phụ); dùng chính mã đó để tự chọn bệnh cổng + cascade phân độ. |
| 15/09/2026 | khainq | **Bổ sung "Xã/phường quản lý"** (nhóm "Địa chỉ hiện nay"): thêm combo `cboXaQuanLy` (bind danh mục `V_SDA_COMMUNE`), mặc định = xã hiện nay, cho sửa. Đẩy cổng `MaXaPhuongQuanLy` = mã xã đã chọn (fallback xã hiện nay → `MaDonVi`) — thay cách cũ hard-code `= MaDonVi` (mọi ca báo về xã của bệnh viện, sai địa bàn giám sát). Thêm ràng buộc bắt buộc trong `ValidateForm`. Không lưu DB (bảng ECDS không có cột, suy từ xã hiện nay mỗi lần). |
| 10/09/2026 | khainq | **Cập nhật thông tin bệnh nhân** (nhóm "Thông tin cá nhân"): thêm nút mở chức năng `HIS.Desktop.Plugins.PatientUpdate` (mẫu `EnterKskInfomantionVer2`) cho bệnh nhân của điều trị hiện tại — args `[PatientId, TreatmentId, DelegateSelectData]` qua `ModuleExt.PluginInstanceBehavior.ShowModule` (ShowDialog chặn). Đóng form → `ReloadPatientData` đọc lại `V_HIS_PATIENT`, đồng bộ `treatment.TDL_PATIENT_NAME/DOB/GENDER_ID` rồi `FillHanhChinhTab` nạp lại họ tên/ngày sinh/giới tính/CCCD/SĐT/dân tộc/nghề/địa chỉ. **CCCD + SĐT đặt `ReadOnly`** — chỉ sửa qua chức năng cập nhật BN. Partial `__PatientUpdate.cs`. |
| 06/09/2026 | khainq | **Tách danh mục bệnh cổng gộp mã thành từng dòng**: "A33, A34, A35" (Uốn ván) → 3 dòng A33/A34/A35 cùng tên. Combo bệnh đổi ValueMember `id`→`ma` (chọn đúng 1 mã ICD), thêm `GetBenhExpanded`/`FindBenhTokenByIcd`/`FindBenhIdByToken`. `maIcd10Benh`/`REPORTED_ICD_CODE` = token đang chọn; `REPORTED_DISEASE_ID` = id cổng của token. |
| 06/09/2026 | khainq | **Thêm ô "Bệnh (hồ sơ)" chỉ đọc** (giống ô NN hồ sơ) hiển thị mã ICD + tên gốc hồ sơ. |
| 06/09/2026 | khainq | **Rút gọn nhãn**: "Nghề nghiệp (hồ sơ/cổng)" → "NN (hồ sơ/cổng)". |
| 06/09/2026 | khainq | **Mặc định Loại cơ sở điều trị = "Y tế cơ quan"** khi form còn trống (EnsureRequiredDefaults). |
| 05/09/2026 | khainq | **Map bệnh cổng gộp nhiều mã**: danh mục bệnh cổng gộp mã ở cấp category ("A15, A16", "A33, A34, A35") trong khi ICD hồ sơ là mã con ("A15.0"). Thêm `IcdMatch` (khớp phân cấp: mã con ↔ category) cho `FindIdByMa` → tự chọn đúng bệnh. Khi đẩy: `ResolveDiseaseIcdToken` chọn đúng **1 token** cổng biết theo ICD hồ sơ (thay vì gửi cả chuỗi gộp hay token đầu sai). Áp dụng cho `maIcd10Benh` + `REPORTED_ICD_CODE`. |
| 05/09/2026 | khainq | **Combo lọc theo Contains**: thêm `PopupFilterMode=Contains` cho GridLookUpEdit → gõ **tên** (nằm giữa chuỗi "Mã - Tên") cũng lọc được, không chỉ mã ở đầu. |
| 05/09/2026 | khainq | **Đổi TẤT CẢ droplist sang `GridLookUpEdit` giống combo giới tính của HisIcd**: gõ **thẳng vào ô** (không cần ô tìm riêng) → popup mở ngay (`ImmediatePopup=true`) và nhảy tới dòng khớp **mã hoặc tên**. `TextEditStyle=Standard`, popup 1 cột "Mã - Tên" (ẩn header/group/indicator). Đổi toàn bộ `SearchLookUpEdit`→`GridLookUpEdit`. |
| 05/09/2026 | khainq | **(Thay bằng dòng trên)** Đổi droplist sang `SearchLookUpEdit` với ô tìm kiếm trong dropdown. |
| 05/09/2026 | khainq | **Giao diện 100% DevExpress + combo tìm theo Mã/Tên**: đổi `TableLayoutPanel` → **`SplitContainerControl`** (DevExpress, `FixedPanel=None` giữ 50/50). Combo danh mục đổi sang **1 cột hiển thị gộp "Mã - Tên"** (`DanhMucItemDto.MaTen`; dữ liệu SDA dân tộc/tỉnh/xã chiếu về `DanhMucItemDto`) + **`SearchMode=AutoFilter`** → gõ **mã HOẶC tên** để lọc dropdown. Sửa cột Mã che cột Tên (phân độ bệnh/thôn khó chọn). |
| 05/09/2026 | khainq | **Người báo cáo tự điền + ghi nhớ**: SĐT/Email (như Tên) lấy từ **tài khoản nhân viên đăng nhập** (`HIS_EMPLOYEE` theo `LOGINNAME` → `TDL_USERNAME`/`TDL_MOBILE`/`TDL_EMAIL`) trong `FillNguoiBaoCaoTab`. Đồng thời **nhớ** tên/SĐT/email đã nhập qua **ControlState** (`ControlStateWorker`, key `REPORTER_NAME/PHONE/EMAIL`) — lưu khi rời ô, tự khôi phục (ghi đè mặc định) cho bệnh nhân/phiên sau. Thêm ProjectReference `HIS.Desktop.Library.CacheClient`. |
| 05/09/2026 | khainq | **Combo Mã+Tên · giữ ô tìm kiếm · nghề nghiệp theo mã**: `ClearInputControls` chỉ xóa `pnlBody` (giữ panel tìm kiếm trái khi chọn BN); combo danh mục hiển thị 2 cột **Mã \| Tên**; thêm trường "Nghề nghiệp (hồ sơ)" chỉ đọc; nghề nghiệp cổng tự chọn theo **mã** `CAREER_CODE`→`ma` cổng. |
| 04/09/2026 | khainq | **Bổ sung trường cổng BẮT BUỘC (nghề nghiệp + hình thức điều trị)**: cổng bắt buộc `maNgheNghiep` + `maHinhThucDieuTri` (Swagger ghi optional nhưng thực tế required). (1) `cboNgheNghiep` chuyển bind THẲNG danh mục cổng `nghe-nghiep` (ValueMember=ma, mã "TT"/"CN"/"HSSV") thay HIS_CAREER → chọn ra mã cổng trực tiếp, bỏ `ResolveEcdsMa`. (2) Thêm combo **Hình thức điều trị** (`cboHinhThucDieuTri`: 1=Nội trú, 2=Ngoại trú — cổng không công bố danh mục, ví dụ dùng "2"), mặc định theo tình trạng hiện nay. Trường XN/vắc xin/địa chỉ chi tiết cũng là required — người dùng nhập trên form. |
| 04/09/2026 | khainq | **Giao diện 1 trang 2 cột (bỏ tab, không cuộn)**: gộp 2 tab (Đối tượng mắc bệnh + Trường hợp bệnh) thành **1 trang chia 2 cột** bằng `TableLayoutPanel` 50/50, mỗi cột 1 `LayoutControl`. Cột trái = Thông tin cá nhân/Địa chỉ HN/Địa chỉ TT/Chẩn đoán; cột phải = Diễn biến & Ra viện/Vắc xin & Xét nghiệm/Người báo cáo. Thu gọn ô nhập (nhãn 165→120px, dòng cao 24→22px, `MinSize` 320→200 để 2 field vừa 1 hàng trong cột nửa form, padding gọn); hạ header 168→138px. Toàn bộ vừa 1 trang, không cần cuộn dọc. Chỉ sửa `frmInfectiousDiseaseReport.Designer.cs`. |
| 04/09/2026 | khainq | **Fix đẩy cổng lỗi danh mục + popup xác nhận rỗng**: (1) cổng dùng mã dân tộc `"1"` còn HIS gửi `"01"` → `FindMaByMa` bổ sung **khớp theo SỐ (bỏ số 0 đầu)**; không đối chiếu được thì trả **null** → `ResolveEcdsMa` bỏ trường optional (dân tộc/nghề) khỏi payload thay vì gửi mã sai làm cổng từ chối cả ca (`KHONG_TIM_THAY_DANH_MUC`). (2) Popup xác nhận rỗng do `Message.Lang.vi.resx` bị build thành **satellite `vi/`** không được deploy tay → thêm **`Message.Lang.resx` neutral (tiếng Việt) nhúng thẳng DLL chính** (cả 2 plugin) để `ResourceMessage` luôn có nội dung. |
| 04/09/2026 | khainq | **Fix crash mở form + lỗi kết nối cổng**: (1) `FillDataFromHis` vỡ do `TypeLoadException MOS.SDO.HisEcdsDiseaseCaseFullSDO` (MOS.SDO/EFMODEL môi trường chạy lệch phiên bản) — tách phần đối soát GetFull ra `TryLoadAndMapSavedCase()` bọc try riêng để **cô lập tham chiếu SDO**: type lỗi thì chỉ bỏ qua đối soát, form vẫn load từ hồ sơ HIS; (2) `CreateClient` kiểm tra `BaseUrl` bằng `Uri.TryCreate` (Absolute + http/https) → ném `InvalidOperationException` báo rõ key config thay vì `UriFormatException` khó hiểu (áp dụng cả 2 plugin). |
| 03/09/2026 | khainq | **Giao diện form chi tiết — căn chỉnh & khoảng cách**: cột nhãn rộng đều **165px** + căn giữa dọc → mọi editor thẳng 1 mốc, caption dài không bị cắt; thêm **padding mỗi dòng** (3,2,6,3) cho nhịp dọc đều; **padding trong nhóm** (10,6,10,8) + **spacing tách nhóm** (top 6) qua `BeginSection`; nâng chiều cao header 156→168 cho cân. Chỉ chỉnh `frmInfectiousDiseaseReport.Designer.cs` (`AddRow`/`BeginSection`/`BuildHeader`). |
| 03/09/2026 | khainq | **Khớp DTO đẩy cổng theo Swagger `DiseaseCaseFastDto`** (`/api/fast/v1/ca-benh/cap-nhat`). Viết lại `EcdsDiseaseCaseDto` **camelCase** + danh mục truyền **MÃ (string)** thay ID nội bộ: `maIcd10Benh` (mã ICD-10) thay `BENHCHUANDOAN_ID` → **fix lỗi cổng "bạn phải chọn bệnh"** (bản cũ đẩy ID=0). Ngày đổi sang **`dd/MM/yyyy`** (`DiseaseCaseMapper.ToPortalDate`). `maGioiTinh`="M"/"F"; `maCoSoDieuTri`/`maDonViNguoiBaoCao`/`maXaPhuongQuanLy` từ config; `maPhanLoaiLamSang`/`maThonHienNay`/`maDonViXetNghiem` lấy `ma` cổng qua `GetSelectedMa`; dân tộc/nghề đối chiếu SDA→mã cổng (`ResolveEcdsMa`/`FindMaByMa`, fallback giữ mã). Thêm enum `EcdsTrangThaiCaBenh`(=1)/`EcdsTrangThaiLuu`(=2). Field null → không serialize (`NullValueHandling.Ignore`). |
...
> ⚠ **ĐÍNH CHÍNH theo Swagger chính thức (03/09/2026)** — hợp đồng THỰC TẾ của cổng khác spec nghiệp vụ ở dưới:
> Endpoint đẩy dùng schema **`DiseaseCaseFastDto`** (`/api/fast/v1/ca-benh/cap-nhat`, batch `/cap-nhat-nhieu` nhận **mảng thô** DTO). Tên trường **camelCase**; danh mục truyền **MÃ (string)** — VD `maIcd10Benh="A97"`, `maDanToc="1"`, `maXaHienNay="00136"` — **KHÔNG phải ID số nội bộ**. Ngày định dạng **`dd/MM/yyyy`**. Bắt buộc: `maIcd10Benh`, `hoVaTen`, `ngaySinh`, `maGioiTinh`("M"/"F"), `maXaHienNay`.
> ⇒ Code hiện hành (`EcdsDiseaseCaseDto`, `BuildDtoFromForm`) đã theo hợp đồng camelCase/mã-string này. Phần mô tả `UPPER_SNAKE` + `ID số nội bộ` bên dưới là **spec nghiệp vụ cũ, chỉ để tham khảo tầng dữ liệu** — KHÔNG dùng cho payload đẩy cổng.

Theo **tài liệu nghiệp vụ ECDS** (tham khảo), ca bệnh gồm 2 khối: **thông tin hành chính** + **TRUONG_HOP_BENH**. Tên trường dạng `UPPER_SNAKE`, danh mục là **ID số nội bộ ECDS** (VD `TINH_ID=709`, `XA_ID=127976`, `BENHCHUANDOAN_ID=40`).
