...
|------|-----------|-----------------|
| 08/05/2026 | sinhnt@vietsens.vn | **Lấy danh sách EMR_SIGNER từ cache static `MPS.ProcessorBase.PrintConfig.EmrSigners`** (giống cách MPS làm), thay vì call `api/EmrSigner/Get` mỗi lần. Bỏ helper `LoadEmrSignerByLoginname()`. Cache này được app chính nạp **1 lần khi đăng nhập** — không tốn API call lặp lại, đồng nhất với toàn bộ luồng print MPS. Lookup signer: `PrintConfig.EmrSigners.FirstOrDefault(o => o.LOGINNAME == loginname)`. Nếu cache null/rỗng → log Warn yêu cầu app chính nạp cache. |
