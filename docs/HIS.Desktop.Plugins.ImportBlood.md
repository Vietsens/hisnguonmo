...
|------|-----------|-----------------|
| 10/07/2026 | huannh | Loại nhập **Hiến máu**: khi chọn loại máu mà chưa chọn/sửa hồ sơ hiến máu → thay vì `return` âm thầm (nút Thêm không bật, không rõ lý do) nay hiện thông báo *"Vui lòng chọn hồ sơ hiến máu trước khi chọn loại máu!"* (message `VuiLongChonHoSoHienMauTruocKhiChonLoaiMau`, `ResourceMessageLang` + Message.Lang.vi/en). Chỉ báo khi người dùng chủ động chọn loại máu (`data != null`), bỏ qua các lời gọi reset `ProcessChoiceBloodTypeADO(null)`. Giữ nguyên thiết kế: luồng hiến máu bắt buộc chọn hồ sơ (chế độ sửa) trước. Kèm: thêm `UCImportBloodPlus__Plus__Qr.cs` vào .csproj (fix CS1061), đổi `ConfigurationSettings` → `ConfigurationManager` ở frmImportError. |
| 10/07/2026 | huannh | Nút **"Mới"** hồ sơ hiến máu (`ProcessBtnNew_BloodGiver`) nay cũng **xóa trắng ô QR túi máu** (`txtQrBloodBag`) — đồng bộ với hành vi sau khi bấm **Thêm** túi máu và nút **Làm mới (Ctrl N)** của form chính. |
