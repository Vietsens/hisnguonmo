...
|------|-----------|-----------------|
| 25/09/2026 | dangth2 | **Việc 3353 / 57799 — đưa lại ô tick "Có thuốc, vật tư đi kèm" lên test (không sửa code).** Từ 22/09 15:06 DLL trên HISTEST bị đè 4 lần (839ca6c1f, 83aa86dd7, 55a402dff, 18b0b98df) bằng bản build từ source chưa có `96cc83b9b` (danh sách Cấu hình chỉ còn 22 mục) → tester không bật/tắt được cờ `IS_REQUIRE_MEDI_MATE`. Chưa kịp đẩy thì 25/09 13:56 dev đó đẩy lại bản mới (HISTEST `37e858d23`) đã có ô tick; so IL với bản build từ Develop `adfecba4e` trùng hoàn toàn nên **không đè**. Bản 22-23/09 có thêm combo "PL PTTT giường" (`BED_PTTT_GROUP_ID`, `cboBedPtttGroup`) chưa push lên nhánh nào; bản 13:56 không còn combo này. Lần sau build HisService phải từ nhánh đã merge Develop để giữ cả hai. |
| 22/09/2026 | dangth2 | **Việc 3353 (PT-56272) — đổi mức xử lý từ CẢNH BÁO sang CHẶN** (chốt anh Cảnh 22/09/2026). Màn danh mục **không đổi code**: vẫn là mục 23 `CO_THUOC_VAT_TU_DI_KEM` ghi cờ `HIS_SERVICE.IS_REQUIRE_MEDI_MATE`. Thay đổi nằm ở phía dùng cờ: thư viện `Library.CheckRequireMediMate` nay **chặn** không cho kết thúc (hộp thông báo chỉ có nút OK, không còn Yes/No), và phạm vi mở rộng thành **5 màn** — thêm `TestServiceExecute` (màn "Xử lý dịch vụ" thứ hai, commit `359ff8013`). Hệ quả cho người khai danh mục: **tick cờ cho dịch vụ nào là khóa luôn việc kết thúc dịch vụ đó khi chưa kê thuốc/vật tư đi kèm**, không còn đường bỏ qua — cân nhắc kỹ trước khi tick, riêng dịch vụ **xét nghiệm** khuyến cáo chưa tick vì màn Xử lý xét nghiệm chưa có chỗ kê thuốc gắn vào dịch vụ. |
