...
|------|-----------|-----------------|
| 08/08/2026 | huannh + Claude | **vCong53286 Đấu nối hệ thống tiền giám định** Hai phần. **(1)** Nút **"Kiểm tra lỗi"** (`btnCheckTienGiamDinh` + `lciCheckTienGiamDinh`) khai báo trong **Designer**, đặt ở hàng nút phía trên **ngay bên trái nút Tìm** (vị trí 672; ba mục sau — `layoutControlItem6`/`11`/`10` — dịch phải 145px thành 817/962/1094). `InitTienGiamDinhButton()` trong Load chỉ đặt nhãn theo ngôn ngữ và bật/tắt `lciCheckTienGiamDinh.Visibility` theo trạng thái đấu nối. **Không thêm cột tích chọn mới** vì lưới đã có sẵn cột chọn nhiều dòng nuôi `listSelection`. Kết quả hiện ở `frmTienGiamDinhResult` (lưới chủ–chi tiết: mỗi hồ sơ một dòng, chọn dòng thì lưới dưới hiện chi tiết ba nhóm lỗi). Lô lớn chạy tuần tự trong `frmTienGiamDinhProgress` có tiến trình + nút Hủy, khoá màn hình trong lúc chạy; vượt 100 hồ sơ thì hỏi xác nhận kèm thời gian dự kiến (hệ ngoài giới hạn 60 lượt/phút, mỗi hồ sơ một lượt). **(2)** Cổng chặn `EnsureTienGiamDinhPassedAsync()` gắn vào **cả 5 chức năng xuất** (Xuất Xml / Xuất XML TT / Xuất XML gộp / Xuất XML 3176 / Xuất XML TT12 — 4 handler đồng bộ đổi sang `async void`): hồ sơ mang **lỗi nghiêm trọng** thì **dừng cả lượt, không sinh tệp nào** (chặn theo lô, tránh tệp ra một nửa); lỗi nhẹ thì cảnh báo cho qua sau xác nhận; hệ ngoài lỗi thì cho xuất kèm cảnh báo; hủy giữa chừng thì không xuất. Kết quả nhớ trong phiên theo mã điều trị để tránh gọi trùng, xoá khi tải lại danh sách. Màn này **không có công tắc bật/tắt riêng** — đã đấu nối là có hiệu lực. Config dùng chung: `HIS.TIEN_GIAM_DINH.CONNECTION_INFO`. Files: `ADO/TienGiamDinhADO.cs`, `Base/TienGiamDinhWorker.cs`, `UCExportXml___TienGiamDinh.cs`, `frmTienGiamDinhResult.cs/.Designer.cs`, `frmTienGiamDinhProgress.cs` (đều mới), `UCExportXml.cs`, `ADO/HisConfigCFG.cs`, `Resources/{Lang,Message.Lang}.{vi,en,my}.resx` + `ResourceMessageLang.cs`, `.csproj`. Tham chiếu: `_Software-Specs/02_Analysis_Design/PTTK_53286_Dau_Noi_Tien_Giam_Dinh_Check_Loi_XML.md` mục B.4.2. |
