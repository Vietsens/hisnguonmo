...
|------|-----------|-----------------|
| 26/06/2026 | nampp | **48008 BV JW — Hiển thị cột "Ghi chú thanh toán" (chỉ đọc).** Thêm 1 cột `SereServTreeColumn` FieldName=`PAYMENT_NOTE`, `allowEdit=false` (chỉ đọc) vào cây dịch vụ UC `HIS.UC.SereServTree` trong `InitSereServTree()` (VisibleIndex=12, sau "Mã yêu cầu"). Cột bind theo tên field nên không sửa được, không tính vào tiền; đọc `PAYMENT_NOTE` từ view dịch vụ (V_HIS_SERE_SERV_5/7) sau khi backend gencode bổ sung field. Cập nhật: `frmTransactionBill.cs` (InitSereServTree), `Resources/Lang.vi/en/my.resx` (key `IVT_LANGUAGE_KEY__FRM_TRANSACTION_BILL__TREE_SERE_SERV__COLUMN_PAYMENT_NOTE`). |
| 11/06/2026 | phuongnm | **Fix lỗi mất chiết khấu khi thanh toán 1 sổ ở giao diện CŨ** (config `MOS.HIS_TRANSACTION_ENABLE_MULTI_DISCOUNT` **!= 1, gồm null**). Nguyên nhân: trong `ProcessSave` (`frmTransactionBill__Plus__Button.cs`) gọi **vô điều kiện** `RecalculateTotalDiscountFromGrid()` — hàm này cộng tổng cột DISCOUNT của grid (rỗng ở chế độ ô đơn) rồi gán đè `totalDiscount = 0`, làm `EXEMPTION` gửi BE = 0 và Cần thu về tiền gốc. Fix: chỉ gọi `RecalculateTotalDiscountFromGrid()` khi `HisConfigCFG.EnableMultiDiscount` (key == "1"); chế độ ô đơn lấy `totalDiscount = txtDiscount.Value`. **Đồng thời** bổ sung validate **Lý do chiết khấu ≤ 250 byte UTF-8** (tiếng Việt có dấu 2-3 byte/ký tự) khi Lưu cho giao diện ô đơn — chặn lưu + cảnh báo, song song với `ValidateDiscountGridBeforeSave` của giao diện grid (key == 1). Thêm `ValidateSingleDiscountReasonBeforeSave` trong `frmTransactionBill__Plus__GridDiscount.cs`, gọi ở nhánh else save. Cập nhật: `frmTransactionBill__Plus__Button.cs`, `frmTransactionBill__Plus__GridDiscount.cs`. |
| 10/06/2026 | phuongnm | Giao diện CŨ (config `MOS.HIS_TRANSACTION_ENABLE_MULTI_DISCOUNT` != 1, ô đơn tại `BuildSingleDiscountControls`): bổ sung **tự tính chéo đ↔%** giống grid — nhập "Chiết khấu (đ)" tự điền "Chiết khấu (%)" và ngược lại (gốc là `totalPatientPrice`, làm tròn 4 số), chặn đệ quy bằng cờ `isSingleDiscountFromCode`. Đổi nhãn 2 ô rõ ràng thành **"Chiết khấu (đ)"** và **"Chiết khấu (%)"** (thu nhỏ ô nhập, bỏ caption "Chiết khấu:" trái để tránh trùng). Guard cờ ở `SetBillSuccessControl` (reflect sau lưu) và `ResetControlValue` (xóa form) để không kích hoạt tính chéo. Cập nhật: `frmTransactionBill__Plus__GridDiscount.cs`, `frmTransactionBill.cs` (2 handler `txtDiscount_EditValueChanged`/`txtDiscountRatio_EditValueChanged` + reset), `frmTransactionBill__Plus__Button.cs`. |
