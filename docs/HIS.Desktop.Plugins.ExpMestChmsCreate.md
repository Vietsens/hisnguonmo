...
| 2026-07-01 | huannh | YC5: Khi hình thức = Xuất (radioExport) mà chưa chọn Kho nhập (`cboImpMediStock`) rồi bấm **Thêm** thuốc/vật tư → hiển thị icon cảnh báo màu vàng tại combobox Kho nhập kèm tooltip "Kho nhập không được để trống" (trước đây không có phản hồi gì). Thêm method `ValidImpMediStockOnAdd()` ở đầu `btnAdd_Click` (`UCExpMestChmsCreate__Plus__Button.cs`) dùng `dxErrorProvider1.SetError(..., ErrorType.Warning)`; clear icon trong `cboImpMediStock_EditValueChanged` (khi chọn kho) và trong `LoadDataToCboMediStock` (khi chuyển sang hình thức Nhập). Thêm message `KhoNhapKhongDuocDeTrong` (`Base/ResourceMessageLang.cs` + `Message.Lang.vi/en.resx`). |
