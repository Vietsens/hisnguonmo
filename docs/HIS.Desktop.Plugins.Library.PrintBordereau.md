...
| 04/07/2026 | dangth2 | **PTTK 2883 — mục 2: Mps000504 bổ sung bộ key gom nhóm theo khoa/phòng theo mẫu 697 (Mps000508)** — đổi hướng từ port Mps000304 (6556) sang **Mps000508 (697)** vì viện không dùng 6556 nữa: `Mps000504Behavior` thêm `LoadExeRoomInput(rdo)` — nạp input pipeline vào PDO (SereServs đã lọc `[fromDateReq, toDateReq]` trên `TDL_INTRUCTION_TIME`, SereServExts, PatientTypeAlterAlls, HeinServiceTypes, Services, Rooms, Departments, DiimTypesList, medicineTypes/MedicineLines/ServiceReqs theo config `IS_SHOW_MEDICINE_LINE`, Branch, TreatmentTypes, `Config.PatientTypeCFG`/`Config.HeinServiceTypeCFG`, HisConfigValue (`IsSurgPriceOption_1`, `IsGroupHeinServiceByUseTime`...), HisServiceUnit, ListOtherPaySource) — mirror `Mps000508Behavior`. MPS side: port pipeline từ Mps000508 sang `MPS.Processor.Mps000504` (ADO SereServADO/GroupDepartmentADO/HeinServiceTypeExt/OtherSourceADO + DataRawProcess + PatientTypeAlterProcessor + AgeUtil + `Mps000504ProcessorPlus.cs` (DataInputProcess/GroupDisplayProcess) + `Mps000504Processor.ExeRoom.cs` (ExeRoomProcess) + ProcessOtherSource + ReplaceValueFunction); PDO port từ 508.PDO: `Mps000504PDO__PublicKey.cs` (SereServKey/PatyAlterBhytADO/HeinServiceTypeADO/MedicineLineADO/CDHACount/PatientADO), `HisConfigValue.cs`, `SingleKeyValue.cs`, `Config/*` + partial `Mps000504PDO__Plus.cs` (`TreatmentView` + input lists). **Bộ key temp dùng được (như 697)**: nền `<#Service.>`, `<#HeinServiceType.>`, `<#PatyAlterBHYT.>`, `<#PatyAlterBHYTAll.>`, `<#MedicineLine.>`, `<#HeinServiceTypeBed.>`, `<#CDHACountList.>`, `<#OtherPaySource.>`; gom nhóm `<#ServiceGroupByDepa.>` (khoa) → `<#ServiceGroupByRoom.>` (phòng) → `<#HeinServiceTypeExeRoom.>`/`<#HeinServiceTypeByDepa.>` (loại DV) → `<#ServiceExeRoom.>`/`<#ServiceExeRoomByDepa.>` (chi tiết) + đầy đủ relationships như Mps000508. CHƯA port bộ single key `*_697` (`SetSingleKey697_*`) — nếu temp viện dùng key đơn `_697` thì bổ sung sau. OFF-safe: input null (behavior cũ) → xuất danh sách rỗng, biểu in phẳng `<#SereServs.>` giữ nguyên. <br/> **Lưu ý build (multi-repo)**: rebuild `MPS.Processor.Mps000504.PDO` + `MPS.Processor.Mps000504`, copy DLL PDO mới vào `ReferencedAssemblies` (theo HintPath của PrintBordereau) / `LIB\MPSv2\MPS.PDO\` trước khi build HIS. |
...
### 2883 — Mps000504 bộ key gom nhóm theo khoa/phòng (mẫu 697 — Mps000508)
- [ ] Temp Mps000504 CŨ (chỉ dùng `<#SereServs.>`) → in như trước, không lỗi (regression).
- [ ] Temp 697 theo KHOA (`<#ServiceGroupByDepa.>` → `<#HeinServiceTypeByDepa.>` → `<#ServiceExeRoomByDepa.>`) → in chi phí gom nhóm theo khoa xử lý, giống bố cục Mps000508.
- [ ] Temp 697 theo KHOA+PHÒNG (`<#ServiceGroupByDepa.>` → `<#ServiceGroupByRoom.>` → `<#HeinServiceTypeExeRoom.>` → `<#ServiceExeRoom.>`) → gom đúng theo phòng xử lý.
- [ ] Lọc thời gian ở bảng kê rồi in Mps000504 → các band nhóm CHỈ chứa DV có `TDL_INTRUCTION_TIME` trong khoảng lọc; tổng nhóm khớp với vùng "CP theo ĐK lọc" phần BHYT.
- [ ] BN không có DV BHYT trong khoảng lọc → band nhóm rỗng, biểu vẫn in phần danh sách phẳng.
- [ ] Config `IS_SHOW_MEDICINE_LINE = 1` → band `MedicineLine` tách Tân dược/Chế phẩm như 508.
- [ ] Đối chiếu 1 BN in cùng khoảng thời gian bằng Mps000508 và Mps000504 (lọc toàn bộ đợt) → số liệu band nhóm khớp nhau.
