...
| 24/09/2026 | nampp + Claude | Theo API V1.5 (18/09/2026): `latest_request`/`requests[]` của KSK 2062 bỏ trường `status`, thay bằng trạng thái Kho (`hoc_status`, `hoc_validation_status`) và kết quả Cổng Bộ Y tế (`byt_status`, `byt_res_code`, `byt_res_msg`, `attempt_count`, `source_channel`). Thêm `VlgHoSoADO.FormatKskRequestStatus`: cột "Lần gửi cuối" hiện `hoc_status ?? status` kèm " \| Bộ: byt_status byt_res_code" (ví dụ `PROCESSED \| Bộ: PENDING`); lần gửi qua API cũ (LEGACY_API) vẫn hiện như trước. Chi tiết `/trang-thai` (`VlgPortalClient.ParseTrangThai`) hiện thêm kênh gửi, trạng thái Kho và dòng "Cổng Bộ Y tế: … \| số lần gửi Bộ". Nhóm KCB và HSSK 831 không đổi. Đã kiểm trên cổng dev 24/09. |
| 24/09/2026 | nampp + Claude | Chi tiết `/trang-thai`: sắp xếp "CÁC LẦN GỬI" và chọn "Lần gửi cuối" theo `received_at` kiểu thời gian (trước đây so chuỗi dạng "MM/dd/yyyy HH:mm:ss" do Newtonsoft tự đổi — sai thứ tự khi qua năm, mất phần lẻ giây). |
