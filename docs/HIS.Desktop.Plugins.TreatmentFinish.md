...
| 08/08/2026 | huannh + Claude | **vCong53286 Đấu nối hệ thống tiền giám định** Bổ sung bước kiểm tra thứ 14 vào chuỗi kiểm tra của chức năng "Kiểm tra hồ sơ": gọi `GET /api/order-check/violations` của hệ tiền giám định, truyền **chỉ mã điều trị**, nhận ba nhóm lỗi (sai sót y lệnh / lỗi tra thẻ BHYT / lỗi hồ sơ XML). **Hồ sơ có bất kỳ lỗi nào thì CHẶN kết thúc điều trị**, không phân biệt mức độ, cảnh báo đặt `IsSkippable = false` nên ô "bỏ qua cảnh báo" dùng chung không vô hiệu hoá được. Hệ ngoài lỗi/quá thời gian chờ thì **không chặn**. Tại thời điểm này nhóm lỗi XML luôn rỗng (tệp chưa sinh) nên thông báo phải nêu rõ chưa kiểm phần XML. Đăng ký ở hai chuỗi: `save()` (chế độ PopupMessage — chặn thật) và `kiemTraHoSo()` (chế độ GetListMessage — gom cảnh báo, gọi bất đồng bộ để không treo giao diện). Config: `HIS.TIEN_GIAM_DINH.CONNECTION_INFO` (`BaseURL \| token [\| timeout]`, không khai = tắt) + `HIS.TIEN_GIAM_DINH.TREATMENT_FINISH_ENABLE` (`1` = bật, mặc định tắt). Files: `EnumTienGiamDinh.cs` (mới), `ADO/TienGiamDinhADO.cs` (mới), `Base/TienGiamDinhWorker.cs` (mới), `FormTreatmentFinish__TienGiamDinh.cs` (mới), `FormTreatmentFinish.cs`, `Config/ConfigKey.cs`, `Resources/Message.Lang.{vi,en,my}.resx` + `ResourceMessage.cs` (10 chuỗi), `.csproj` (4 file + `System.Net.Http`). Tham chiếu: `_Software-Specs/02_Analysis_Design/PTTK_53286_Dau_Noi_Tien_Giam_Dinh_Check_Loi_XML.md` mục B.4.1. |
