...
|------|-----------|-----------------|
| 15/05/2026 | anhnh2 | Tinh chỉnh rule "bị lặp lại" trong `CheckValidDataInGridService` (file `UCBidCreate_Event.cs`) theo phản hồi tester: khi config `MOS.HIS_BID.ALLOW_ZERO_AMOUNT_IMPORT = 1`, KHÔNG bỏ check trùng hoàn toàn — thay vào đó **mở rộng tổ hợp check** bằng 4 trường: `MEDICINE_USE_FORM_ID` (Đường dùng), `PACKING_TYPE_NAME` (Quy cách đóng gói), `CONCENTRA` (Nồng độ), `DOSAGE_FORM` (Dạng bào chế). Quy tắc mới: 2 dòng cùng `MEDICINE_TYPE_CODE + SUPPLIER_ID + BID_GROUP_CODE` chỉ bị coi là "lặp lại" khi **cả 4 trường mở rộng cũng giống nhau**; nếu khác ít nhất 1 trong 4 → cho phép lưu. Đồng nhất với unique constraint DB `HIS_BID_MEDICINE_TYPE_UK1`/`HIS_BID_MATERIAL_TYPE_UK1`/`HIS_BID_BLOOD_TYPE_UK1` BE đã thêm (không bao gồm `REGISTER_NUMBER` — theo confirm của test). Khi config = 0 hoặc không có: giữ nguyên hành vi cũ (vẫn check trùng 3 trường). Check SL=0 + `ADJUST_AMOUNT` rỗng vẫn giữ logic cũ (bypass khi config bật). |
