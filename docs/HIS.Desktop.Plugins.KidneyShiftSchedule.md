...
| 09/07/2026 | dangth2 | **Fix chọn 1 trong 2 grid (đột xuất / theo lịch)**: grid phải chuyển sang dùng `gridViewHemoSchedule_FocusedRowChanged` (thay vì chỉ `Click` — lần click đầu `GetFocusedRow` trả null nên `treatmentId` giữ dòng grid trái). Khi chọn 1 grid thì clear selection grid còn lại (mutual exclusivity), cờ `isLoadingHemoGrid` chặn cướp selection lúc bind. `treatmentId` là nguồn dữ liệu chuẩn cho `ProcessAddClick`. |

| 16/07/2026 | dangth2 | **Bộ lọc riêng vùng "BN theo lịch"**: wire code cho các control dựng tay trong VS Designer — Phòng chạy (`cboExecuteRoomHemo`/`txtExecuteRoomHemo`), Ngày (`dtHemoScheduleDate` + prev/next `btnDatePreviousHemo`/`btnDateNextHemo`), Ca (`cboShiftHemoSchedule` + prev/next `btnShiftPreviousHemo`/`btnShiftNextHemo`), từ khóa (`txtSearchForHemoSchedule`), Tìm (`btnSearchForHemoSchedule`). `FillDataToGridHemoSchedule` lọc theo bộ lọc riêng (Phòng + Ca + Ngày đơn + từ khóa); `InitHemoScheduleFilter` init combo + mặc định hôm nay; `WireHemoScheduleFilterEvents` gắn sự kiện nút trong constructor. |

| 16/07/2026 | dangth2 | **Validate vùng "Đưa vào lịch" đúng thiết kế**: `ValidateForm` chỉ còn bắt buộc 5 trường vùng dưới (Ngày, Ca, Dịch vụ, ĐTTT, Người chỉ định); bỏ validate Máy + Gói (optional) và bỏ validate `cboDepartment`/`dteInTimeFrom`/`dteInTimeTo` (bộ lọc grid trái, không thuộc vùng dưới). `ProcessAddClick`: RoomId lấy theo grid đang chọn (grid phải → `cboExecuteRoomHemo`, grid trái → `cboExecuteRoom`). Wire cứng event grid phải trong code + tô sáng 1-trong-2 grid bằng RowStyle. |
