...
|------|-----------|-----------------|
| 24/06/2026 | huannh | Gắn cột "Thời gian thực hiện" (UseTime) vào config key mới **`HIS.Desktop.Plugins.BedHistory.UseTime`** (HisConfigKeys.cs) — chỉ thêm key + gate, **giữ nguyên tên cột và message như cũ**. **Chỉ khi config = "1"**: hiện cột ở **cả 2 grid** — `Gv_BedServiceType__Gc_UseTime` (grid chỉ định, editable) và `Gv_BedServiceReq__Gc_UseTime` (grid yêu cầu DV giường, hiển thị); bật cảnh báo T7/CN (gate bằng cờ tĩnh `HisBedServiceTypeADO.IsUseTimeConfigOn` trong `GetPropertyError`); truyền `UseTime` vào `HisBedServiceSDO` khi có giá trị. Khi config ≠ 1 → ẩn cả 2 cột, không cảnh báo, không truyền UseTime. Thêm method `ApplyUseTimeConfig()` gọi trong `FormBedHistory_Load`. |
