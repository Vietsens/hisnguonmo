...
| 29/07/2026 | tuanln | PT-44730: khóa ô ĐTTT **theo từng dòng** với dịch vụ đã khai trong bảng cấu hình `HIS_SERVICE_DEFAULT_PATY` (chờ backend). Thêm partial `frmBordereau___ServiceDefaultPaty.cs` (worker nạp cấu hình 1 lần/form + `IsAllowEditPatientTypeByServiceConfig`, người chỉ định lấy theo `TDL_REQUEST_LOGINNAME` của dòng). `gridViewBordereau_CustomRowCellEdit` cột `PATIENT_TYPE_ID`: thêm nhánh không đủ quyền → `repositoryItemLookUpEdit_PatientType_Disable`, đặt sau nhánh `DepaPatientTypeMode.Lock`. Dòng của dịch vụ không khai cấu hình vẫn sửa bình thường. Ngoài ra bổ sung `frmBordereau___ConsultNote.cs` vào csproj — file đã commit ở 0f796b938 nhưng thiếu entry Compile nên project không biên dịch được từ CLI |
