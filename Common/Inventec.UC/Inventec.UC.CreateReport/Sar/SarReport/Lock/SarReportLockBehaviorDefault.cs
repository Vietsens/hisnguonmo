...
{
    class SarReportLockBehaviorDefault : Inventec.UC.CreateReport.Base.BusinessBase, ISarReportLock
    {
        internal SarReportLockBehaviorDefault(CommonParam paramGet, SAR.EFMODEL.DataModels.SAR_REPORT data)
            : base(paramGet)
        {
            Data = data;
        }
        ...
        public SAR.EFMODEL.DataModels.SAR_REPORT Lock()
        {
            ...
            try
            {
                var rs = Base.ApiConsumerStore.SarConsumer.Post<Inventec.Core.ApiResultObject<SAR.EFMODEL.DataModels.SAR_REPORT>>("/api/SarReport/Lock", param, Data);
                if (rs != null)
                {
                    param = rs.Param != null ? rs.Param : param;
                    result = rs.Data;
                }
                if (result == null) { LogInOut(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => rs), rs) + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => Data), Data)); } return result;
            }
