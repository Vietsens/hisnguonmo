...
{
    class SarReportSendBehaviorDefault : Inventec.UC.ListReports.Base.BusinessBase, ISarReportSend
    {
        internal SarReportSendBehaviorDefault(CommonParam paramSend, SAR.EFMODEL.DataModels.SAR_REPORT data)
            : base(paramSend)
        {
            Data = data;
        }
        ...
        public SAR.EFMODEL.DataModels.SAR_REPORT Send()
        {
            ...
            try
            {
                var rs = Base.ApiConsumerStore.SarConsumer.Post<Inventec.Core.ApiResultObject<SAR.EFMODEL.DataModels.SAR_REPORT>>("/api/SarReport/Send", param, Data);
                if (rs != null)
                {
