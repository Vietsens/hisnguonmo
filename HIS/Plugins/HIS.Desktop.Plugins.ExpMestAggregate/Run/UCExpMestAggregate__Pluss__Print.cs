        ...
        private void OnClickInTraPhieuDoiThuoc(string fileName)
        {
            try
            {
                ...
                foreach (var item in dicMedi)
                {
                    ...
                    Inventec.Common.SignLibrary.ADO.InputADO inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode((!string.IsNullOrWhiteSpace(this.currentAggrExpMest.TDL_TREATMENT_CODE) ? this.currentAggrExpMest.TDL_TREATMENT_CODE : printTypeCode), "Mps000047", this.currentModule.RoomId);
