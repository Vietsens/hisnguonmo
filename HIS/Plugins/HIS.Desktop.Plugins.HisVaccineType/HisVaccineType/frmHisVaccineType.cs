...
{
    public partial class frmHisVaccineType : HIS.Desktop.Utility.FormBase
    {
        #region ---Decalre
        ...
        int dataTotal = 0;
        int rowCount = 0;
        HIS_VACCINE_TYPE CurrentData;
        RefeshReference dlgRefresh;
        #endregion
        public frmHisVaccineType()
        {
            InitializeComponent();
        }
        public frmHisVaccineType(Module module, RefeshReference dlg)
            : base(module)
        {
            try
            {
        ...
        private void frmHisAccidentBodyPart_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ValidateText(txtHisVaccineTypeCode, 20);
                ValidateText(txtHisVaccineTypeName, 100);
            }
            catch (Exception ex)
            {
        ...
        private void SetDefautData()
        {
            try
            {
                ...
                txtHisVaccineTypeCode.Text = "";
                txtHisVaccineTypeName.Text = "";
                txtSearch.Text = "";
                txtSearch.Focus();
            }
            catch (Exception ex)
        ...
        private void ChangeDataRow(HIS_VACCINE_TYPE datarow)
        {
            try
            {
                if (datarow != null)
        ...
        private void FillDataEditorControl(HIS_VACCINE_TYPE datarow)
        {
            try
            {
                
                txtHisVaccineTypeCode.Text = datarow.VACCINE_TYPE_CODE;
                txtHisVaccineTypeName.Text = datarow.VACCINE_TYPE_NAME;
                cboTypeVaccine.EditValue = datarow.TYPE_VACCINE;
            }
            catch (Exception ex)
            {
        ...
        private void UpdateDataFromform(HIS_VACCINE_TYPE updateDTO)
        {
            try
            {
                updateDTO.VACCINE_TYPE_CODE = txtHisVaccineTypeCode.Text.Trim();
                updateDTO.VACCINE_TYPE_NAME = txtHisVaccineTypeName.Text.Trim();
                if (cboTypeVaccine.EditValue != null && cboTypeVaccine.EditValue != DBNull.Value)
                    updateDTO.TYPE_VACCINE = System.Convert.ToInt16(cboTypeVaccine.EditValue);
                else
                    updateDTO.TYPE_VACCINE = null;
            }
            catch (Exception ex)
            {
                ...
                try
                {
                    foreach (DevExpress.XtraLayout.BaseLayoutItem item in lcEditorInfo.Items)
                    {
                        ...
                        if (lci != null && lci.Control != null && lci.Control is DevExpress.XtraEditors.BaseEdit)
                        {
                            ...
                            txtHisVaccineTypeCode.Focus();
                            txtHisVaccineTypeCode.SelectAll();
                        }
                    }
        ...
        private void SaveProcessor()
        {
            try
            {
                ...
                HIS_VACCINE_TYPE UpdateDTO = new HIS_VACCINE_TYPE();
                UpdateDataFromform(UpdateDTO);
                if (this.ActionType == GlobalVariables.ActionAdd)
                {
                    var ResultData = new BackendAdapter(param).Post<HIS_VACCINE_TYPE>(HisRequestUriStore.HisVaccineType_Create, ApiConsumers.MosConsumer, UpdateDTO, param);
                    if (ResultData != null)
                    {
                        BackendDataWorker.Reset<HIS_VACCINE_TYPE>();
                        FillDataToGridControl();
                        success = true;
                        RestFromData();
                        if (dlgRefresh != null)
                ...
                else
                {
                    if (CurrentData != null)
                    {
                        ...
                        var ResultData = new BackendAdapter(param).Post<HIS_VACCINE_TYPE>(HisRequestUriStore.HisVaccineType_Update, ApiConsumers.MosConsumer, UpdateDTO, param);
                        if (ResultData != null)
                        {
                            BackendDataWorker.Reset<HIS_VACCINE_TYPE>();
                            FillDataToGridControl();
        ...
        private void LoadPaging(object commonParam)
        {
            try
            {
                ...
                ApiResultObject<List<HIS_VACCINE_TYPE>> apiResult = null;
               ...
               // SetFilter(ref filter);
                ...
                apiResult = new BackendAdapter(paramcommon).GetRO<List<HIS_VACCINE_TYPE>>(HisRequestUriStore.HisVaccineType_Get, ApiConsumers.MosConsumer, filter, paramcommon);
                ...
                if (apiResult != null)
                {
                    var data = (List<HIS_VACCINE_TYPE>)apiResult.Data;
        ...
        private void SetFilter(ref HisVaccineTypeFilter filter)
        {
            try
            {
                filter.ORDER_DIRECTION = "DESC";
        ...
        private void gridViewAccidentBodyPart_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var data = (HIS_VACCINE_TYPE)gridViewHisVaccineType.GetRow(e.RowHandle);
        ...
        private void gridViewAccidentBodyPart_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    HIS_VACCINE_TYPE DataRow = (HIS_VACCINE_TYPE)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    if (DataRow != null)
                    {
                        DevExpress.XtraGrid.Views.Grid.GridView view = sender as DevExpress.XtraGrid.Views.Grid.GridView;
        ...
        private void gridViewAccidentBodyPart_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var dataRow = (HIS_VACCINE_TYPE)((IList)((BaseView)sender).DataSource)[e.RowHandle];
                    if (dataRow != null)
                    {
        ...
        private void gridViewAccidentBodyPart_Click(object sender, EventArgs e)
        {
            try
            {
                var datarow = (HIS_VACCINE_TYPE)gridViewHisVaccineType.GetFocusedRow();
                if (datarow != null)
                {
        ...
        private void txtAccidentBodyPartCode_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                {
                    txtHisVaccineTypeName.Focus();
                    txtHisVaccineTypeName.SelectAll();
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnLock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                CommonParam param = new CommonParam();
                var rowData = (HIS_VACCINE_TYPE)gridViewHisVaccineType.GetFocusedRow();
                bool success = false;
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonKhoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (rowData != null)
                    {
                        HIS_VACCINE_TYPE Result = new BackendAdapter(param).Post<HIS_VACCINE_TYPE>(HisRequestUriStore.HisVaccineType_Changelock, ApiConsumer.ApiConsumers.MosConsumer, rowData.ID, param);
                        if (Result!=null)
                        {
                            BackendDataWorker.Reset<HIS_VACCINE_TYPE>();
                            success = true;
        ...
        private void btnDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonXoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (rowData != null)
                    {
                        success = new BackendAdapter(param).Post<bool>(HisRequestUriStore.HisVaccineType_Delete, ApiConsumer.ApiConsumers.MosConsumer, rowData.ID, param);
                        if (success)
                        {
                            BackendDataWorker.Reset<HIS_VACCINE_TYPE>();
                            FillDataToGridControl();
        ...
        #region ---Loại vắc-xin (TYPE_VACCINE)
        private void InitCboTypeVaccine()
        {
            try
            {
                List<TypeVaccineADO> dataSource = new List<TypeVaccineADO>()
                {
                    new TypeVaccineADO(){ KEY = 1, NAME = "Tiêm chủng cơ bản cho trẻ em" },
                    new TypeVaccineADO(){ KEY = 2, NAME = "Tiêm chủng ngoài TCMR" },
                    new TypeVaccineADO(){ KEY = 3, NAME = "Tiêm chủng UV cho phụ nữ có thai" }
                };
                cboTypeVaccine.Properties.DataSource = dataSource;
                cboTypeVaccine.Properties.ValueMember = "KEY";
                cboTypeVaccine.Properties.DisplayMember = "NAME";
                cboTypeVaccine.Properties.PopulateViewColumns();
                cboTypeVaccine.Properties.View.OptionsView.ShowColumnHeaders = false;
                if (cboTypeVaccine.Properties.View.Columns["KEY"] != null)
                    cboTypeVaccine.Properties.View.Columns["KEY"].Visible = false;
            }
            catch (Exception ex)
            {
                LogSystem.Warn(ex);
        ...
        private void cboTypeVaccine_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                if (e.Button != null && e.Button.Kind == DevExpress.XtraEditors.Controls.ButtonPredefines.Delete)
                {
                    cboTypeVaccine.EditValue = null;
                }
            }
            catch (Exception ex)
            {
