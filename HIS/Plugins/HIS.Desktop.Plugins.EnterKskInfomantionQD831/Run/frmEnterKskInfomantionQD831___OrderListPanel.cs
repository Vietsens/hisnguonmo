    ...
    public partial class frmEnterKskInfomantionQD831
    {
        ...
        private void gridViewYlenh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ReloadForServiceReq(row.ID); // tự guard theo _loadedKskServiceReqId (bỏ qua nếu đã nạp)
            }
            catch (Exception ex) { Inventec.Common.Logging.LogSystem.Error(ex); }
        }
        ...
        private void gridViewYlenh_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                ReloadForServiceReq(row.ID); // tự guard theo _loadedKskServiceReqId
        ...
        private void ReloadForServiceReq(long serviceReqId)
        {
            try
            {
                if (serviceReqId <= 0 || serviceReqId == _loadedKskServiceReqId) return; // đã nạp y lệnh này rồi
                ...
                // XÓA dữ liệu y lệnh cũ trước khi nạp y lệnh mới.
                ClearAllKskControls();
                ...
                // Nạp lại dữ liệu nền + thông tin hành chính + hồ sơ KSK theo y lệnh mới.
                PrefetchFormData();
                ShowInformationPatient();
                LoadKskProfileFull();          // GetFull -> đổ đủ dữ liệu y lệnh mới (tab lazy nạp khi mở/nếu đã Init)
                _loadedKskServiceReqId = sreq.ID;
