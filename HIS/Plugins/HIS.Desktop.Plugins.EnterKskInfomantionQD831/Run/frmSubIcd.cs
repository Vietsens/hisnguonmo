...
namespace HIS.Desktop.Plugins.EnterKskInfomantionQD831.Run
{
    public partial class frmSubIcd : Form
    {
        ...
        private void Language_secondaryDisease()
        {
            try
            {
                grdColCode.Caption = "Mã bệnh";
                grdColName.Caption = "Tên bệnh";
                txtIcdNames.Properties.NullValuePrompt = "Danh sách tên bệnh";
                txtIcdCodes.Properties.NullValuePrompt = "Danh sách mã bệnh";
                btnChoose.Text = "Chọn";
                txtKeyword.Properties.NullValuePrompt = "Nhập mã hoặc tên bệnh để tìm";
                gridViewSecondaryDisease.OptionsFind.FindNullPrompt = "Nhập mã hoặc tên bệnh để tìm";
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
