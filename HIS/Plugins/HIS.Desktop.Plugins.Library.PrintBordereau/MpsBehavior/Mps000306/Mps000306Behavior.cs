...
namespace HIS.Desktop.Plugins.Library.PrintBordereau.Mps000306
{
    public class Mps000306Behavior : MpsDataBase, ILoad
    {
        private string documentName;
        public Mps000306Behavior(long? roomId, V_HIS_PATIENT_TYPE_ALTER currentPatientTypeAlter, List<HIS_SERE_SERV> _sereServs, List<V_HIS_DEPARTMENT_TRAN> _departmentTrans, List<V_HIS_TREATMENT_FEE> _treamentFees, V_HIS_TREATMENT _treatment, V_HIS_PATIENT _patient, List<V_HIS_ROOM> _rooms, List<V_HIS_SERVICE> _services, List<HIS_HEIN_SERVICE_TYPE> _heinServiceTypes, long _totalDayTreatment, string _statusTreatmentOut, string _departmentName, string _roomName, string _userNameReturnResult, string documentname,List<HIS_TRANSACTION> ListTransaction)
        ...
        bool ILoad.Load(string printTypeCode, string fileName, Inventec.Common.FlexCelPrint.DelegateReturnEventPrint returnEventPrint)
        {
            ...
            try
            {
                MPS.Processor.Mps000306.PDO.HeinServiceTypeCFG heinServiceType = new MPS.Processor.Mps000306.PDO.HeinServiceTypeCFG();
                ...
                MPS.Processor.Mps000306.PDO.PatientTypeCFG patientTypeCFG = new MPS.Processor.Mps000306.PDO.PatientTypeCFG();
                ...
                MPS.Processor.Mps000306.PDO.Mps000306PDO rdo = null;
                if (isShowMedicineLine == 1)
                {
                    ...
                    rdo = new MPS.Processor.Mps000306.PDO.Mps000306PDO(this.CurrentPatientTypeAlter, patientTypeAlters, DepartmentTrans, TreatmentFees, heinServiceType, patientTypeCFG, this.SereServs, sereServExts, Treatment, this.Patient, HeinServiceTypes, Rooms, Services, treatmentTypes, branch, medicineTypes, materialTypes, medicineLines, serviceReqs, departments, singleValue, servuceUnit, hisConfigValue, mediOrg, otherPaySource, this.ListTransaction);
                }
                else
                {
                    rdo = new MPS.Processor.Mps000306.PDO.Mps000306PDO(this.CurrentPatientTypeAlter, patientTypeAlters, DepartmentTrans, TreatmentFees, heinServiceType, patientTypeCFG, this.SereServs, sereServExts, Treatment, this.Patient, HeinServiceTypes, Rooms, Services, treatmentTypes, branch, materialTypes, departments, singleValue, servuceUnit, hisConfigValue, mediOrg, otherPaySource, this.ListTransaction);
                }
