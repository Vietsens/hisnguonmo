...
using MPS.Processor.Mps000510.PDO.Config;
...
namespace HIS.Desktop.Plugins.Library.PrintBordereau.Mps000510
{
    // PTTK 2689 — mục 3.5: Bảng kê ngoại trú Viện phí theo QĐ 697/QĐ-BYT
    // Logic xử lý giống Mps000281Behavior, khác biệt: PDO yêu cầu List<V_HIS_SERE_SERV_2> (view denormalized)
    // → fetch qua api/HisSereServ/GetView2 thay cho List<HIS_SERE_SERV> sẵn có
    // Mps000511 (nội trú Viện phí 697) tái sử dụng class này (giống Mps000282 reuse Mps000281)
    public class Mps000510Behavior : MpsDataBase, ILoad
    {
        private string documentName;
        public Mps000510Behavior(long? roomId, V_HIS_PATIENT_TYPE_ALTER currentPatientTypeAlter, List<HIS_SERE_SERV> _sereServs,
        ...
        bool ILoad.Load(string printTypeCode, string fileName, Inventec.Common.FlexCelPrint.DelegateReturnEventPrint returnEventPrint)
        {
            ...
            try
            {
                MPS.Processor.Mps000510.PDO.Config.HeinServiceTypeCFG heinServiceType = new MPS.Processor.Mps000510.PDO.Config.HeinServiceTypeCFG();
                ...
                MPS.Processor.Mps000510.PDO.Config.PatientTypeCFG patientTypeCFG = new MPS.Processor.Mps000510.PDO.Config.PatientTypeCFG();
                ...
                // 697/QĐ-BYT: Mps000510PDO yêu cầu List<V_HIS_SERE_SERV_2> (view denormalized) thay cho HIS_SERE_SERV
                // Lấy V_HIS_SERE_SERV_2 theo treatment rồi lọc lại theo danh sách SereServs (đã được filter ngoài form)
                HisSereServView2Filter sereServView2Filter = new HisSereServView2Filter();
                sereServView2Filter.TREATMENT_IDs = new List<long> { this.Treatment.ID };
                List<V_HIS_SERE_SERV_2> sereServView2All = new BackendAdapter(param)
                    .Get<List<MOS.EFMODEL.DataModels.V_HIS_SERE_SERV_2>>("api/HisSereServ/GetView2", ApiConsumers.MosConsumer, sereServView2Filter, param);
                HashSet<long> selectedSsIds = new HashSet<long>(this.SereServs.Select(o => o.ID));
                List<V_HIS_SERE_SERV_2> sereServView2 = sereServView2All != null
                    ? sereServView2All.Where(o => selectedSsIds.Contains(o.ID)).ToList()
                ...
                PrintCustomShow<Mps000510PDO> printShow = new PrintCustomShow<Mps000510PDO>(printTypeCode, fileName, rdo, returnEventPrint, this.isPreview);
