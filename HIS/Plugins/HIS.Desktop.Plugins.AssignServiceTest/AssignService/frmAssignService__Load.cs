    ...
    public partial class frmAssignService : HIS.Desktop.Utility.FormBase
    {
        ...
        private void ProcessChoiceServiceReqPrevious(MOS.EFMODEL.DataModels.V_HIS_SERVICE_REQ_6 serviceReq)
        {
            try
            {
                if (serviceReq != null)
                {
                    ...
                    List<MOS.EFMODEL.DataModels.HIS_SERE_SERV> sereServs = new BackendAdapter(param).Get<List<MOS.EFMODEL.DataModels.HIS_SERE_SERV>>(HisRequestUriStore.HIS_SERE_SERV_GET, ApiConsumers.MosConsumer, sereServFilter, ProcessLostToken, param);
        ...
        private void SetDefaultData()
        {
            try
            {
                ...
                pnlPrintAssignService.Enabled = false;
                ...
                //txtDescription.Text = "";
                //chkExpendAll.Checked = false;
                lblTotalServicePrice.Text = "0";
                this.actionType = GlobalVariables.ActionAdd;
                dtInstructionTime.EditValue = DateTime.Now;
        ...
        private void InitConfig()
        {
            try
            {
                this.currentServiceTypeId__Bed = IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__G;
        ...
        void VisibleColumnInGridControlService()
        {
            try
            {
                ...
                {
                    this.gridColumnChiPhiNgoaiGoi_TabService.Visible = false;
                }
        ...
        {
            Thread thread = new System.Threading.Thread(new System.Threading.ParameterizedThreadStart(this.LoadDataByPackageServiceNewThread));
            try
            {
                thread.Start(param);
            }
        ...
        private void LoadDataSereServWithTreatment(HisTreatmentWithPatientTypeInfoSDO treatment, DateTime? intructionTime)
        {
            try
            {
                if (treatment != null)
                {
                    ...
                    hisSereServFilter.INTRUCTION_TIME_FROM = Inventec.Common.DateTime.Get.StartDay();
                    hisSereServFilter.INTRUCTION_TIME_TO = Inventec.Common.DateTime.Get.EndDay();
                    hisSereServFilter.SERVICE_TYPE_ID = IMSys.DbConfig.HIS_RS.HIS_SERVICE_REQ_TYPE.ID__XN;
        ...
        private void FillDataIntoPatientTypeCombo(SereServADO data, GridLookUpEdit patientTypeCombo)
        {
            try
            {
                ...
                {
                    var arrPatientTypeCode = this.servicePatyInBranchs[data.SERVICE_ID].Select(o => o.PATIENT_TYPE_CODE).ToList();
        ...
        private void FillDataIntoExcuteRoomCombo(SereServADO data, DevExpress.XtraEditors.GridLookUpEdit excuteRoomCombo)
        {
            try
            {
                ...
                if (excuteRoomCombo != null && executeRoomViews != null && serviceRoomViews != null && serviceRoomViews.Count > 0)
                {
                    ...
                    {
                        InitComboExecuteRoom(excuteRoomCombo, null);
                    }
                }
            }
            catch (Exception ex)
