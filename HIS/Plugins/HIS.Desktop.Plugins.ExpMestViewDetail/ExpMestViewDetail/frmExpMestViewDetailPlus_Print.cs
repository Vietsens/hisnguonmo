...
namespace HIS.Desktop.Plugins.ExpMestViewDetail.ExpMestViewDetail
{
    public partial class frmExpMestViewDetail : HIS.Desktop.Utility.FormBase
    {
        ...
        private bool MpsPrinterRun(string treatmentCode,
            ...
            MPS.ProcessorBase.PrintConfig.PreviewType previewType)
        {
            ...
            try
            {
                ...
                {
                    treatmentCode = !string.IsNullOrEmpty(this._CurrentExpMest?.TDL_TREATMENT_CODE) ? this._CurrentExpMest.TDL_TREATMENT_CODE : printTypeCode;
                }
        ...
        //private Inventec.Common.SignLibrary.ADO.InputADO ProcessPrint(String printTypeCode)
        //{
        //    Inventec.Common.SignLibrary.ADO.InputADO inputADO = new Inventec.Common.SignLibrary.ADO.InputADO();
        //    try
        //    {
        ...
        //        inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode((this._CurrentExpMest != null && this._CurrentExpMest.TDL_TREATMENT_CODE != null ? this._CurrentExpMest.TDL_TREATMENT_CODE : printTypeCode), printTypeCode, this.moduleData != null ? this.moduleData.RoomId : 0);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        void RunPrint(string printTypeCode, string fileName, object data, Inventec.Common.FlexCelPrint.DelegateEventLog EventLogPrint, bool result, long? roomId)
        {
            try
            {
                ...
                {
                    Inventec.Common.SignLibrary.ADO.InputADO inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode(_CurrentExpMest.TDL_TREATMENT_CODE ?? printTypeCode, printTypeCode, roomId);
        ...
        private void InPhieuXuatTheoDieuKien(string printTypeCode, string fileName, ref bool result)
        {
            ...
            try
            {
                ...
                MPS.Processor.Mps000134.PDO.Mps000134PDO mps000134PDO = new MPS.Processor.Mps000134.PDO.Mps000134PDO(
                listMetyReq,
                listMatyReq,
                listMedicine,
                listMaterial,
                ...
                result = MpsPrinterRun(printTypeCode, printTypeCode, fileName, mps000134PDO, MPS.ProcessorBase.PrintConfig.PreviewType.Show);
                //if (result)
                //{
                //    this.Close();
                //}
        ...
        private void InPhieuXuatBuThuocLe(string printTypeCode, string fileName, ref bool result)
        {
            try
            {
                ...
                MPS.Processor.Mps000216.PDO.Mps000216PDO pdo = new MPS.Processor.Mps000216.PDO.Mps000216PDO(
            ...
            try
            {
                ...
                if (expMestBltyReqs != null && expMestBltyReqs.Count > 0)
                {
                 ...
                 keyPhieuTra
                    ...
                    result = MpsPrinterRun(null, printTypeCode, fileName, mps000198PDO, MPS.ProcessorBase.PrintConfig.PreviewType.Show);
                }
        ...
        private void InPhieuXuatMatMat(string printTypeCode, string fileName, ref bool result)
        {
            try
            {
                ...
                result = MpsPrinterRun(null, printTypeCode, fileName, mps000168PDO, MPS.ProcessorBase.PrintConfig.PreviewType.ShowDialog);
            }
            catch (Exception ex)
            {
                WaitingManager.Hide();
                ...
                //else
                {
                    ...
                    if (keyGOP == 1)
                    {
                        ...
                        if (_ExpMestMetyReq_GN_HTs != null && _ExpMestMetyReq_GN_HTs.Count > 0)
                        {
                            ...
                            result = MpsPrinterRun(null, printTypeCode, fileName, mps000135RDO, MPS.ProcessorBase.PrintConfig.PreviewType.Show);
                        }
                        #endregion
                    }
                    else
        ...
        (
         ...
         "THUỐC THƯỜNG"
                    ...
                    Inventec.Common.SignLibrary.ADO.InputADO inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode(_CurrentExpMest.TDL_TREATMENT_CODE ?? printTypeCode, printTypeCode, this.moduleData.RoomId);
        ...
        private void Mps000347(ref bool result, string printTypeCode, string fileName)
        {
            try
            {
                ...
                if (this._ExpMestMetyReqs_Print != null && this._ExpMestMetyReqs_Print.Count > 0)
                {
                    ...
                    {
                        var dataMedi = mediTypes.FirstOrDefault(p => p.ID == item.MEDICINE_TYPE_ID);
                        Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(" medicine id:", dataMedi.ID));

                        Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData("dataMedi group id:", dataMedi.MEDICINE_GROUP_ID));
                        Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData("dataMedi group name:", dataMedi.MEDICINE_GROUP_NAME));
                        Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData("medicine name:", dataMedi.MEDICINE_TYPE_NAME));
                        if (dataMedi != null)
                        {
                            if (dataMedi.IS_CHEMICAL_SUBSTANCE == 1)
                            {
                            ...
                            else if (dataMedi.MEDICINE_GROUP_ID == IMSys.DbConfig.HIS_RS.HIS_MEDICINE_GROUP.ID__HCGN && hcgn)
                            {
                                _ExpMestMetyReq_HCGNs.Add(item);
                            }
                            else if (dataMedi.MEDICINE_GROUP_ID == IMSys.DbConfig.HIS_RS.HIS_MEDICINE_GROUP.ID__HCHT && hcht)
                            {
  ...
  Config.HisConfigCFG.ODER_OPTION
        ...
        (
         ...
         "TIỀN CHẤT",
           Config.HisConfigCFG.ODER_OPTION
                ...
                #endregion
                #region ---- HCGN ----
                if (_ExpMestMetyReq_HCGNs != null && _ExpMestMetyReq_HCGNs.Count > 0)
                {
                    MPS.Processor.Mps000347.PDO.Mps000347PDO Mps000347PDO = new MPS.Processor.Mps000347.PDO.Mps000347PDO
        (
         _CurrentExpMest,
         ...
         "DƯỢC CHẤT GÂY NGHIỆN",
           Config.HisConfigCFG.ODER_OPTION
                ...
                #endregion
                #region ---- HCHT ----
                if (_ExpMestMetyReq_HCHTs != null && _ExpMestMetyReq_HCHTs.Count > 0)
                {
                    MPS.Processor.Mps000347.PDO.Mps000347PDO Mps000347PDO = new MPS.Processor.Mps000347.PDO.Mps000347PDO
        (
         _CurrentExpMest,
            ...
            try
            {
                ...
                if (keyPrintType == 1)
                {
                    ...
                    if (_ExpMestMetyReq_GN_HTs != null && _ExpMestMetyReq_GN_HTs.Count > 0)
                    {
                        ...
                        result = MpsPrinterRun(null, printTypeCode, fileName, mps000254PDO, MPS.ProcessorBase.PrintConfig.PreviewType.ShowDialog);
                    }
                    #endregion
                    ...
                    MoreInfo
                        ...
                        result = MpsPrinterRun(null, printTypeCode, fileName, mps000254PDODC, MPS.ProcessorBase.PrintConfig.PreviewType.ShowDialog);
                    }
                    #endregion
                }
                else
                {
                    ...
                    if (_ExpMestMetyReq_GNs != null && _ExpMestMetyReq_GNs.Count > 0)
                    {
                        ...
                        result = MpsPrinterRun(null, printTypeCode, fileName, mps000254PDO, MPS.ProcessorBase.PrintConfig.PreviewType.Show);
                    }
                    #endregion
        ...
        private void Mps000349(string printTypeCode, string fileName)
        {
            ...
            try
            {
                MPS.Processor.Mps000349.PDO.Mps000349PDO pdo = new MPS.Processor.Mps000349.PDO.Mps000349PDO(_ExpMest_Sale_Prints, _ExpMestMedi_Sale__GNs, _Transaction_Sale_Print, impMests);
                result = MpsPrinterRun(printTypeCode, printTypeCode, fileName, pdo, MPS.ProcessorBase.PrintConfig.PreviewType.ShowDialog);
            }
            catch (Exception ex)
            {
        ...
        private void Mps000350(string printTypeCode, string fileName)
        {
            ...
            try
            {
                MPS.Processor.Mps000350.PDO.Mps000350PDO pdo = new MPS.Processor.Mps000350.PDO.Mps000350PDO(_ExpMest_Sale_Prints, _ExpMestMedi_Sale__HTs, _Transaction_Sale_Print, impMests);
                result = MpsPrinterRun(printTypeCode, printTypeCode, fileName, pdo, MPS.ProcessorBase.PrintConfig.PreviewType.ShowDialog);
            }
            catch (Exception ex)
            {
        ...
        private void Mps000351(string printTypeCode, string fileName)
        {
            ...
            try
            {
                MPS.Processor.Mps000351.PDO.Mps000351PDO pdo = new MPS.Processor.Mps000351.PDO.Mps000351PDO(_ExpMest_Sale_Prints, _ExpMestMedi_Sale__TPCNs, _ExpMestMaterials_Print, _Transaction_Sale_Print, impMests);
                result = MpsPrinterRun(printTypeCode, printTypeCode, fileName, pdo, MPS.ProcessorBase.PrintConfig.PreviewType.ShowDialog);
            }
            catch (Exception ex)
            {
        ...
        private void Mps000352(string printTypeCode, string fileName)
        {
            ...
            try
            {
                MPS.Processor.Mps000352.PDO.Mps000352PDO pdo = new MPS.Processor.Mps000352.PDO.Mps000352PDO(_ExpMest_Sale_Prints, _ExpMestMedi_Sale__Ts, _Transaction_Sale_Print, impMests);
                result = MpsPrinterRun(printTypeCode, printTypeCode, fileName, pdo, MPS.ProcessorBase.PrintConfig.PreviewType.ShowDialog);
            }
            catch (Exception ex)
            {
        ...
        private void InPhieuHieuTruyenMauVaChePham(string printTypeCode, string fileName, ref bool result)
        {
            try
            {
                ...
                // Lấy các y lệnh xét nghiệm con đính kèm vào y lệnh máu (PARENT_ID = SERVICE_REQ_ID của y lệnh máu)
                List<V_HIS_SERVICE_REQ> listServiceReqTest = null;
                if (lstExpMest != null && lstExpMest.SERVICE_REQ_ID.HasValue)
                {
                    HisServiceReqViewFilter serviceReqTestFilter = new HisServiceReqViewFilter();
                    serviceReqTestFilter.PARENT_ID = lstExpMest.SERVICE_REQ_ID.Value;
                    listServiceReqTest = new BackendAdapter(new CommonParam()).Get<List<V_HIS_SERVICE_REQ>>(HisRequestUriStore.HIS_SERVICE_REQ_GETVIEW, ApiConsumers.MosConsumer, serviceReqTestFilter, new CommonParam());
                }
