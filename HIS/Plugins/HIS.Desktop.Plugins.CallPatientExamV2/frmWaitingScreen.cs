...
namespace HIS.Desktop.Plugins.CallPatientExamV2
{
    public partial class frmWaitingScreen : Form
    {
        ...
        System.Threading.Thread threadCall;
        List<HIS_SERVICE_REQ> ListCall = new List<HIS_SERVICE_REQ>();
        ...
        {
            threadCall = new System.Threading.Thread(() => CallPatientTimer());
            try
            {
                threadCall.Start();
            }
        ...
        private void CallPatientTimer()
        {
            try
            {
                ...
                var itemsToRemove = new List<HIS_SERVICE_REQ>();
                        ...
                        else
                        {
                            switch (word)
                            {
                                case "ROOM_NAME":
                                    ...
                                    string room = Rooms.FirstOrDefault(o => o.ROOM_ID == serviceReq.EXECUTE_ROOM_ID).EXECUTE_ROOM_NAME;
        ...
        private void ProcessShowUcRoom()
        {
            try
            {
                ...
                {
                    UcRoom uc = new UcRoom(ServiceReqGateADO, sortedRoomGateSDOs[i], ProcessDelegateCall);
        ...
        private void ProcessDelegateCall(HIS_SERVICE_REQ PatientCall)
        {
            try
            {
                ListCall.RemoveAll(x => x.EXECUTE_ROOM_ID == PatientCall.EXECUTE_ROOM_ID);
                ListCall.Add(PatientCall);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
