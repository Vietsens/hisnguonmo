...
using HIS.Desktop.Plugins.TreatmentInspectionV2.Resources;
...
{
    public partial class UCTreatmentInspectionV2 : UserControlBase
    {
        int rowCount = 0;
        int dataTotal = 0;
        int start = 0;
        ...
        /// <summary>
        /// Import time of the treatments shown on the current page, keyed by treatment id.
        /// Pre-computed before binding so CustomUnboundColumnData stays cheap.
        /// When a treatment belongs to several batches the latest moment inside the
        /// filtered range is kept.
        /// </summary>
        Dictionary<long, long> dicImportTime = new Dictionary<long, long>();

        const string MODULE_LINK = "HIS.Desktop.Plugins.TreatmentInspectionV2";

        /// <summary>Excel template used to import the inspection list.</summary>
        const string IMPORT_TEMPLATE_FILE_NAME = "IMPORT_RECORD_INSPECTION.xlsx";



        public UCTreatmentInspectionV2(Inventec.Desktop.Common.Modules.Module module)
            : base(module)
        {
            InitializeComponent();
            try
        ...
        private void UCTreatmentInspectionV2_Load(object sender, EventArgs e)
        {
            try
            {
                WaitingManager.Show();
        ...
        private void FillDataToGridTreatment(object param)
        {
            try
            {
                ...
                HIS.Desktop.Plugins.TreatmentInspectionV2.ADO.HisTreatmentView11FilterV2 filter11 =
                    new HIS.Desktop.Plugins.TreatmentInspectionV2.ADO.HisTreatmentView11FilterV2();
                ...
                if (!string.IsNullOrEmpty(txtPATIENTCODE.Text))
                {
                    ...
                    filter11 = new HIS.Desktop.Plugins.TreatmentInspectionV2.ADO.HisTreatmentView11FilterV2();
                ...
                else
                {
                    ...
                    // The warning that blocks the search when no date range is filled is NOT
                    // inherited here: the list is always restricted to the imported inspection
                    // list, so a full table scan cannot happen and the user must be able to open
                    // the module and see the batch straight away.
                    ...
                    if (dtImportTimeFrom.EditValue != null && dtImportTimeFrom.DateTime != DateTime.MinValue)
                    {
                        filter11.IMPORT_TIME_FROM = Inventec.Common.TypeConvert.Parse.ToInt64(
                            Convert.ToDateTime(dtImportTimeFrom.EditValue).ToString("yyyyMMdd") + "000000");
                    }
                    if (dtImportTimeTo.EditValue != null && dtImportTimeTo.DateTime != DateTime.MinValue)
                    {
                        filter11.IMPORT_TIME_TO = Inventec.Common.TypeConvert.Parse.ToInt64(
                            Convert.ToDateTime(dtImportTimeTo.EditValue).ToString("yyyyMMdd") + "235959");
                    }
                ...
                // The module only ever lists treatments that belong to an imported inspection
                // list — a treatment never imported must not appear even when it satisfies
                // every other condition.
                filter11.HAS_RECORD_INSPECTION_IMP = true;
                ...
                if (result != null)
                {
                    ...
                    LoadImportTimeOfCurrentPage(filter11.IMPORT_TIME_FROM, filter11.IMPORT_TIME_TO);
        ...
        /// <summary>
        /// Reads the import moment of the treatments listed on the current page and keeps it in
        /// a dictionary so the "Ngay nhap khau" column costs O(1) per row while painting.
        /// A treatment belonging to several batches keeps the latest moment inside the filtered
        /// range, so it still shows up as a single row.
        /// </summary>
        private void LoadImportTimeOfCurrentPage(long? importTimeFrom, long? importTimeTo)
        {
            try
            {
                dicImportTime = new Dictionary<long, long>();
                if (ListHisTreatment == null || ListHisTreatment.Count == 0) return;
                ...
                filter.TREATMENT_IDs = ListHisTreatment.Select(o => o.ID).Distinct().ToList();
                filter.IMPORT_TIME_FROM = importTimeFrom;
                filter.IMPORT_TIME_TO = importTimeTo;

                CommonParam param = new CommonParam();
                var imports = new BackendAdapter(param).Get<List<HIS.Desktop.Plugins.TreatmentInspectionV2.ADO.RecordInspectionImpADO>>(
                ...
                {
                    long current;
                    if (dicImportTime.TryGetValue(item.TREATMENT_ID, out current))
                    {
                        if (item.IMPORT_TIME > current) dicImportTime[item.TREATMENT_ID] = item.IMPORT_TIME;
                    }
                    else
                    {
                        dicImportTime.Add(item.TREATMENT_ID, item.IMPORT_TIME);
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <summary>Downloads the Excel template used to prepare the inspection list.</summary>
        private void btnDownloadTemplate_Click(object sender, EventArgs e)
        {
            try
            {
                string sourceFile = System.IO.Path.Combine(
                    System.Windows.Forms.Application.StartupPath + "\\Tmp\\Imp\\", IMPORT_TEMPLATE_FILE_NAME);
        ...
        /// <summary>Opens the import window; refreshes the list when a batch was saved.</summary>
        private void btnImportList_Click(object sender, EventArgs e)
        {
            try
            {
                using (frmImportRecordInspection frm = new frmImportRecordInspection(this.currentModule))
                {
                    frm.ShowDialog();
                    if (!frm.HasSaved) return;
                }

                // The batch just saved carries the newest import moment, so clearing the range
                // keeps it visible at the top of the list.
                dtImportTimeFrom.EditValue = null;
                dtImportTimeTo.EditValue = null;
                FillDataToGrid();
            }
            catch (Exception ex)
            {
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.Desktop.Plugins.TreatmentInspectionV2.Resources.Lang", typeof(HIS.Desktop.Plugins.TreatmentInspectionV2.TreatmentInspectionV2.UCTreatmentInspectionV2).Assembly);
                ////Gan gia tri cho cac control editor co Text/Caption/NullText/NullValuePrompt/FindNullPrompt

                this.layoutControl1.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.layoutControl1.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.layoutControl2.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.layoutControl2.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.layoutControl3.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.layoutControl3.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());

                this.txtTreatmentCode.Properties.NullValuePrompt = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.txtTreatmentCode.Properties.NullValuePrompt", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.txtStorageCode.Properties.NullValuePrompt = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.txtStorageCode.Properties.NullValuePrompt", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.txtKeyWord.Properties.NullValuePrompt = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.txtKeyword.Properties.NullValuePrompt", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());

                this.layoutControlItem10.OptionsToolTip.ToolTip = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.layoutControlItem10.OptionsToolTip.ToolTip", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());

                this.btnSearch.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.btnSearch.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.btnUnSave.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.btnUnSave.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.btnDuyetGiamDinh.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.btnDuyetGiamDinh.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.btnDownloadTemplate.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.btnDownloadTemplate.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.btnImportList.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.btnImportList.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.layoutControlItem21.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.layoutControlItem21.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gcImportTime.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gcImportTime.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gcImportTime.ToolTip = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gcImportTime.ToolTip", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.bar2.Text = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.bar2.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());

                this.gridColumn1.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn1.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn2.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn2.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn3.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn3.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn4.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn4.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn5.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn5.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn6.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn6.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn7.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn7.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn8.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn8.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn9.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn9.Caption", ResourceLanguageManager.LanguageResource,
LanguageManager.GetCulture());
                this.gridColumn10.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn10.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn11.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn11.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn12.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn12.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn13.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn13.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn13.ToolTip = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn13.ToolTip", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn14.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn14.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn14.ToolTip = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn14.ToolTip", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn15.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn15.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn15.ToolTip = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn15.ToolTip", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn16.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn16.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn17.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn17.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn18.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn18.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn19.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn19.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn20.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn20.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn21.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn21.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn22.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn22.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn23.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn23.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.gridColumn24.Caption = Inventec.Common.Resource.Get.Value("UCTreatmentInspectionV2.gridColumn24.Caption", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
        ...
        private void SetDefaultControl()
        {
            try
            {
                ...
                // No date range is pre-filled: the list is already restricted to the imported
                // treatments, and intersecting it with "discharged today" would almost always
        ...
        private void LoadComboStatus()
        {
            try
            {
                ...
                // The list is already restricted to the imported treatments, so the status
                // filter starts empty in order to show the whole batch sent by the appraiser.
                cboStatus.EditValue = null;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (e.IsGetData && e.Column.UnboundType != DevExpress.Data.UnboundColumnType.Bound)
                {
                    ...
                    if (data != null)
                    {
                        ...
                        else if (e.Column.FieldName == "IMPORT_TIME_STR")
                        {
                            long importTime;
                            e.Value = dicImportTime.TryGetValue(data.ID, out importTime)
