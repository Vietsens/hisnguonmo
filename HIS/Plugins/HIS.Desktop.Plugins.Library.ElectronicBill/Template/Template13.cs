...
{
    /// <summary>
    /// Mẫu 13 = bản sao của Mẫu 1 (gom dịch vụ BHYT thành 1 dòng đồng chi trả, dịch vụ khác liệt kê theo nhóm),
    /// khác duy nhất một điểm: diện "Điều trị nội trú ban ngày" dùng chung tên dòng với "Điều trị nội trú"
    /// ("... - Viện phí bệnh án nội trú") thay cho tên mặc định "... - Khám bệnh" của Mẫu 1.
    /// Tách thành mẫu riêng để các viện đang dùng Mẫu 1 không bị ảnh hưởng. Chọn mẫu: key HIS.Desktop.Plugins.Library.ElectronicBill.Template = 13.
    /// Khi sửa lỗi ở Mẫu 1 cần soát lại Mẫu 13.
    /// </summary>
    public class Template13 : IRunTemplate
    {
        ...
        public Template13(long _treatmentId, HIS_BRANCH _branch, List<HIS_SERE_SERV_BILL> list, Base.ElectronicBillDataInput dataInput)
        {
        ...
        object IRunTemplate.Run()
        {
            ...
            try
            {
                ...
                {
                    //Mẫu 13: luồng in hoá đơn nháp gọi thẳng bộ tạo mẫu (không qua ElectronicBillProcessor) nên dữ liệu đầu vào
                    //chưa có lần đổi đối tượng thanh toán cuối -> tự tải giống ElectronicBillProcessor.GetCurrentPatientTypeAlter
                    if (DataInput.LastPatientTypeAlter == null)
                    {
                        if (DataInput.Treatment != null && DataInput.Treatment.ID > 0)
                        {
                            CommonParam param = new CommonParam();
                            DataInput.LastPatientTypeAlter = new BackendAdapter(param).Get<V_HIS_PATIENT_TYPE_ALTER>("/api/HisPatientTypeAlter/GetViewLastByTreatmentId", ApiConsumers.MosConsumer, DataInput.Treatment.ID, param);
                        }
                        else //không có hồ sơ điều trị (thanh toán khác): coi như khám để hiển thị toàn bộ
                        {
                            DataInput.LastPatientTypeAlter = new V_HIS_PATIENT_TYPE_ALTER();
                            DataInput.LastPatientTypeAlter.TREATMENT_TYPE_ID = IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__KHAM;
                            DataInput.LastPatientTypeAlter.PATIENT_TYPE_ID = 0;
                        }
                    }
                    ...
                    if (DataInput.LastPatientTypeAlter.PATIENT_TYPE_ID == HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT)
                    {
                        ...
                        if (dicSereServType[SERE_SERV_TYPE.BHYT_NOT_SERVICE_CONFIG] != null && dicSereServType[SERE_SERV_TYPE.BHYT_NOT_SERVICE_CONFIG].Count > 0)
                        {
                            ...
                            //Mẫu 13: "Điều trị nội trú ban ngày" tính là bệnh án nội trú (khác Mẫu 1 - rơi vào tên mặc định "Khám bệnh")
                            else if (DataInput.LastPatientTypeAlter.TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNOITRU
                                || DataInput.LastPatientTypeAlter.TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTBANNGAY)
                            {
                                if (ratio != 100)
                                {
