...
namespace HIS.Desktop.Plugins.ExamServiceReqExecute
{
    ...
    ///
    /// Kiem tra hai buoc, CA HAI buoc deu chay roi moi thong bao MOT lan (moi buoc mot muc trong cung
    /// hop thoai) de nguoi dung thay het viec phai lam, khong bi sua xong buoc 1 lai chan tiep boi buoc 2.
    ///
    /// BUOC 1 - theo TUNG y lenh chi dinh (bo qua y lenh Kham va 4 loai DON vi khong can van ban ky):
    ...
    ///  - y lenh co van ban nhung van ban chua hoan thanh -> CHAN.
    ///
    ...
    /// Xet TOAN BO van ban cua ho so, KHONG bo qua van ban nao - ke ca van ban dang gan vao y lenh va
    /// da duoc buoc 1 neu ten.
    ///
    ...
    /// cho phep luu - van ban se duoc tao va ky ngay sau khi luu (thu tu thuc te khi bam luu:
    /// kiem tra -> luu -> MOI ky, xem SaveExamServiceReq). Khong mien khi loai do da co van ban nhung
    /// con cai chua hoan thanh, vi viec ky chi tao va ky MOT van ban, khong lam xong duoc van ban cung
    /// loai dang do dang san.
    ///
    ...
    /// "api/EmrDocument/GetView" cung tra ve List&lt;V_EMR_DOCUMENT&gt;. Ban EMR.EFMODEL.dll hien tai
    /// trong lib\EMR khong con chua cac kieu view V_* nen khong tham chieu truc tiep duoc (loi CS0570).
    /// Vi vay dung ADO cuc bo MediRecordCheckingDocumentADO cho ca hai api - ban tin la JSON nen chi can
    ...
    public partial class ExamServiceReqExecuteControl
    {
        ...
        /// <summary>
        /// Kiem tra van ban truoc khi cho phep nhap vien.
        /// </summary>
        ...
        private bool CheckEmrDocumentBeforeHospitalize()
        {
            try
            {
                ...
                // ---- BUOC 2: moi loai van ban duoc tich "chan nhap vien" phai co it nhat 1 van ban da hoan thanh ----
                ...
                else
                {
                    ...
                    TraceCheckDocumentHospitalize(string.Format(
                        "buoc 2: ho so {0} co {1}/{2} van ban thuoc loai chan nhap vien. "
                        + "Loai duoc tich chan = [{3}]. Loai thuc te cua tung van ban = [{4}].",
                        treatmentCode,
                ...
                // Mien tru cho nut ky tren UC.Hospitalize - xem ghi chu dau file.
        ...
        /// EmrDocumentTypeFilter khong co dieu kien IS_HOSPITALIZATION, va filter cua BackendAdapter.Get
        /// di qua query string nen truyen danh sach ID vao day la them mot duong tra ve null im lang.
