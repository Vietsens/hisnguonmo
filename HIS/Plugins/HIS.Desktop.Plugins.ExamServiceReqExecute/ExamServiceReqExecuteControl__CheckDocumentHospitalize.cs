...
namespace HIS.Desktop.Plugins.ExamServiceReqExecute
{
    ...
    public partial class ExamServiceReqExecuteControl
    {
        ...
        /// </summary>
        private const string STAMP__CHECK_DOCUMENT_HOSPITALIZE = "log-co-tich-in-va-ky-v15";
        ...
        private bool CheckEmrDocumentBeforeHospitalize()
        {
            try
            {
                ...
                // Doc MOT LAN trang thai cac nut tren UC.Hospitalize va ghi ra log NGAY tai day, khong doi
                // den luc xet mien tru: neu ho so thieu nhieu loai van ban thi khong bao gio toi do,
                // va se khong biet nguoi dung dang tich in hay tich ky.
                HIS.UC.Hospitalize.ADO.HospitalizeExamADO hospitalizeExamAdo = GetHospitalizeExamAdo();
                ...
                // Tach tung ve ra bien rieng roi GHI HET ra log truoc khi quyet dinh.
                // Neu de nguyen mot bieu thuc && thi khi mien tru khong ap dung, log khong cho biet ve nao
                // truot - va && con ngan mach nen IsSignHospitalizeExamChecked() co the khong duoc goi,
                // tuc khong co ca dong "UC.Hospitalize IsSign = ...".
                bool isStep1Clean = serviceReqMessages.Count == 0;
                bool isNoTypeWithUnfinishedDocument = typesWithUnfinishedDocument.Count == 0;
                bool isExactlyOneTypeMissing = typesWithoutDocument.Count == 1;
                bool isMissingTypeHospitalizeExam = isExactlyOneTypeMissing
                    && IsHospitalizeExamDocumentType(typesWithoutDocument[0]);

                bool isReadyToAskSign = isStep1Clean && isNoTypeWithUnfinishedDocument && isMissingTypeHospitalizeExam;
                bool isSignChecked = isReadyToAskSign && hospitalizeExamAdo != null && hospitalizeExamAdo.IsSign;

                TraceCheckDocumentHospitalize(string.Format(
                    "mien tru tich ky: buoc-1-sach={0}, khong-loai-nao-co-van-ban-do-dang={1}, so-loai-thieu-van-ban={2} [{3}], "
                    + "loai-thieu-la-phieu-kham-vao-vien={4}, da-tich-ky={5}.",
                    isStep1Clean,
                    isNoTypeWithUnfinishedDocument,
                    typesWithoutDocument.Count,
                    string.Join(", ", typesWithoutDocument.Select(o => string.Format("{0}#{1}#{2}", o.ID, o.DOCUMENT_TYPE_CODE, o.DOCUMENT_TYPE_NAME))),
                    isMissingTypeHospitalizeExam,
                    isReadyToAskSign ? isSignChecked.ToString() : "(khong xet vi ve tren da truot)"));
                ...
                TraceCheckDocumentHospitalize(string.Format(
                    "KET LUAN: CHAN nhap vien ho so {0} - buoc 1 co {1} dong [{2}], buoc 2 co {3} dong [{4}].",
                    treatmentCode,
                    serviceReqMessages.Count,
                    string.Join(" | ", serviceReqMessages),
                    blockingDocumentMessages.Count,
                    string.Join(" | ", blockingDocumentMessages)));
        ...
        {
            bool isMatch = documentType != null
                && string.Equals((documentType.DOCUMENT_TYPE_CODE ?? "").Trim(), "02", StringComparison.OrdinalIgnoreCase);

            // In nguyen van ma doc duoc: vien nao khai ma khac "02" thi nhin dong nay la ra ngay,
            // khong phai doan tai sao phep mien tru khong chay.
            TraceCheckDocumentHospitalize(string.Format(
                "doi chieu loai phieu kham vao vien: DOCUMENT_TYPE_CODE doc duoc = '{0}', can = '02' => {1}.",
                documentType == null ? "(documentType null)" : (documentType.DOCUMENT_TYPE_CODE ?? "(null)"),
                isMatch));
            return isMatch;
        }

        /// <summary>
        /// Doc trang thai cac nut tren UC.Hospitalize (chkPrintHospitalizeExam / chkSign /
        /// chkPrintDocumentSigned -> IsPrintHospitalizeExam / IsSign / IsPrintSign) va ghi het ra log.
        ///
        /// Goi DUNG MOT LAN o dau phep kiem tra roi dung lai ket qua, vi hai ly do:
        ///  - GetValue cua UC co goi dxValidationProvider1.Validate() nen co tac dung phu ve giao dien,
        ///    goi nhieu lan la ve lai loi validate nhieu lan;
        ///  - phai co log du ca ba co NGAY CA KHI phep kiem tra thoat som, de biet nguoi dung dang tich
        ///    in hay tich ky - neu doi den luc xet mien tru thi ho so thieu nhieu loai se khong bao gio toi do.
        ///
        ...
        /// </summary>
        /// <returns>null khi khong doc duoc - diem goi coi nhu khong tich ky, tuc van chan.</returns>
        private HIS.UC.Hospitalize.ADO.HospitalizeExamADO GetHospitalizeExamAdo()
        {
            try
            {
                if (this.ucHospitalize == null || this.hospitalizeProcessor == null)
                {
                    TraceCheckDocumentHospitalize("UC.Hospitalize: chua nap UC (ucHospitalize hoac hospitalizeProcessor null), coi nhu KHONG tich in, KHONG tich ky.");
                    return null;
                }
                ...
                {
                    TraceCheckDocumentHospitalize("UC.Hospitalize: GetValue tra ve null (co the do dxValidationProvider1.Validate that bai), coi nhu KHONG tich in, KHONG tich ky.");
                    return null;
                }

                TraceCheckDocumentHospitalize(string.Format(
                    "UC.Hospitalize: IsPrintHospitalizeExam (tich in phieu kham vao vien) = {0}, IsSign (tich ky) = {1}, IsPrintSign (in ban da ky) = {2}.",
                    hospitalizeADO.IsPrintHospitalizeExam,
                    hospitalizeADO.IsSign,
                    hospitalizeADO.IsPrintSign));
                return hospitalizeADO;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(LOG__CHECK_DOCUMENT_HOSPITALIZE
                    + "loi khi doc trang thai nut tren UC.Hospitalize, coi nhu KHONG tich in, KHONG tich ky.", ex);
                return null;
            }
        }
