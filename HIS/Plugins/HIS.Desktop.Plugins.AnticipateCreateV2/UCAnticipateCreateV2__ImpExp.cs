...
namespace HIS.Desktop.Plugins.AnticipateCreateV2
{
    public partial class UCAnticipateCreateV2
    {
        ...
        // vCong 52461 — Số liệu dự trù theo LOẠI (gộp toàn viện) từ API GetForAnticipate. Key = typeId.
        internal Dictionary<long, ADO.AnticipateRowADO> dicMediAnticipate = new Dictionary<long, ADO.AnticipateRowADO>();
        internal Dictionary<long, ADO.AnticipateRowADO> dicMateAnticipate = new Dictionary<long, ADO.AnticipateRowADO>();

        // Giá trị người dùng nhập trực tiếp trên cây: SL dự trù + Ghi chú (Ghi chú CHƯA lưu DB — vCong 52461).
        internal Dictionary<long, decimal?> dicMediAnticipateQty = new Dictionary<long, decimal?>();
        internal Dictionary<long, decimal?> dicMateAnticipateQty = new Dictionary<long, decimal?>();
        internal Dictionary<long, string> dicMediAnticipateNote = new Dictionary<long, string>();
        internal Dictionary<long, string> dicMateAnticipateNote = new Dictionary<long, string>();

        // vCong 52461 — ngữ cảnh thầu/giá capture lúc nhập SL dự trù (để build DTO khi Lưu). Key = typeId.
        internal Dictionary<long, long?> dicMediAnticipateSupplierId = new Dictionary<long, long?>();
        internal Dictionary<long, long?> dicMateAnticipateSupplierId = new Dictionary<long, long?>();
        internal Dictionary<long, long?> dicMediAnticipateBidId = new Dictionary<long, long?>();
        internal Dictionary<long, long?> dicMateAnticipateBidId = new Dictionary<long, long?>();
        internal Dictionary<long, decimal> dicMediAnticipatePrice = new Dictionary<long, decimal>();
        internal Dictionary<long, decimal> dicMateAnticipatePrice = new Dictionary<long, decimal>();
        ...
        private void pluginChkHideGroup_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IsMedicineChecked && lstMediInStocks != null)
                {
                    BindPivotTree(true);
                }
                else if (IsMaterialChecked && lstMateInStocks != null)
                {
                    BindPivotTree(false);
                }
            }
        ...
        /// <summary>
        /// vCong 52461 — Gọi API GetForAnticipate (gộp toàn viện) và dựng dictionary số liệu dự trù theo LOẠI:
        /// SL thầu / thầu đã nhập / thầu còn lại, Tồn đầu, Nhập mới, Số sử dụng, Tồn cuối,
        /// Xuất nhiều nhất (+ tháng), Giá sau VAT. Key = typeId.
        /// Không xóa giá trị SL dự trù / Ghi chú người dùng đã nhập (giữ theo typeId qua các lần tính lại).
        /// </summary>
        private void BuildAnticipateData(bool isMedicine, long? fromTime, long? toTime)
        {
            try
            {
                if (isMedicine)
                    dicMediAnticipate = new Dictionary<long, ADO.AnticipateRowADO>();
                else
                    dicMateAnticipate = new Dictionary<long, ADO.AnticipateRowADO>();

                if (this.mediStockIds == null || this.mediStockIds.Count == 0)
                    return;

                CommonParam param = new CommonParam();
                ADO.AnticipateReqFilter filter = new ADO.AnticipateReqFilter();
                filter.MEDI_STOCK_IDs = this.mediStockIds;
                filter.FROM_TIME = fromTime;
                filter.TO_TIME = toTime;
                filter.IS_ACTIVE = 1;
                ...
                string uri = isMedicine
                    ? RequestUriStore.HIS_MEDI_STOCK_METY_GET_FOR_ANTICIPATE
                    : RequestUriStore.HIS_MEDI_STOCK_MATY_GET_FOR_ANTICIPATE;

                var result = new BackendAdapter(param).Post<List<ADO.AnticipateRowADO>>(uri, ApiConsumers.MosConsumer, filter, param);
                if (result == null) return;

                var dic = isMedicine ? dicMediAnticipate : dicMateAnticipate;
                foreach (var item in result)
                {
                    if (item == null) continue;
                    long typeId = isMedicine ? item.MEDICINE_TYPE_ID : item.MATERIAL_TYPE_ID;
                    if (typeId == 0) continue;
                    dic[typeId] = item;
                }
            }
