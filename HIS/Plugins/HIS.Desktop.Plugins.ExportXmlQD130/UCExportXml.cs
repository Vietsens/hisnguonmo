...
namespace HIS.Desktop.Plugins.ExportXmlQD130
{
    public partial class UCExportXml : HIS.Desktop.Utility.UserControlBase
    {
        ...
        //                try
        //                {
        //                    var expMedIds = resultSS
        //                        .Where(x => x.EXP_MEST_MEDICINE_ID.HasValue)
        //                        .Select(x => x.EXP_MEST_MEDICINE_ID.Value)
        //                        .Distinct()
        //                        .ToList();

        //                    var expMatIds = resultSS
        //                        .Where(x => x.EXP_MEST_MATERIAL_ID.HasValue)
        //                        .Select(x => x.EXP_MEST_MATERIAL_ID.Value)
        //                        .Distinct()
        //                        .ToList();

        //                    if (expMedIds.Count > 0 || expMatIds.Count > 0)
        //                    {
        //                        // NOTE: filter + endpoint anh thay đúng theo hệ thống MOS đang có
        //                        var usedFilter = new HisExpMedimateUsedFilter();
        //                        usedFilter.EXP_MEST_MEDICINE_ID = expMedIds;
        //                        usedFilter.EXP_MEST_MATERIAL_ID = expMatIds;

        //                        var usedRs = new Inventec.Common.Adapter.BackendAdapter(param)
        //                            .Get<List<HIS_EXP_MEDIMATE_USED>>("api/HisExpMedimateUsed/Get", ApiConsumers.MosConsumer, usedFilter, param);

        //                        if (usedRs != null && usedRs.Count > 0)
        //                        {
        //                            // tránh trùng (do chạy theo page)
        //                            ListExpMedimateUsed.AddRange(usedRs);
        //                        }
        //                    }
        //                }
        ...
        private void ThreadGetSereServ2(object obj)
        {
            try
            {
                ...
                var usedById = new Dictionary<long, HIS_EXP_MEDIMATE_USED>(); // dedupe theo used.ID
