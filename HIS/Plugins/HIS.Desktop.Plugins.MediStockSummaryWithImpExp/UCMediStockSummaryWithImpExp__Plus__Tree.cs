            ...
            try
            {
                ...
                if (data != null)
                {
                    ...
                    dXmenu.Caption = Inventec.Common.Resource.Get.Value(
                        ...
                        Base.ResourceLangManager.LanguageUCMediStockSummaryWithImpExp,
            ...
            try
            {
                if (mediStockAdo != null)
                {
                    ...
                    if ((mediStockAdo.AvailableAmount ?? 0) < (mediStockAdo.TotalAmount ?? 0))
                    {
                        ...
                        MessageBox.Show(Inventec.Common.Resource.Get.Value("IVT_LANGUAGE_KEY__UC_MEDI_STOCK_SUMMARY_KHA_DUNG_BANG_TON_KHO", Base.ResourceLangManager.LanguageUCMediStockSummaryWithImpExp, Inventec.Desktop.Common.LanguageManager.LanguageManager.GetCulture()), "", MessageBoxButtons.OK);
                }
            }
            catch (Exception ex)
            {
        ...
        private List<DevExpress.Utils.Menu.DXMenuItem> menuItemMaterial(HisMaterialInStockADO data, TreeListNode node)
        {
            try
            {
                ...
                if (data != null)
                {
                    ...
                    dXmenu.Caption = Inventec.Common.Resource.Get.Value("IVT_LANGUAGE_KEY__UC_MEDI_STOCK_SUMMARY_XEM_LICH_SU_XUAT_NHAP_VAT_TU", HIS.Desktop.Plugins.MediStockSummaryWithImpExp.Base.ResourceLangManager.LanguageUCMediStockSummaryWithImpExp, Inventec.Desktop.Common.LanguageManager.LanguageManager.GetCulture());
                    dXmenuItem.Add(dXmenu);
                    if (this.mediStockIds != null && mediStockIds.Count == 1)
                    {
                        dxMenu1.Click += Material_RightMouseClick_TraKD;
                        dxMenu1.Caption = Inventec.Common.Resource.Get.Value("IVT_LANGUAGE_KEY__UC_MEDI_STOCK_SUMMARY_TRA_KHA_DUNG_VT", HIS.Desktop.Plugins.MediStockSummaryWithImpExp.Base.ResourceLangManager.LanguageUCMediStockSummaryWithImpExp, Inventec.Desktop.Common.LanguageManager.LanguageManager.GetCulture());
                        dXmenuItem.Add(dxMenu1);
                    }
