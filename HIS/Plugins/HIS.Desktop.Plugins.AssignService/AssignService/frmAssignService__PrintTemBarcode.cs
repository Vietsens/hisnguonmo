...
namespace HIS.Desktop.Plugins.AssignService.AssignService
{
    ...
    {
        // In tem barcode truc tiep qua BarTender khi tich chon "In tem barcode" (gridView7_5)
        // trong bang thiet lap in (config HIS.Desktop.Plugins.IsPrintTemBarcodeBartender = 1).
        // Du lieu lay tu serviceReqComboResultSDO (ket qua luu chi dinh):
        // XN -> mau Mps000423, GPBL -> mau Mps000425 (Tmp/TempBartend/...).
        ...
        private void InTemBarcodeGpbl()
        {
            try
            {
                ...
                var serviceReqGpbl = this.serviceReqComboResultSDO.ServiceReqs.Where(o => o.SERVICE_REQ_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_REQ_TYPE.ID__GPBL).ToList();
                string txt = "";
                if (serviceReqGpbl != null && serviceReqGpbl.Count > 0)
                {
                    foreach (var item in serviceReqGpbl)
                    {
                        var sereServBySRQ = this.serviceReqComboResultSDO.SereServs != null ? this.serviceReqComboResultSDO.SereServs.Where(o => o.SERVICE_REQ_ID == item.ID).ToList() : null;
                        if (sereServBySRQ != null && sereServBySRQ.Count() > 0)
                        {
        ...
        private void GenTextTemBarcodeGpbl(V_HIS_SERVICE_REQ serviceReq, V_HIS_SERE_SERV sereServ, ref string txt)
        {
            var service = BackendDataWorker.Get<HIS_SERVICE>().FirstOrDefault(o => o.ID == sereServ.SERVICE_ID);
            HIS_SERVICE servicePr = null;
            if (service != null && service.PARENT_ID.HasValue && service.PARENT_ID.Value > 0)
