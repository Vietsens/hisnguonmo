...
{
    public partial class frmWaitingScreen_ExpMest_SeparateScreen : FormBase
    {
        internal MOS.EFMODEL.DataModels.HIS_EXP_MEST hisExpMest;
        ...
        public frmWaitingScreen_ExpMest_SeparateScreen(Inventec.Desktop.Common.Modules.Module module
        , MOS.EFMODEL.DataModels.HIS_EXP_MEST hisExpMest, List<MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT> ServiceReqStts)
            : base(module)
        {
            InitializeComponent();
...
(gridpatientBodyForceColorCodes[0], gridpatientBodyForceColorCodes[1], gridpatientBodyForceColorCodes[2]);
        ...
        private void gridViewWatingExams_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    MOS.EFMODEL.DataModels.HIS_EXP_MEST data = (MOS.EFMODEL.DataModels.HIS_EXP_MEST)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    if (data != null)
                    {
        ...
        private void gridViewWaitingCls_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
            try
            {
                var result = (HIS_EXP_MEST)gridViewWaitingCls.GetRow(e.RowHandle);

                if (result != null && this.hisExpMest != null)
                {
                    if (result.NUM_ORDER == this.hisExpMest.NUM_ORDER)
                    {
                        if (countTimer > 0)
                        {
                            countTimer--;
        ...
        private void gridViewWaitingCls2_RowStyle(object sender, RowStyleEventArgs e)
        {
            try
            {
                var result = (HIS_EXP_MEST)gridViewWaitingCls2.GetRow(e.RowHandle);
        ...
        void FillDataToDictionaryWaitingPatient(List<MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT> serviceReqStts)
        {
            try
            {
                ...
                if (serviceReqStts != null && serviceReqStts.Count > 0)
                {
                    ...
                    hisExpMestFilter.EXP_MEST_TYPE_IDs = lstServiceReqSTTFilter;
                }
                ...
                //var result = BackendDataWorker.Get<HIS_EXP_MEST>().ToList();
        ...
        private List<HIS.Desktop.LocalStorage.BackendData.ADO.ServiceReq1ADO> ConnvertListServiceReq1ToADO(List<HIS_EXP_MEST> serviceReq1s)
        {
            ...
            try
            {
                foreach (var item in serviceReq1s)
                {
                    ...
                    AutoMapper.Mapper.CreateMap<HIS_EXP_MEST, HIS.Desktop.LocalStorage.BackendData.ADO.ServiceReq1ADO>();
        ...
        private void SetDataToCurrentCallPatient()
        {
            try
            {
                if (CallPatientDataWorker.DicCallPatient != null && CallPatientDataWorker.DicCallPatient.Count > 0 && CallPatientDataWorker.DicCallPatient[room.ID] != null && CallPatientDataWorker.DicCallPatient[room.ID].Count > 0)
                {
                    List<ServiceReq1ADO> PatientIsCall = (this.serviceReqStts != null && this.serviceReqStts.Count > 0) ? CallPatientDataWorker.DicCallPatient[room.ID].Where(o => this.serviceReqStts.Select(p => p.ID).Contains(o.EXP_MEST_STT_ID)).Where(o => o.CallPatientSTT).ToList() : null;
        ...
        private void SetDataToGridControlWaitingCLSs()
        {
            try
            {
                if (CallPatientDataWorker.DicCallPatient != null && CallPatientDataWorker.DicCallPatient.Count > 0 && CallPatientDataWorker.DicCallPatient[room.ID] != null && CallPatientDataWorker.DicCallPatient[room.ID].Count > 0)
                {
                    ...
                    var ServiceReqFilterSTTs = CallPatientDataWorker.DicCallPatient[room.ID].Where(o => this.serviceReqStts.Select(p => p.ID).Contains(o.EXP_MEST_STT_ID)).OrderBy(o => o.PRES_NUMBER).ThenBy(o => o.NUM_ORDER).ToList();
                    ...
                    gridControlWaitingCls2.DataSource = ServiceReqFilterSTTs;
        ...
        private void nhapNhay(object data)
        {
            try
            {
                ...
                Inventec.Common.Mapper.DataObjectMapper.Map<HIS_EXP_MEST>(this.hisExpMest, data);
