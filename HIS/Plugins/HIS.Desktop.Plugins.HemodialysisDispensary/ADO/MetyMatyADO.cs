...
namespace HIS.Desktop.Plugins.HemodialysisDispensary.ADO
{
    /// <summary>
    /// Dòng chi tiết thuốc trên lưới phải-dưới (Chi tiết thuốc + Còn lại + dấu (+)).
    /// Bọc chung V_HIS_SERVICE_REQ_METY (đơn BS — có KIDNEY_AMOUNT_LEFT) và
    /// HIS_EXP_MEST_MEDICINE (chi tiết thuốc phiếu xuất) để hiển thị thống nhất.
    /// </summary>
    public class MetyMatyADO
    {
        public V_HIS_SERVICE_REQ_METY ReqMety { get; set; }
        ...
        public decimal KidneyAmountLeft { get; set; }
        ...
        public MetyMatyADO(V_HIS_SERVICE_REQ_METY reqMety, V_HIS_MEDICINE_TYPE medicineType)
        {
            if (reqMety != null)
            {
                ...
                this.KidneyAmountLeft = reqMety.KIDNEY_AMOUNT_LEFT ?? 0;
            }
            if (medicineType != null)
            {
                this.MedicineTypeCode = medicineType.MEDICINE_TYPE_CODE;
