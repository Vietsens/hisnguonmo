    ...
    public partial class UCTreatmentList : UserControlBase
    {
        ...
        private void InGiayChuyenVien(string printTypeCode, string fileName, ref bool result)
        {
            try
            {
                ...
                LoadServiceReq(treatment.ID);
                CommonParam paramDhst = new CommonParam();
                HisDhstFilter DhstFilter = new HisDhstFilter();
                HIS_DHST dhst = new HIS_DHST();
                if (serviceReqExamEndType != null && serviceReqExamEndType.EXAM_END_TYPE == 3)
                {
                    DhstFilter.ID = serviceReqExamEndType.DHST_ID;
                    dhst = new BackendAdapter(paramDhst)
                        .Get<List<MOS.EFMODEL.DataModels.HIS_DHST>>("api/HisDhst/Get", ApiConsumers.MosConsumer, DhstFilter, paramDhst).FirstOrDefault();
                }
                else
                {
                    DhstFilter.TREATMENT_ID = treatment.ID;
                    List<HIS_DHST> listDhst = new BackendAdapter(paramDhst)
                        .Get<List<MOS.EFMODEL.DataModels.HIS_DHST>>("api/HisDhst/Get", ApiConsumers.MosConsumer, DhstFilter, paramDhst).ToList();
                    if (listDhst != null && listDhst.Count > 0)
                    {
                        dhst = listDhst.OrderByDescending(o => o.EXECUTE_TIME).ThenByDescending(o => o.ID).FirstOrDefault();
                    }
                }
