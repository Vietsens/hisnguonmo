            ...
            try
            {
                ...
                //else
                {
                 ...
                 BackendDataWorker.Get<HIS_MEDICINE_USE_FORM>()
                        ...
                        Inventec.Common.SignLibrary.ADO.InputADO inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode((!string.IsNullOrEmpty(this.hisExpMest?.TDL_TREATMENT_CODE) ? this.hisExpMest?.TDL_TREATMENT_CODE : printTypeCode), printTypeCode, this.currentModule.RoomId);
                        PrintData.EmrInputADO = inputADO;
                        result = MPS.MpsPrinter.Run(PrintData);
                    }
                    #endregion
                ...
                if (keyPrintType == 1)
                {
                    ...
                    if (this._ExpMestMetyReq_GN_HTs != null && this._ExpMestMetyReq_GN_HTs.Count > 0)
                    {
                     ...
                     keyPhieuTra
                        ...
                        Inventec.Common.SignLibrary.ADO.InputADO inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode((!string.IsNullOrEmpty(this.resultSdo?.ExpMest?.TDL_TREATMENT_CODE) ? this.resultSdo.ExpMest.TDL_TREATMENT_CODE : printTypeCode), printTypeCode, this.roomId);
                        PrintData.EmrInputADO = inputADO;
                        result = MPS.MpsPrinter.Run(PrintData);
                    }
                    #endregion
