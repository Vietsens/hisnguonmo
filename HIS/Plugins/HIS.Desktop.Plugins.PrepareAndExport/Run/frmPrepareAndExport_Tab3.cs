            ...
            try
            {
                ...
                if (data != null)
                {
                    if (data.EXP_MEST_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_TYPE.ID__THPK)
                    {
                        ...
                        res.ReqRoomId = this.currentModule.RoomId;
                        if (gcPrepareMedicine.DataSource != null)
                        {
                            var apiresul = new Inventec.Common.Adapter.BackendAdapter(param).Post<HIS_EXP_MEST>("api/HisExpMest/AggrExamUnapprove", ApiConsumer.ApiConsumers.MosConsumer, res, param);
                            if (apiresul != null)
                            {
                                foreach (var item in lstAll)
                                {
