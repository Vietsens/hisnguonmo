...
namespace HIS.UC.ExamTreatmentFinish
{
    public partial class frmSecondaryIcd : FormBase
    {
        ...
        public frmSecondaryIcd(DelegateRefeshIcdChandoanphu delegateIcds, string icdCodes, string icdNames, long _limit)
        {
            InitializeComponent();
            try
            {
                ...
                icdAdoChecks = (from m in BackendDataWorker.Get<HIS_ICD>().Where(o => o.IS_TRADITIONAL == (short)1) select new IcdADO(m, codes)).ToList();
                limit = (int)_limit;
            }
            catch (Exception ex)
            {
        ...
        private void Language_secondaryDisease()
        {
            try
            {
                if (ResourceLangManager.LanguageUCHospitalize == null)
                {
