...
using HIS.UC.ExecuteRoomView.Popup;
using DevExpress.XtraBars;

namespace HIS.UC.ExecuteRoomView
{
    public partial class UC_ExecuteRoom : UserControl
    {
        ExecuteRoomViewInitADO initADO = null;
        ...
        HIS.UC.ExecuteRoomView.ExecuteRoomViewADO currentAdo;
        DevExpress.XtraBars.BarManager barManager1;
        PopupMenuProcessor popupMenuProcessor;

        public UC_ExecuteRoom(ExecuteRoomViewInitADO ado)
        {
            InitializeComponent();
            try
            {
        ...
        public object GetDataGridView()
        {
            ...
            try
            {
                result = (List<HIS.UC.ExecuteRoomView.ExecuteRoomViewADO>)gridControlExecuteRoom.DataSource;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void gridViewRoom_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    var data = (V_HIS_EXECUTE_ROOM)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        internal void Reload(List<ExecuteRoomViewADO> data)
        {
            try
            {
                if (this.initADO != null)
        ...
        internal void ReloadColumn(List<ExecuteRoomViewColumn> data)
        {
            try
            {
                if (data != null && data.Count > 0)
                {
                    ...
                    {
                        foreach (GridColumn header in this.gridViewExecuteRoom.Columns)
                        {
                            if (item.FieldName == header.FieldName)
                            {
                                header.Visible = item.Visible;
                                header.VisibleIndex = item.VisibleIndex;
                                header.Width = item.ColumnWidth;
                                header.Image = item.image;
                                header.ImageAlignment = item.imageAlignment;
                                header.OptionsColumn.AllowEdit = item.AllowEdit;
                                header.Caption = item.Caption;
                                header.ToolTip = item.ToolTip;
                                //col.Visible=visible;
                                if (item.UnboundColumnType != null && item.UnboundColumnType != UnboundColumnType.Bound)
                                    header.UnboundType = item.UnboundColumnType;
                                if (item.Format != null)
                                {
                                    header.DisplayFormat.FormatString = item.Format.FormatString;
                                    header.DisplayFormat.FormatType = item.Format.FormatType;
                                }
                            }
                        }
                    }
        ...
        private void gridViewExecuteRoom_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var data = (ExecuteRoomViewADO)gridViewExecuteRoom.GetRow(e.RowHandle);
                    if (data != null)
                    {
                        //if (data.isKeyChoose)
                        //{
                        ...
                        if (e.Column.FieldName == "isPriorityRequire")
                        {
                            if (data.isKeyChoose)
                            {
                                e.RepositoryItem = CheckDisable;
                        ...
                        if (e.Column.FieldName == "isHoldOrder")
                        {
                            if (data.isKeyChoose)
                            {
                                e.RepositoryItem = CheckDisable;
                        ...
                        } if (e.Column.FieldName == "isAllowRequest")
                        {
                            if (data.isKeyChoose)
                            {
                                e.RepositoryItem = CheckDisable;
        ...
        private void RadioEnable_Click(object sender, EventArgs e)
        {
            try
            {
                var row = (ExecuteRoomViewADO)gridViewExecuteRoom.GetFocusedRow();
        ...
        private void gridViewExecuteRoom_CellValueChanged(object sender, CellValueChangedEventArgs e)
        {
            try
            {
                ...
                var data = (ExecuteRoomViewADO)gridViewExecuteRoom.GetFocusedRow();
                if (data != null && data is ExecuteRoomViewADO && this.gridView_CellValueChanged != null)
                {
        ...
        private void gridView_PopupMenuShowing(object sender, PopupMenuShowingEventArgs e)
        {
            try
            {
                currentAdo = (HIS.UC.ExecuteRoomView.ExecuteRoomViewADO)gridViewExecuteRoom.GetFocusedRow();
