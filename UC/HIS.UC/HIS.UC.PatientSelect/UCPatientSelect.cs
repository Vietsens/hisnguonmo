...
namespace HIS.UC.PatientSelect.Run
{
    public partial class UCPatientSelect : UserControl
    {
        ...
        Dictionary<long, bool> dicPescriotionPerious;
        ...
        private void GetPrescriptionPerious()
        {
            try
            {
                List<HIS_EXP_MEST> lst = new List<HIS_EXP_MEST>();
                int skip = 0;
                while (this.ListTreatmentBedRooms.Count - skip > 0)
                {
                    var listIds = this.ListTreatmentBedRooms.Skip(skip).Take(100).ToList();
                    skip += 100;
                    ...
                    expMestFilter.TDL_PATIENT_IDs = listIds.Select(o => o.PATIENT_ID).ToList();
                ...
                {
                    this.dicPescriotionPerious = new Dictionary<long, bool>();
                    this.dicPescriotionPerious = lst.GroupBy(o => o.TDL_PATIENT_ID ?? 0).ToDictionary(o => o.Key, o => o.ToList().Count > 0);
                }
            }
            catch (Exception ex)
            {
