...
namespace HIS.UC.Icd
{
    public partial class UCIcd : UserControl
    {
        ...
        private void UCIcd_Load(object sender, EventArgs e)
        {
            try
            {
                if (this.InitAdo.hisTreatment != null)
                {
                    checkIcd = new Desktop.Plugins.Library.CheckIcd.CheckIcdManager(DelegateIcd, this.InitAdo.hisTreatment);
                }
        ...
        public bool ValidationIcd()
        {
            ...
            try
            {
                ...
                else
                {
                    //chkEditIcd.Focus();
                    this.positionHandle = -1;
                    result = (dxValidationProvider2.Validate()) && String.IsNullOrWhiteSpace(txtIcdCode.ErrorText);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(searchCode))
                {
                    ...
                    if (result != null && result.Count > 0)
                    {
                        ...
                        cboIcds.EditValue = result.First().ID;
        ...
        private void DelegateIcd(string icdCodes, string icdNames)
        {
            try
            {
                this.refreshSubIcd(icdCodes, icdNames);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        /// <summary>
        ///Hàm xét ngôn ngữ cho giao diện UCIcd
        /// </summary>
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResourceUCIcd = new ResourceManager("HIS.UC.Icd.Resources.Lang", typeof(UCIcd).Assembly);

                ////Gan gia tri cho cac control editor co Text/Caption/ToolTip/NullText/NullValuePrompt/FindNullPrompt
                this.layoutControl1.Text = Inventec.Common.Resource.Get.Value("UCIcd.layoutControl1.Text", Resources.ResourceLanguageManager.LanguageResourceUCIcd, LanguageManager.GetCulture());
                this.cboIcds.Properties.NullText = Inventec.Common.Resource.Get.Value("UCIcd.cboIcds.Properties.NullText", Resources.ResourceLanguageManager.LanguageResourceUCIcd, LanguageManager.GetCulture());
                this.gridLookUpEdit1View.OptionsFind.FindNullPrompt = Inventec.Common.Resource.Get.Value("UCIcd.gridLookUpEdit1View.OptionsFind.FindNullPrompt", Resources.ResourceLanguageManager.LanguageResourceUCIcd, LanguageManager.GetCulture());
                this.chkEditIcd.Properties.Caption = Inventec.Common.Resource.Get.Value("UCIcd.chkEditIcd.Properties.Caption", Resources.ResourceLanguageManager.LanguageResourceUCIcd, LanguageManager.GetCulture());
                this.lciIcdText.OptionsToolTip.ToolTip = Inventec.Common.Resource.Get.Value("UCIcd.lciIcdText.OptionsToolTip.ToolTip", Resources.ResourceLanguageManager.LanguageResourceUCIcd, LanguageManager.GetCulture());
                this.lciIcdText.Text = Inventec.Common.Resource.Get.Value("UCIcd.lciIcdText.Text", Resources.ResourceLanguageManager.LanguageResourceUCIcd, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
