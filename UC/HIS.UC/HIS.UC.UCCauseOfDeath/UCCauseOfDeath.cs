...
namespace HIS.UC.UCCauseOfDeath
{
    public partial class UCCauseOfDeath : UserControl
    {
        ...
        private String[] ArrNameRowDefault = new String[] { "Nguyên nhân tử vong trực tiếp (trước ngừng thở, ngừng tim)", "Nguyên nhân gây ra 1 (a)", "Nguyên nhân gây ra 1 (b)", "Nguyên nhân gây ra 1 (c)" };
        ...
        internal void ReadOnlyAll(bool isReadOnly)
        {
            try
            {
                ...
                {
                    item.OptionsColumn.AllowEdit = !isReadOnly;
                }
                foreach (GridColumn item in gridView2.Columns)
                {
                    item.OptionsColumn.AllowEdit = !isReadOnly;
                }
                foreach (GridColumn item in gridView3.Columns)
                {
                    item.OptionsColumn.AllowEdit = !isReadOnly;
                }
            }
        ...
        private void InitData()
        {
            try
            {
                ...
                IsEdit = false;
                ArrDeathType = new CheckEdit[] { chkDeathType1, chkDeathType2, chkDeathType3 };
        ...
        internal object GetValue()
        {
            ...
            try
            {
                ...
                if (ArrDeathType.FirstOrDefault(o => o.Checked) != null)
                    severe.DEATH_TYPE = (short)(ArrDeathType.ToList().IndexOf(ArrDeathType.FirstOrDefault(o => o.Checked)) + 1);
        ...
        private void FillControl()
        {
            try
            {
                if (Treatment != null)
                {
                    ...
                    this.lblPatientDob.Text = Treatment.TDL_PATIENT_IS_HAS_NOT_DAY_DOB == 1 ? Int64.Parse(Treatment.TDL_PATIENT_DOB.ToString().Substring(0, 4)).ToString() : Inventec.Common.DateTime.Convert.TimeNumberToDateString(Treatment.TDL_PATIENT_DOB);
                ...
                if (SevereIllNessInfo != null)
                {
                    ...
                    if (SevereIllNessInfo.DEATH_TYPE != null)
                        ArrDeathType[(SevereIllNessInfo.DEATH_TYPE ?? 0) - 1].Checked = true;
