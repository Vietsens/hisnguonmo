...
{
    public partial class UCMotherName : UserControlBase
    {
        #region Declare
        ...
        public UCMotherName()
        {
            try
            {
                ...
                this.txtMotherName.TabIndex = this.TabIndex;
                this.SetCaptionByLanguageKeyNew();
                SetMaxlength(txtMotherName, 100);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        ///Hàm xét ngôn ngữ cho giao diện UCMotherName
        /// </summary>
        private void SetCaptionByLanguageKeyNew()
        {
            try
            {
                ...
                ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.UC.PlusInfo.Resources.Lang", typeof(UCMotherName).Assembly);

                ////Gan gia tri cho cac control editor co Text/Caption/ToolTip/NullText/NullValuePrompt/FindNullPrompt
                this.layoutControl1.Text = Inventec.Common.Resource.Get.Value("UCMotherName.layoutControl1.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.lciMotherName.OptionsToolTip.ToolTip = Inventec.Common.Resource.Get.Value("UCMotherName.lciMotherName.OptionsToolTip.ToolTip", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.lciMotherName.Text = Inventec.Common.Resource.Get.Value("UCMotherName.lciMotherName.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void UCMotherName_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                this.lciMotherName.Text = Inventec.Common.Resource.Get.Value("UCPlusInfo.UCMotherName", HIS.UC.PlusInfo.ShareMethod.ResourceLanguageManager.ResourceUCPlusInfo, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        internal string GetValue()
        {
            try
            {
                if (!String.IsNullOrEmpty(this.txtMotherName.Text))
                    return this.txtMotherName.Text;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        internal void SetValue(string MotherName)
        {
            try
            {
                this.txtMotherName.Text = MotherName;
                //this.txtMotherName.TabIndex = this.TabIndex;
            }
            catch (Exception ex)
            {
        ...
        internal void DisposeControl()
        {
            try
            {
                ...
                this.Load -= new System.EventHandler(this.UCMotherName_Load);
