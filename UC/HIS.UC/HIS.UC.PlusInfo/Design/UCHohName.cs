...
{
    public partial class UCHohName : UserControlBase
    {
        #region Declare
        ...
        public UCHohName()
        {
            try
            {
                ...
                this.txtHohName.TabIndex = this.TabIndex;
                SetMaxlength(txtHohName, 100);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        ///Hàm xét ngôn ngữ cho giao diện UCHohName
        /// </summary>
        private void SetCaptionByLanguageKeyNew()
        {
            try
            {
                ...
                ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.UC.PlusInfo.Resources.Lang", typeof(UCHohName).Assembly);

                ////Gan gia tri cho cac control editor co Text/Caption/ToolTip/NullText/NullValuePrompt/FindNullPrompt
                this.layoutControl1.Text = Inventec.Common.Resource.Get.Value("UCHohName.layoutControl1.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.lciHohName.Text = Inventec.Common.Resource.Get.Value("UCHohName.lciHohName.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                this.lciHohName.Text = Inventec.Common.Resource.Get.Value("UCPlusInfo.UCHohName", HIS.UC.PlusInfo.ShareMethod.ResourceLanguageManager.ResourceUCPlusInfo, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        internal string GetValue()
        {
            try
            {
                if (!String.IsNullOrEmpty(this.txtHohName.Text))
                    return this.txtHohName.Text;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        internal void SetValue(string HohNameNumber)
        {
            try
            {
                this.txtHohName.Text = HohNameNumber;
