...
namespace HIS.UC.Speciality
{
    public partial class UCSpeciality : UserControl
    {
        ...
        public object GetDataGridView()
        {
            ...
            try
            {
                result = (List<HIS.UC.Speciality.SpecialityADO>)gridControlSpeciality.DataSource;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void UpdateDataSource()
        {
            try
            {
                gridControlSpeciality.BeginUpdate();
                gridControlSpeciality.DataSource = this.initADO.ListSpeciality;
                gridControlSpeciality.EndUpdate();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void ProcessColumn()
        {
            try
            {
                if (this.initADO.ListSpecialityColumn != null && this.initADO.ListSpecialityColumn.Count > 0)
                {
                    foreach (var item in this.initADO.ListSpecialityColumn)
                    {
                        GridColumn col = gridViewSpeciality.Columns.AddField(item.FieldName);
        ...
        private void gridViewSpeciality_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                if (e.IsGetData && e.Column.UnboundType != UnboundColumnType.Bound && e.ListSourceRowIndex >= 0)
                {
                    var data = (HIS_SPECIALITY)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
        ...
        internal void Reload(List<SpecialityADO> data)
        {
            try
            {
                if (this.initADO != null)
                {
                    this.initADO.ListSpeciality = data;
                    UpdateDataSource();
                }
            }
            catch (Exception ex)
        ...
        private void gridViewSpeciality_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var data = (SpecialityADO)gridViewSpeciality.GetRow(e.RowHandle);
                    if (data != null)
                    {
        ...
        private void repositoryItemRadio_Enable_Click(object sender, EventArgs e)
        {
            try
            {
                var row = (HIS_SPECIALITY)gridViewSpeciality.GetFocusedRow();
                foreach (var item in this.initADO.ListSpeciality)
                {
                    if (item.ID == row.ID)
                    {
                        item.radio1 = true;
                ...
                gridControlSpeciality.RefreshDataSource();
                ...
                gridViewSpeciality.LayoutChanged();
            }
            catch (Exception ex)
            {
        ...
        private void gridViewSpeciality_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                var row = (HIS_SPECIALITY)gridViewSpeciality.GetFocusedRow();
                if (row != null && this.GridViewSpeciality_MouseDown != null)
                {
                    this.GridViewSpeciality_MouseDown(sender, e);
                }
            }
            catch (Exception ex)
            {
        ...
        private void gridView_PopupMenuShowing(object sender, PopupMenuShowingEventArgs e)
        {
            try
            {
                currentAdo = (HIS.UC.Speciality.SpecialityADO)gridViewSpeciality.GetFocusedRow();
