...
namespace HIS.UC.TreeSereServ7
{
    public class SereServADO : V_HIS_SERE_SERV_7
    {
        ...
        public SereServADO(V_HIS_SERE_SERV_7 service, List<HIS_SERVICE_REQ> serviceReqs)
        {
            if (service != null)
            {
                Inventec.Common.Mapper.DataObjectMapper.Map<SereServADO>(this, service);
                IsExpend = (service.IS_EXPEND == 1);
                HIS_SERVICE_REQ serviceReq = serviceReqs.FirstOrDefault(o => o.ID == service.SERVICE_REQ_ID);
                if (serviceReq != null)
                {
                    this.SAMPLE_TIME = serviceReq.SAMPLE_TIME;
                    this.RECEIVE_SAMPLE_TIME = serviceReq.RECEIVE_SAMPLE_TIME;
                }
            }
        }
        ...
        public SereServADO(DHisSereServ2 service)
        {
            if (service != null)
            {
                ...
                this.TDL_REQUEST_DEPARTMENT_ID = service.REQUEST_DEPARTMENT_ID ?? 0;
                this.TDL_REQUEST_ROOM_ID = service.REQUEST_ROOM_ID ?? 0;
            }
