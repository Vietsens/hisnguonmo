...
namespace HIS.UC.UCPatientRaw
{
    public partial class UCPatientRaw : HIS.Desktop.Utility.UserControlBase
    {
                            ...
                            if (strTemp.Contains("|") || (strValue.Trim().Length == 12 && !string.IsNullOrEmpty(txtPatientName.Text) && (!string.IsNullOrEmpty(txtPatientDob.Text) || dtPatientDob.EditValue != null)))
                            {
                                ...
                                if (dataQr != null)
                                {
                                    ...
                                    if (strTemp.Contains("|"))
                                    {
                                        ...
                                        var communes = BackendDataWorker.Get<V_SDA_COMMUNE>();
                                        Inventec.Common.Address.AddressProcessor adProc = new Inventec.Common.Address.AddressProcessor(provinces, districts, communes);
        ...
        bool IsChangeStrucAdreess = false;

        public void GetStateChangeStrucAddess(bool IsChangeStrucAdreess)
        {
            this.IsChangeStrucAdreess = IsChangeStrucAdreess;
        }
